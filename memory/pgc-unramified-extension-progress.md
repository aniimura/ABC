...
## ★★★★★★★★★★★★★★★2026-09-05: Corollary 3.3 も偽だった——`ρ` と `ρ'` が無関係

`Check/PGC/Cor33Degenerate.lean`(新規)+ `Skeleton/PGC/Section3.lean::cor_3_3` の修理。

原文は**ひとつの** `V` を `α` で移して見比べるが、旧形は `ρ` と `ρ'` を
**互いに無関係な**パラメータで受け取っていた。`K' = K`・`α = 恒等` を取ると
「どんな2つの表現も uniformizing 性が一致する」と言っていることになる。

**反例**(`cor_3_3_statement_false`、sorry 無し):
- **真になる側**: `E := K`・`ι := id`・`I := U_K`。★**本日無条件化した
  `Γ_K ↠ 𝒪_K^×`(第 962)**から `ρ := Γ_K ↠ 𝒪_K^× ↪ K^×`、
  `toGal := その全射の切断(選択)`。`ρ(toGal x) = x` が `U_K` 全体で成立。
- **偽になる側**: 自明な表現 `ρ' = 1`(`not_isUniformizing_one`)。

★2026-09-04 の監査は「真になる `ρ` を作るには相互律相当が要る」で止まって
「反証は効かない」と結論していた。**その相互律の半分が今日入ったので、
初めて反証できた**——監査の結論は道具の在庫に依存する、という実例。

**修理**: `∀ g, ρ' (α.equiv g) = ρ g` を課した。`K'=K`・`α=恒等` なら
`ρ' = ρ` が強制され、反例は塞がる。
...
## ★★★★★★★★★★★★★★★★★2026-09-05: 「Γ_{ℚ_p} は非可換」への2本の柱

`Found/PGC/QpRootsOfUnity.lean`(新規)

- **`eq_one_of_pow_eq_one_of_norm_one`** : `ℓ` 素数・`ℓ ≠ p`・`ℓ ∤ q-1` なら
  `K` の中の `ℓ` 乗根は 1 だけ。★本日の `card_primeToPTorsion`
  (`|μ^{(p')}(𝒪_K)| = q-1`)の**最初の応用**。
- **`eq_one_of_pow_eq_one_qp`** : `ℚ_p` 版(`q = p`)
- **`not_isPow_p`** : `ℚ_p` の値群は `p^ℤ` なので `n ≥ 2` では `b^n = p` は
  `n·v(b) = 1` を要求して不可能
- **`irreducible_X_pow_sub_C_p`** : `X^ℓ - p` は `ℚ_p` 上既約
  (mathlib `X_pow_sub_C_irreducible_iff_of_prime`)

### 次の段(手順は確定)

`ℓ > p` なる素数を取れば `ℓ ≠ p` かつ `ℓ ∤ p-1`。すると
`L := ℚ_p(p^{1/ℓ})` は次数 `ℓ` で **normal でない**:
normal なら `X^ℓ - p` が `L` で分解し、2根の比として原始 `ℓ` 乗根 `ζ` が
`L` に入る。`[ℚ_p(ζ):ℚ_p] ∣ ℓ`(素数)かつ `ζ` は `∑_{i<ℓ} X^i`
(次数 `ℓ-1`)の根なので `< ℓ` ——ゆえに 1、すなわち `ζ ∈ ℚ_p`。
上の第一の柱で `ζ = 1`、矛盾。

normal でない有限拡大 ⟹ 固定部分群が**正規でない開部分群** ⟹ `Γ_{ℚ_p}` 非可換。

これは `Skeleton/PGC/Section2.lean::prop_2_2`(`IntKbar`・`CompKbar` が自由な
型族)の反証に必要な材料——`Check/PGC/RefutationAttempts.lean` が
