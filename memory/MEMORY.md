...
- [pGC 局所類体論: ★★★★★★★★★★★★★★★★★★★★最大の節目——実在のp進局所体K・実在の捩れ点x∈Λ_nに対するLubin-Tate加群構造(単位律・零元律・乗法性・加法性)がsorry無しで完全に構築された](pgc-lubin-tate-existence-progress.md) — 2026-09-04に到達。経路: D_nの分離性→|Λ_n|=q^n→spectralNorm<1→整性→有限次拡大性・完備性→HasEval、と積み上げ、AdjoinIntegers.leanでPowerSeries.aevalの全条件をValued非経由で直接構成(adjoinIntegers・isLinearTopology_adjoinIntegers・adjoinIntegersAlgebra)しlubinTateEvalAtTorsionPoint(commit 88879620)・lubinTateActionAtTorsionPoint(a·x:=[a]_f(x)、commit eab1c178)を完成。単位律1·x=x・零元律0·x=0(commit bc5cbe76・8bf3e26b)を確認後、★理論的突破としてsubst(形式的代入)とaeval(位相的評価)を繋ぐ連鎖律aeval_subst_eq_aeval_aevalをトランケーション経由で自前構成(mathlibに無かったギャップを埋めた、commit 7f09476c)。これで乗法性a·(b·x)=(ab)·x(commit cb53f8ab)、続けて2変数版に拡張し加法性(a+b)·x=F_f(a·x,b·x)(commit 106726d7)を確立——𝒪_K加群公理が全て揃った。次の一歩: この作用が実際にΛ_∞(=∪Λ_n)に戻ることを示し、K(Λ_n)の正規性・Galois群計算Gal(L_n/K)≅(𝒪_K/π^n)^×・相互律写像の構成へ進むこと。技術的詳細(instance diamond調査・Valued回避・連鎖律構成の経緯)はpgc-lubin-tate-existence-progress.mdに全履歴あり。
