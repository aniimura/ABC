...
- [pGC 局所類体論: ★★★★★★★★★★★★★節目——冪級数をΛ_nの実際の元で評価するlubinTateEvalAtTorsionPointが完成(実在の任意のp進局所体で、sorry無し)](pgc-lubin-tate-existence-progress.md) — 2026-09-04に到達。経路: D_nの分離性→|Λ_n|=q^n(Finset化)→spectralNorm<1→整性→K.carrier⟮x⟯の有限次拡大性・完備性→HasEval、と積み上げた末、AdjoinIntegers.lean(新規)でPowerSeries.aevalの全条件をValued/IsDiscreteValuationRingを経由せず直接構成: adjoinIntegers(ノルム≤1のSubring、Valued不使用)・valuationRing_adjoinIntegers(付値環、ノルム比較の商を直接与えるだけ)・isLinearTopology_adjoinIntegers(半径εの閉球がそのままイデアルになることから、Ideal.isLinearTopologyもValuedも経由しない直接証明)・adjoinIntegersAlgebra(𝒪[K.carrier]→adjoinIntegers K xの埋め込み、spectralNorm_extendsから)を組み合わせ、lubinTateEvalAtTorsionPoint : PowerSeries (𝒪[K.carrier]) →ₐ[𝒪[K.carrier]] adjoinIntegers K x(commit 88879620)を完成。続けてlubinTateActionAtTorsionPoint(commit eab1c178、a·x:=[a]_f(x)をlubinTateEvalAtTorsionPointで実際に計算)を追加。続けてaeval_X_eq_self・lubinTateActionAtTorsionPoint_one(commit bc5cbe76、1·x=x)・LubinTateAction_zero_eq_zero・lubinTateActionAtTorsionPoint_zero(commit 8bf3e26b、0·x=0)を追加。さらに★norm_lubinTateActionAtTorsionPoint_le/_lt_one(commit b0cfe199、‖a·x‖≤‖x‖<1)をsubst/aeval橋渡し無しで確立——[a]_fの定数項0からX_dvd_iffでX*hと分解しaevalの環準同型性(map_mul)だけで得た。これでa·x自身も位相的冪零と分かり作用の反復適用への土台が整った。★★★★★★★★★★★★★★★★理論的な突破(commit 7f09476c): 上記のギャップを実際に埋めた——subst自身の連続性でなくトランケーション経由の別経路(coeff_pow_eq_zero_of_lt→coeff_subst_trunc_eq→tendsto_subst_trunc→aeval_subst_eq_aeval_aeval)で、aeval x(subst g p)=aeval(aeval x g)pという本物の連鎖律を自前で構成。★★★★★★★★★★★★★★★★節目達成(commit cb53f8ab): lubinTateAction_mul——a·(b·x)=(ab)·x(乗法性)を確立。鍵はhasEval_iff_coe(adjoinIntegers K xの位相=誘導位相、tendsto_subtype_rngから直ちに)・lubinTateEvalAtPoint(任意の点での評価、xの座標系のみに依存しCompleteSpace等はそのまま流用可)。連鎖律+LubinTateAction_comp+これらの組み合わせで完成。次の一歩: 加法性(a+b)·x=F_f(a·x,b·x)をLubinTateAction_addと同じ連鎖律で確立(formalGroupLaw経由の分だけ複雑)。その先にΛ_∞に戻ること・K(Λ_n)の正規性・Galois群計算・相互律写像の構成が続く。その先にK(Λ_n)の正規性・Galois群計算Gal(L_n/K)≅(𝒪_K/π^n)^×・相互律写像の構成が続く。技術的詳細(instance diamond調査・Valued回避の発見の経緯)はpgc-lubin-tate-existence-progress.mdに全履歴あり。
