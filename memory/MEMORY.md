...
- [pGC 局所類体論: 節目(5)(古典的Lubin-Tate理論の射影極限による実現)が構成的にすべて完成](pgc-lubin-tate-existence-progress.md) — 2026-09-04〜05。(i)principalUnitsQuotientEquivの2つの自然性、(ii)reciprocityMapLimit:Gal(K.closure/K.carrier)→*CompatibleUnitsの群準同型としての構成、(iii)その全射性(コンパクト性論法)と核の特徴づけ(ker=「無限compatible生成元列をすべて固定する自己同型」、単射性はGal(K̄/K)が非可換なので成り立たないと判明・訂正済み)——すべてsorry無し。固定レベルnでの完全な相互律galoisReciprocityEquiv(Gal(K.carrier⟮x⟯/K.carrier)≃*(𝒪_K/π^n)^×)も既存。次: Proposition 1.2への接続はpgc-prop12-reciprocity-gap.md参照。全履歴はpgc-lubin-tate-existence-progress.mdにあり。
- [pGC Proposition 1.2/2.1: どちらも節目(5)だけでは閉じない——原文精読でProp 2.1も相互律Γ_K^ab≅(K^×)^∧に依存すると判明、不分岐拡大理論がmathlib・本プロジェクトともに未着手と確認](pgc-prop12-reciprocity-gap.md) — 2026-09-05。Γ_K^ab≅(K^×)^∧には節目(5)(完全分岐部分)に加え不分岐拡大・付値分解・両者の組み合わせ・群構造からのq/[K:ℚ_p]読み取りが要る。Proposition 2.1(p進対数+Verlagerung経路)も"regard U_K as a subgroup of Γ_K^ab"の一文が同じ相互律を前提にしており独立ではない——境界外入力(padicLog系・MonoidHom.transfer)は解消済みでもこれだけでは閉じない。次に戻るなら不分岐拡大理論の新規スケルトンが具体的な出発点。
- [pGC 不分岐拡大理論: 各次数の不分岐拡大の**存在**まで完成(e·f=[L:K]・剰余体判定・Hensel・構成・存在)](pgc-unramified-extension-progress.md) — 2026-09-05。Found/PGC/UnramifiedExtension.lean(622行→sorry無し): isIntegral_iff_norm_le_one(spectral norm で数行)→module_finite_adjoinIntegers→ramificationIndex_mul_inertiaDegree(e·f=[K(x):K])→isUnramifiedAdjoin_of_residueDegree→exists_isUnramifiedAdjoin(各次数n)。henselianLocalRing_carrierIntegers も入った(IsAdicComplete.henselianRing 経由。「mathlibに無い」は誤測定だった)。新規 Found/FiniteFieldIrreducible.lean。次: 一意性(拡大体側の IsAdicComplete が要る)と K^ur・Gal(K^ur/K)≅Ẑ。
