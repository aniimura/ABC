...
- [pGC 局所類体論: ★★★★★★★★★★★ψ_n:=D_n/D_{n-1}は既約(任意のn≥1)——K(α)/K(Λ_{n-1})は次数q^n-q^{n-1}の完全分岐拡大(Eisenstein判定法、Lubin-Tate核心定理の完全一般化)](pgc-lubin-tate-existence-progress.md) — Found/PGC/LubinTateActionPsi.lean::irreducible_iteratedLubinTatePsi が2026-09-04に完成(n=1限定のirreducible_iteratedLubinTatePrimitive_oneを完全一般化)。残るはΛ_n本体の𝒪_K加群構造・Galois群計算Gal(L_n/K)≅(𝒪_K/π^n)^×・相互律写像そのものの構成という大きな仕事。
