...
- [pGC 局所類体論: Lubin-Tate形式群法則3性質・𝒪_K作用への拡張の節目(1)(2)(2b)完成+節目(3)Weierstrass標準分解+[π^n]_f∣[π^m]_f(n≤m)まで到達](pgc-lubin-tate-existence-progress.md) — Found/PGC/LubinTateActionPiPow.lean::iteratedLubinTate_dvd_of_le が2026-09-04に完成([IsAdicComplete]仮定を追加しmathlibのWeierstrass標準分解をそのまま適用、D_n次数=q^n・φ_n:=D_n/X次数=q^n-1も既出)。残るはΛ_n(φ_nの根)の構成本体・相互律写像そのものの構成という大きな仕事。
