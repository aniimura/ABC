...
- [pGC 局所類体論: Lubin-Tate形式群法則の存在・単位元則2つ・可換律がsorry無しで完成](pgc-lubin-tate-existence-progress.md) — Found/PGC/LubinTateCommutativity.lean::formalGroupLaw_commutative(F_f(X,Y)=F_f(Y,X))が2026-09-04に完成。残るは結合律・torsion点・相互律写像そのものの構成。
- [ABC3b/cとは.git自体を共有——他セッションのcommitで自分のコミット内容が合流・作業ツリーが差し戻ることがある](shared-git-repo-race-hazard.md) — commit直後は`git log -1 --stat`で自分のメッセージか確認、触る前は`git status`/`git diff`でHEADとの一致を確認してから編集する。2026-09-04実測。
