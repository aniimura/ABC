...
- [pGC 不分岐拡大理論: **存在・一意性・Galois 群 ℤ/n** まで到達(残りは K^ur と Gal(K^ur/K)≅Ẑ)](pgc-unramified-extension-progress.md) — 2026-09-05、1 日で 13 コミット。Found/PGC/UnramifiedExtension.lean(1585 行・sorry 無し)の中心は`isUnramifiedAdjoin_of_lift`(判定子: f モニック・f̄ 既約・x は f_K の根 ⟹ 不分岐 ∧ normal ∧ Gal(K(x)/K)≃Gal(𝓀_{K(x)}/𝓀))と`exists_integral_generator`(逆向き: 不分岐 ⟹ 定義多項式が取れる)。e·f=[L:K]・Henselian(両側)・spectral norm での整性判定が土台。新規 Found/HenselianSplits.lean・Found/FiniteFieldIrreducible.lean はmathlib へ出せる一般結果。
