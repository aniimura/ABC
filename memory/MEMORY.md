...
- [pGC 局所類体論: ★★★★★★★★★★★★★節目(3)の核心完成——D_nは分離的、Λ_nは真にq^n個の相異なる元(実在のp進局所体で)、Finset梱包済み](pgc-lubin-tate-existence-progress.md) — Found/PGC/LubinTateDistinguishedSeparable.lean::card_roots_iteratedLubinTateDistinguished_map が2026-09-04に完成(D_n=D_{n-1}・ψ_n・D_0=Xの漸化式を分離性+将来段との互いの素性を同時に保つ帰納法で押し進めた)。同日中に同ファイルへ iteratedLubinTateTorsionPoints/card_iteratedLubinTateTorsionPoints(|Λ_n|=q^n)/zero_mem_iteratedLubinTateTorsionPoints/iteratedLubinTateTorsionPoints_subset_of_le(Λ_n⊆Λ_m, n≤m)というFinsetレベルの梱包を追加(commit 1630ebdc)——短い名前torsionPointsは既存のGaloisRep側torsionPointsとcheck.mjsのG1検査上で衝突したため改名。残るはΛ_nへの𝒪_K加群構造・K(Λ_n)の正規性・Galois群計算Gal(L_n/K)≅(𝒪_K/π^n)^×・相互律写像そのものの構成という仕事。
