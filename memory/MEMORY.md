...
- [pGC 局所類体論: ★★★★★★★★★★分離性完成——混標数でψ_nの根はq^n-q^{n-1}個の相異なる元(Λ_nギャップ解消)](pgc-lubin-tate-existence-progress.md) — Found/PGC/LubinTateActionPsiField.lean::nodup_iteratedLubinTatePsiRoots が2026-09-04に完成。[CharZero A]を新規追加(CLAUDE.md逸脱記録、混標数=ℚ_pの有限次拡大の場合に対応)。残るはΛ_n本体の𝒪_K加群構造・Galois群計算Gal(L_n/K)≅(𝒪_K/π^n)^×・相互律写像そのものの構成という大きな仕事。
