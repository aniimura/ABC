...
- [pGC Proposition 1.2: 節目(5)だけでは閉じない——不分岐拡大理論(Gal(K^ur/K)≅Ẑ)がmathlib・本プロジェクトともに未着手と確認](pgc-prop12-reciprocity-gap.md) — 2026-09-05。Γ_K^ab≅(K^×)^∧には節目(5)(完全分岐部分)に加え不分岐拡大・付値分解・両者の組み合わせ・群構造からのq/[K:ℚ_p]読み取りが要る。一方Proposition 2.1(p進対数+Verlagerung経路)は境界外入力がほぼ解消済み(padicLog系・MonoidHom.transfer)で、節目(5)より近い可能性がある——次にLubin-Tate方面から離れるならここが具体的な出発点。
