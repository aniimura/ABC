...
- [pGC 局所類体論: Lubin-Tate形式群法則3性質・𝒪_K作用への拡張の節目(1)(2)(2b)完成+節目(3)代数閉体での根の個数(重複度込みq^n・q^n-1)まで到達](pgc-lubin-tate-existence-progress.md) — Found/PGC/LubinTateActionWeierstrass.lean::card_iteratedLubinTateDistinguishedRoots が2026-09-04に完成(AlgebraicClosure(FractionRing A)上でD_n・φ_nの根の多重集合を定義、IsAlgClosed.card_roots_eq_natDegreeで個数を計算)。残るは分離性(根が相異なること)・𝒪_K加群構造・相互律写像そのものの構成という大きな仕事。
