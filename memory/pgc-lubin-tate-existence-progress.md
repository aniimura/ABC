...
## ★★★重要な軌道修正(2026-09-05、(iii)「単射性」は**そのままの形では
数学的に偽**——ゴールの言い換えが必要)

上の「単射性」というゴール設定を、着手前に検算した結果、**このまま
では成り立たない**ことが分かった。記録として残す(CLAUDE.mdの「逸脱」
条項——原典の意図を優先した読み替えの必要性が生じたケース)。

**理由**: `reciprocityMapLimit`の定義域`σ:K.closure≃ₐ[K.carrier]
K.closure`は`K.absGal`(`Skeleton/PGC/Setup.lean::absGal`)そのもの
——すなわち`K̄/K`の**完全な**絶対Galois群`Gal(K̄/K)`(`K.closure:=
AlgebraicClosure K.carrier`、`Skeleton/PGC/Setup.lean::closure`)。
一方、値域`CompatibleUnits K hπmax`は`Π n,(𝒪_K/π^n)^×`の部分群
——`𝒪_K`が可換環である以上**アーベル群**。p進局所体`K`の
`Gal(K̄/K)`は(分岐拡大も込みで)**非可換**であることが知られている
(例: 次数3以上の完全分岐非可換拡大が存在する)。**非可換群からアーベル
群への準同型は必ず交換子部分群`[Gal(K̄/K),Gal(K̄/K)]`を核に含む**
——ゆえに`reciprocityMapLimit`はこの核が非自明である限り**単射に
なり得ない**。

**何が真の主張か**: 古典的Lubin-Tate理論が実際に主張するのは
`Gal(K_π/K)≅𝒪_K^×`(`K_π:=K(Λ_∞)`、Lubin-Tateの塔`K(Λ_n)`全体の
合併——**完全な代数閉包`K̄`ではない**、`K`の**ある1つの完全分岐
アーベル拡大**)であり、`reciprocityMapLimit:Gal(K̄/K)→Compatible
Units`はこの同型を`Gal(K̄/K)→Gal(K_π/K)`(制限写像、`K_π/K`が
Galois拡大なので矛盾なく定義できる)経由で見ているにすぎない。
ゆえに`reciprocityMapLimit`の**核は`Gal(K̄/K_π)`**(`K_π`の元を
すべて固定する`σ`全体)に一致するはずで、これは(`K_π⊊K̄`である
限り)**非自明**。「単射性」ではなく「核が`Gal(K̄/K_π)`に一致する
(=`σ`が`K_π`上恒等的であることと`reciprocityMapLimit σ=1`が同値)」
が、形式化すべき**正しい**主張になる。

**今後どう進めるか(次に戻るときの具体的な選択肢)**:
1. **全射性**を先に片付ける(こちらは真であり、`/goal`の他の項目
   への波及も大きい見込み): 各レベル`n`で`Gal(K̄/K)→Gal(K(Λ_n)/K)`
   (制限写像)が全射であること(有限次Galois拡大`K(Λ_n)/K`が`K̄`の
   中にある以上、一般のGalois理論の「制限写像の全射性」定理——
   Mathlibに`IsGalois`関連で存在する見込み、要探索——から従う)と
   `Gal(K(Λ_n)/K)≅(𝒪_K/π^n)^×`(既に確立済みの`existsUnique_
   unitActionQuotient_eq_algEquiv`系の全単射性)を組み合わせれば
   届く。`CompatibleUnits`(射影極限)への全射性は、各レベルへの
   全射性から(`Gal(K̄/K)`がコンパクト/副有限であることを使った
   コンパクト性論法、あるいは射影極限の普遍性からの直接論法)従う
   見込みだが、この最後の一歩はまだ検証していない。
2. **核の特徴づけ**(「単射性」の正しい言い換え)は、後回しでも
   節目(5)の**主目的**(古典的Lubin-Tate理論の実現)は既に(i)(ii)+
   上記(iii-1、全射性)で本質的に達成される——`K_π`という**部分体
   としての対象自体を明示的に構成していない**(`K(Λ_n)`の合併と
   しての`K_π`、あるいは`Gal(K_π/K)`を`Gal(K̄/K)`の商として捉える
   道具立て)ため、この部分は**新たな骨格(スケルトン)の追加**が
   要る——「工数の大きな塊」ではなく、CLAUDE.mdの進め方(スケルトン
   →依存グラフ更新→葉から形式化)に従って追加すべき次の層。
3. 節目(5)自体が`/goal`の11項目(pGC §1–4)を**直接**満たすための
   ものではなく、その**基盤**(古典的Lubin-Tate理論の局所類体論
   同型)であることを踏まえると、`/goal`の残り8項目(Prop 1.1・1.2・
   Cor 1.3・Prop 2.1・2.2・Cor 3.1・3.3・Thm 4.2)がこの射影極限
   同型の**全射性だけ**を要するのか、**核の特徴づけ**(あるいは
   完全な同型)まで要るのかは、それらの項目自身の証明を見据えないと
   確定しない——次にこの方向で戻る際は、まず`/goal`側の実際の
   必要性を確認するのが優先度として高い。

**教訓**: 「射影極限を構成できたら次は全射性・単射性」という
一般的なパターンを機械的に当てはめる前に、**定義域と値域の代数的
