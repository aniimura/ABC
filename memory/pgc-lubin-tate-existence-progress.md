...
**残る作業**(数学的な障害は無く、純粋な組み立て): この列の極限 `F`
(各係数が `k` が十分大きいとき安定するという事実——`φ_k` の次数が
`k+2` 以上であることから従う——を使って係数関数として直接定義する)を
構成し、`F` が関数等式を exact に満たすことを示す。その後さらに、実際の
局所類体論の相互律写像を形式群の上に構成する段が残る(pGC の11項目の
どれかを閉じるにはそこまで要る)。
