...
## ★★★現在の律速 —— `Proposition 4.4, (iii)` の全射性

★(i)(ii)(iv) は閉じた。残るのは `phiBiratGen A ≤ phiBiratAt A` 1 点。

★★**還元し尽くした形**: 剰余類の補題と
「isotropic なら同じ底で `𝒞^birat` 同型」により、
**`B` ごとに同型 `h_B : A ≅ B` を 1 本取って
`Div^gp(h_B) ∈ phiBiratAt A` を示せば十分**。

★★★**封じられた道(二度掘らないため)**:
1. `Div^gp(h) + Div^gp(h⁻¹) = 0` は**自動**。同型性だけからは何も出ない。
2. `𝔽_{Φ^gp}` は**底 1 つに対象 1 つ**なので `T(h) ∈ 𝒪^×(X)_𝔽` にはなるが、
   `𝒪^×(A^birat)` の**像**に入る保証はない。`Proposition 1.5, (ii)` だけでは出ない。
3. ★`Div : 𝒪^▷ → Φ` の全射性を仮定するのは**不可** ——
   `Φ^birat = Φ^gp` に退化する。`phiBiratAt_eq_top_of_divSurj` として**証明して残した**。

★原典(p.85)はここを「follows **immediately**」としか書いていない。
★★次の一手は **model 型に限定して全射を述べる**こと
(下流の `Theorem 5.2`・`Example 6.1`・`6.3` はすべて model 型で、これなら退化しない)。

★設計の教訓: `Φ^birat` は**「生成される部分関手」として定義**する。
像として定義すると「経由する」側に内容が集まり、退化する仮定を呼び込む。
★原典の論理構造(存在と一意性が先、全射があと)に合わせること。
