...
3. **PDF目視**: 対象範囲の各ページを画像化して見る。`notationRisk: high` の論文では **locator が指す全ページで必須**(装飾が疑わしい箇所は `pdftoppm -r 400` に `-x -y -W -H` を添えて拡大する——IUTchIII の二重下線は 150 dpi では単線と区別できなかった)。`medium`/`low` でも、数式が現れるページは見る。見たページは `papers.json` の `verifiedPages` に記録する。**記録に無いページの記法については何も言えない**、を規約の一部とする(初版は「high の論文では全ページ必須」と書いていたが、199 ページの IUTchIII で達成不能と判明したため改訂1で読み替えた)。
...
| `scr` | script体(𝒪, 𝒟, ℐ 等) | `font-style: italic` |
| `frak` | フラクトゥール(𝔇, ℜ, 𝔉, 𝔩𝔬𝔤, 𝔪𝔬𝔡 等) | `font-style: italic` |

利点: `grep 'class="ul2"'` でその対象への言及箇所を機械的に洗い出せる。`.txt` では不可能。

### 平文表現 `X[cls]` — Lean のコメント等で使う

HTML を書けない場所(Lean のコメント・メモ・チャット)では、**同じクラス名を角括弧で書く**。1つの体系・2つの表現とし、対応表を持たない。

```
Gal(Y[ul2]/X[ul2]) (≅ l·Z[bb])       -- [EtTh] p.43 Def 2.13(i)、下線2本
Gal(Y/X) (≅ Z[bb])                   -- [EtTh] p.11、下線なし
Γ_K ≝ Gal(K[bar]/K),  K[prime]       -- [pGC] p.2
0 → U_K → (K^×)^∧ → Z[bb][hat] → 0   -- [pGC] p.3
```

`grep '\[ul2\]'` でその対象への言及を横断的に洗える(HTML 側の `grep 'class="ul2"'` と対になる)。

**照合射影**: 実測により、装飾は `pdftotext` で一律に脱落し**基底文字だけが残る**——`K̄`→`K` / `Ẑ`→`Z` / `𝒪`→`O` / **Z**→`Z` / `Y̲̲`→`Y` / `K′`→`K`。ゆえに射影は「角括弧を落とす」だけでよい。あわせて `_`(下付き)・`^`(上付き)も落とす(原文 `Γ_K` は `pdftotext` で `ΓK`)。

Lean のコメント内では、この形式で引用すると `check.mjs` が **PDF に対して照合する**:
...
`原文 (<タグ> p.<物理ページ>):` の次行以降、`>` で始まる行が引用本体。**引用を2箇所(HTML と Lean)に持つ以上、ズレたら落ちるようにしてある**——引用のズレは本プロジェクトが最も防ごうとしている失敗だから。

> **落とし穴(2026-08-14、IUTchIII で実際に踏んだ)**: 引用の最終行に docstring の終端 `-/` を同じ行で書くと、`-/` が引用本体に取り込まれて照合が落ちる(「108/110 文字まで一致・次に来るはず `-/`」)。**`-/` は必ず次の行に置く**。ゲートは正しく検出したので穴ではないが、書くたびに踏むので明記しておく。

> **★平文記法の限界(2026-08-14 実測)**: HTML 側の `data-txt=""`(「`.txt` には何も出力されない」)に対応するものが、Lean のコメント側に**無い**。`X[cls]` は角括弧を落とすだけで基底文字は残るので、**`pdftotext` が何も出力しない記号**——IUTchIII の `⋃`(大合併)・`↠`(全射)・`△`・下線つき `ε`——を含む箇所は Lean から引用できない。現状の回避は「その記号を含まない範囲を選んで引用する」。「1つの体系・2つの表現」が完全には成り立っていない箇所であり、必要になったら記法を足す(例: 消える記号を `⟦X⟧` と書いて射影で丸ごと落とす)。
...
- **2026-08-14 改訂1(IUTchIII §3 への適用で判明した2点)**。
  1. **記法クラスに `frak`(フラクトゥール)を追加**(§3 の表)。IUTchIII は**書体だけで別対象を区別する**——原文 物理 p.156 は「添字 MOD の方は両立するが 𝔪𝔬𝔡 の方は両立しない」と明示的に対比しており、ローマン体 `MOD` とフラクトゥール `𝔪𝔬𝔡`、`LGP` と `𝔩𝔤𝔭`、log-volume の `log` と 𝔩𝔬𝔤-link の `𝔩𝔬𝔤` が別物である。書体は下線と同じ資格で対象の同定に効くので、クラス語彙に無いのは欠落だった。「1つの体系・2つの表現」を保つため `tools/check.mjs` の `DECOR_CLASSES` にも同時に追加し、fixture D25 を足した。
  2. **§2-3 の「`notationRisk: high` の論文では全ページ必須」は達成不能なので読み替える**。IUTchIII は 199 ページあり、着手のたびに全ページを目視することはできない。実際に必要かつ十分なのは **locator が指すページの目視**であり、`papers.json` の EtTh の記述(「全 locator で PDF 目視必須」)の方が正しい。§2-3 の文言はこちらに合わせるべき——ただし「まだ見ていないページの記法については何も言えない」ことを `papers.json` の `verifiedPages` が明示し続けること。
