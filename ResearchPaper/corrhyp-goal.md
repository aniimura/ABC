...
## 2026-09-05(続き11): 「項目(1)」(実データへの`specialize`)の正確な
道筋を精査——ℝレベルでは自明・Rレベルで初めて非自明になる理由

実データへの接続に着手する前に、「何を`ψ`・`ψ'`として渡すべきか」を
精査した。重要な気づきを正直に記録する:

**ℝレベルでの貼り合わせは実は自明である**——`α⁻¹(U_i)`(`X.left`の
アフィン開`U_i`ごとの`c.C`側の片)は、すべて**同一のスキーム`c.C`の
開集合**なので、その重なり`α⁻¹(U_i)∩α⁻¹(U_j) = α⁻¹(U_i∩U_j)`も
`c.C`自身の開集合であり、「貼り合わせデータ」は`c.C`の層構造が
**既に自動的に**与えている(非自明な遷移同型を要する`GlueData`
形式は、抽象的な断片を貼り合わせるときにのみ要る)。これが、ℝレベル
では`gdT`/`transitionElem`のような重い機構が(単一のアフィン開`U`
**内部**の標準エタール片の貼り合わせを除けば)実は不要だった理由。

**Rレベルで初めて非自明になる**——`descendPieceR`(`U_i`ごとの`Spec(S_0)`)
は`c.C`の開集合そのものではなく、`c.C`の対応する開集合と**ℝへbase
changeして初めて同型になる**抽象的なRレベルスキームなので、
`α⁻¹(U_i)`・`α⁻¹(U_j)`が`c.C`の中で重なっているという事実だけでは、
`descendPieceR`のU_i版・U_j版がRレベルで**直接**同型であることは
従わない——`exists_mvPolynomial_quotient_specIso_descend`が与える
非自明な遷移同型が本当に必要になるのはここ。

**実際の`ψ`の構成に要る具体的な部品(未着手、次の一手として記録)**:
`P_i := Algebra.Presentation.ofFinitePresentation (Γ(X.left,U_i)⊗ℝ)
Γ(C,piece_i)`の`P_i.val : Fin n_i → Γ(C,piece_i)`(mathlib、`Algebra.
Generators.val`、生成元の実際の環の元)を、`Γ(C,piece_i) → Γ(C,
overlap)`という層の制限写像(標準的なscheme制限、`X.presheaf.map`
経由)で`overlap := α⁻¹(U_i∩U_j)`へ送り、それを`P_j`の生成元の
多項式として表す(`P_j.val`が`Γ(C,overlap)`を生成することを使う、
`aeval`の全射性経由の**選択**が要る)ことで`ψ_i : Fin n_i →
MvPolynomial (Fin n_j) (Γ(C,piece_j)⊗ℝ)`を構成する——これが
`exists_mvPolynomial_quotient_specIso_descend`の`ψ`引数になる。
`hround1`・`hround2`(往復が恒等)は、両方向の制限写像の合成が
`Γ(C,overlap)`上で恒等であるという、層の制限公理そのものから従う
見込み。

この精査により、「実データへの接続」が具体的にどの3つの新しい部品
(生成元の制限・多項式としての表示の選択・恒等性の層公理からの導出)
を要するかが明確になった——数学的な内容としては素直だが、
`Scheme.Opens`/`TopCat.Presheaf`の制限写像のAPIとの配線作業がまだ
残っている。集計は10/24で変わらず——§4は引き続き0/2。

## 2026-09-05(続き12): `piece_restrict_hom`——制限写像の第一部品を実装、
かつ前回の道筋の**訂正**(共通のさらなる細分`W`が要ることが判明)

前回の精査(続き11)で特定した第一部品——`X.left`の異なるアフィン開
`U`・`V`(`V≤U`)にまたがる`c.C`側の片同士の制限写像——を実装した
(commit `e5250e76`):

- `piece_le_of_le`: `V≤U`のとき`c.C`側の対応する片も同じ向きの包含
  になること(2段の逆像がどちらも単調)。
- `piece_restrict_hom`: 標準的な層の制限(`C.presheaf.map (homOfLE
  ...).op`)による、`c.C`側の片同士の制限写像。

`lake build ABC3.Found.CorrHyp.ExtLimit`・`ABC3`いずれも0エラーで
確認、push済み。

**前回の道筋の訂正(正直な記録)**: `piece_restrict_hom`を実際に組み
立てようとして、前回書いた「`ψ`は`P_j`の生成元の多項式として直接
表示する」という道筋が**不完全**だったことに気づいた——`U_i∩U_j`
(overlap)は一般に`U_j`自体とは異なる(真に小さい)ので、`P_j.val`
(`U_j`全体上の生成元)がoverlap上でも生成系であるとは限らない。
正しくは: `U_i`・`U_j`双方に含まれる**共通のさらなる細分**`W`
(`W≤U_i`かつ`W≤U_j`なるアフィン開、たとえば`W:=U_i⊓U_j`——`X.left`
が分離的なら共通部分もアフィンになる見込み、要確認)を取り、
`piece_restrict_hom`で`P_i.val`・`P_j.val`双方を`W`へ制限してから、
`W`**自身の**`Presentation`(`Algebra.Presentation.ofFinitePresentation`
を`Γ(C,piece_W)`へ適用したもの)の生成元を仲立ちにして`ψ`を構成する
必要がある——「2つの片を直接比較する」のではなく「両方を共通の
第三の片`W`へ制限してから比較する」という、より標準的な(層の)貼り
合わせの形になる。

この訂正は、`exists_mvPolynomial_quotient_specIso_descend`自体の
正しさには一切影響しない(その定理はあくまで「2つの`Presentation`の
生成元同士に、往復合成が恒等になる対応があれば、Rレベルで同型が
得られる」という完全に一般な事実であり、その対応`ψ`をどう構成する
かという「配線」側の設計ミスだった)。次の一手(未着手): `W:=U_i⊓U_j`
がアフィンであることの確認(または適当なアフィン部分開への細分)+
`piece_restrict_hom`を`U_i→W`・`U_j→W`双方向に適用する配線。集計は
10/24で変わらず——§4は引き続き0/2。この種の設計ミスを早期に(実装
段階で)発見・訂正できたこと自体は、拙速に大きな構造を組み立てず
小さな部品から検証していく方針の効用と言える。

## 2026-09-05(続き13): ★重要な簡略化★ 「共通の細分`W`」問題は
`X.basicOpen`(基本開)の被覆を使えば分離性の仮定なしに解決する

前回(続き12)の課題「`W:=U_i⊓U_j`がアフィンであることの確認」に
着手したところ、`AlgebraicGeometry.IsAffineOpen.inf`(mathlib)が
`[IsAffineHom (pullback.diagonal (terminal.from X))]`(実質的に
`X`の分離性)という**追加の仮定**を要求することが判明した——
`QcqsSpace`(`corrHypInstance4`)の定義は`CompactSpace`・
`QuasiSeparatedSpace`のみを課しており、完全な分離性は含まれて
いない。ここで安易に分離性を仮定に足すと、既存の完成済み部分への
影響を評価しないまま`HyperbolicCurveData`インターフェースを変更する
ことになりかねない——`Nonempty C`の教訓と同種のリスクである。

**発見(朗報)**: この問題は、`U_i`・`U_j`を**任意の**アフィン開のペア
として扱うのをやめ、代わりに`X.left`を**単一の**アフィン開`U`の
**基本開被覆**(`X.basicOpen f_i`、`i`は有限)で覆う、という設計に
切り替えれば、分離性を一切使わずに解決する——`Scheme.basicOpen_mul`
(mathlib、既にこのファイルで多用): `X.basicOpen f ⊓ X.basicOpen g =
X.basicOpen (f*g)`。基本開同士の交わりは**再び基本開**(同じ環
`Γ(X,U)`の別の元による局所化)なので、**常にアフィン**——分離性の
仮定が一切要らない。これで「共通の細分`W`」は`W := X.basicOpen
(f_i*f_j)`として具体的に取れる。

**設計の見直し**: これは項目(d)全体の設計を「任意の異なるアフィン開
`U_i`同士を直接貼り合わせる」から、「**単一の**アフィン開`U`の基本開
被覆`{D(f_i)}`を貼り合わせる」へ切り替えることを意味する——これは
実は`corrHypGlueData`(ℝレベル、既存)が最初から採用していた設計
そのものである(`corrHypGlueDataOfEtale`が`exists_finite_
standardEtaleCover`で得る被覆も、単一の`Γ(X,U)⊗ℝ`上の標準エタール
元による基本開被覆だった)——今回の`piece_restrict_hom`の一般化
(異なるアフィン開`U`・`V`、`V≤U`に対して定義)自体は無駄にはならない
(`D(f_i) ≤ U`という包含にそのまま使える、`V := D(f_i)`とすれば
よいだけ)が、「`U_i`・`U_j`が互いに任意」という前提を外し、「共通の
`U`の基本開`f_i`・`f_j`」という前提に変えることで、`W := D(f_i f_j)`
が自動的にアフィンになり、分離性の仮定が完全に不要になる。

**次の一手(未着手)**: `descendPieceR`(現状`U`一般に対して定義済み)
を`U := X.basicOpen f_i`(`X.left`上の基本開)の場合に適用し、
`piece_restrict_hom`(`D(f_i f_j) ≤ D(f_i)`・`D(f_i f_j) ≤ D(f_j)`の
2方向)で生成元を`W := D(f_i f_j)`側へ制限し、`W`自身の
`Presentation`を仲立ちに`ψ`を構成する——これで`exists_mvPolynomial_
quotient_specIso_descend`の前提が実際に満たせる見込み。分離性問題を
...
**「名前付きdef分割」を試して判明した数学的な落とし穴(2026-09-05夜
続き、自律ループ2ティック目、依然ファイル変更なし)**: 上の「次に
試すべき道」を実行したところ、2つのことが分かった。(1) `R'`・`p₀`を
`.choose`で取り出す際、`exists_piece_basicOpen_R_lift_data`の結論を
`True`にすると型は軽く済む(4件とも数秒で通った)が、**取り出される
`p₀`が実際の正しさの等式(`hp₀`)と紐付いていない**——`Exists.choose`
は`Classical.choice`経由で「型の合う何らかの証人」を返すだけで、
`True`しか主張していない存在からは`hp₀`を捨てた時点で証人の正しさが
失われる(証明項の構成に使った具体的な`p₀`が返る保証はない、命題の
証明無関係性のため)。(2) 名前付き定数(`piece_basicOpen_R_lift_R'`等)
を**参照**する形で正しさの等式を書けば型は軽い(`piece_basicOpen_R_
lift_spec`、4.73秒で通った)が、その際`e`(`pieceAlgebra_R_model_
baseChange`が与える環同型)を`∀ e, ...`と**全称量化**する形にすると、
これは**数学的に誤り**——一般に`Nonempty(S₀⊗[B]T ≃+* Γ(C,piece))`を
満たす環同型は一意とは限らず、正しさの等式は「証明の中で実際に構成した
特定の`e`」についてのみ成り立つ(`∀e`は反例がありうる、証明できない
可能性が高い)。

**結論**: 「存在の証明」と「証人の正しさ」を**分離して**名前付き`def`
にするという設計そのものが誤りだった——`e`・`R'`・`p₀`・正しさの等式は
**1つの証明の中で一括して**(`obtain`で`e`を取り出したその場で)構成
する必要がある、という原点(`exists_piece_basicOpen_R_lift`、続き
1回目)に戻る。巨大な型の重複を避けるには、代わりに`structure`
(名前付きフィールドを持つ構造体)で束ねる、または頻出する部分式
(`Fin(Algebra.Presentation.ofFinitePresentationVars(...))`・
`Ideal.span(Set.range(...))`)自体を**独立した`def`**として先に固定
してから式中で参照する、という2つの方向性が次に試す価値がある——
