...
**Found 4/13**(`Lemma 1.1`・`Definition 2.1`・**`Theorem 2.2`**・**`Theorem 2.4`**)。以下、13項目それぞれの
...
| §1 | Theorem 1.2 | **残り 1 段(塔の構成)**——★2026-09-05(後半): 証明の骨格・不等式・長さの算術は**すべて Lean で閉じた**(`thm_1_2`、`.src`/`.needs`・非空虚性 2 件つき) | ★次の一手: `Vₙ` の列と `Wₙ = Vₙ⊗W` の正規化を Lean で立て、各段で `Thm12StepData` を供給する。節点 A(`ker_contains_pTorsion_of_faltings`、★構造定理不要)・節点 B の (B1)(`quot_annihilated_of_conductor_subalgebra`、★`b·Dx = D(b·x) − x·Db` の 1 行)は完了、(B2) は既証の `cancel_conductor_delta` へ帰着済み。以下は経緯の記録。★2026-09-05に原典(物理p.4=印字p.257)を逐語精読した結果、Faltingsの証明は**`differentIdeal`のtower diamond公式ではなく、`Lemma 1.1`を使った「長さ」の評価**だった。骨格: (1)`Vₙ`を取り替えて`Wₙ`を整域としてよい。(2)写像の列`Ω_{Wₙ/Vₙ}⊗W_{n+1} → Ω_{W_{n+1}/Vₙ} → Ω_{W_{n+1}/V_{n+1}}`を見る。(3)第2写像の核は`Ω_{V_{n+1}/Vₙ}⊗W_{n+1}`を含み、これは`(W_{n+1}/pW_{n+1})^{d+1}`を商に持つ。(4)`Ω_{V_{n+1}/Vₙ}`が`V_{n+1}/p^aV_{n+1}`型の`d+1`個の直和なので、合成は`Ω_{Wₙ/Vₙ}⊗W_{n+1}`上の`p`倍の核を零化する。(5)`δₙ :=`差積 とすると、その核の長さは`W_{n+1}/p^{δ'}W_{n+1}`(`δ' = min{1, δₙ/(d+1)}`)以上。(6)**`p^{δₙ−δ_{n+1}}`が`W_{n+1}/(Wₙ⊗_{Vₙ}V_{n+1})`を零化する**(原文は*"it is clear that"*で畳んでいる——ここが我々が「ブロッカー」と特定していた非正規性の評価)。(7)★**2026-09-05に原典を260dpiで描画して逐語確認**した正確な形は`δₙ − δ_{n+1} ≥ β − (d+1)(δₙ − δ_{n+1})`(`β = min{1, δₙ/(d+1)}`)であり、そこから*"So if δₙ ≥ d+1, then δ_{n+1} ≤ δₙ − 1/(d+2), and otherwise δ_{n+1} ≤ (1 − 1/((d+1)(d+2)))δₙ. In any case δₙ → 0"*。★以前の記録の`(1 − 1/(d+2))δₙ`はOCRの潰れによる誤りだった。**この第7段は形式化済み**(`Section1.lean`の`delta_two_regime_of_key`・`thm_1_2_tendsto_zero`、非空虚性は`δₙ = (1/2)^n`が等号で満たす)——`Theorem 1.2`は**その1つの不等式(長さの評価、第2〜6段)**に完全に帰着した。★**したがって必要なのは`differentIdeal Wₙ Wₙ₊₁`の閉じた式ではなく、`Module.length`の算術と`p^δ`の指数(有理数)の評価**である。`Lemma 1.1`(既にFound)がその変換器になる——差積 ↔ `Ω`の余核の長さ。これまで積み上げた`differentIdeal_tower_diamond`・`cancel_conductor_delta`はこの骨格の中では**必須ではない**(独立に成り立つ補題として残す)。以下は旧記録: `Wₙ⊗_{Vₙ}Vₙ₊₁`の非正規性(`differentIdeal Wₙ Wₙ₊₁`)の評価が必要。`Vₙ`塔側の技術(Eisenstein多項式・differentIdeal計算)は完成済みだが`Wₙ`側(任意のalmost étale `W`)には転用できない(`differentIdeal_tower_diamond`のdocstring参照)。退化した`W`(非分岐)を使うと`δₙ≡0`は自明に出るが、原文の「任意の`W`」という全称量化を証明したことにはならない。 |
...
| §3 | Theorem 3.1 | ブロック(★依存先を訂正) | ★2026-09-05に原典(物理p.10=印字p.263)を逐語精読した結果、**`Theorem 2.2`/`2.3`には依存していない**。証明は`dim(R)`についての帰納法で、使うのは(a)`Definition 2.1`、(b)**`Theorem 1.2`**(`dim = 1` の底の場合——*"the sequence Rₙ kills ramification (complete and use part 1, Theorem 1.2)"*)、(c)反射的加群・正則局所環上の自由性、(d)`dim ≥ 3` で局所コホモロジー`H²_n(Sₙ)`の有限性。したがって**このトラックの本当のブロッカーは`Theorem 1.2`**であって`Theorem 2.3`ではない。 |
| §3 | Theorem 3.2 | ブロック(★依存先を訂正) | ★2026-09-05: 同じく原典(物理p.11=印字p.264)を確認。証明が使うのは`Definition 2.1`・trace formの像・「almost 直和因子」の議論で、`2.2`/`2.3`には依存しない。 |
| §4 | Theorem 4.1 | **(ii)は✅証明済み**(`thm_4_1_ii`、非空虚性つき) | ★2026-09-05: `Found/Falt1/Section4.lean`。**§2の壁を継承していなかった**——原文の証明は3行で、入力は**`Theorem 2.4(i)`の核側**だけである: *"We only have to show that the first map of the exact sequence in (ii) is injective. We already know that its kernel is annihilated by m. But Ω_{V̄/V}⊗_V R̄ has no m-torsion."* `thm_2_4_i`(証明済み)から**単射性**が出て、中央の完全性と右の全射性は Jacobi–Zariski(mathlibの`exact_mapBaseChange_map`・`map_surjective`)で形式的に従う。非空虚性は`R=ℤ`・`A=ℤ[X]`(`Ω[A⁄R]≠0`)・`B=Fin 2→A`・`p=5`。**残るのは(i)**。★**2026-09-05に原典を260dpiで描画して逐語再確認し、我々のSkeletonの転記が誤っていたことが判明**——原典の bar は **`Ω` ではなく添字の `R`・`V`** に付いており、`Ω_{R̄/V̄}`(整閉包の微分)であって **`p`進完備化ではない**。正しくは *"(i) The map `Ω_{R/V} → Ω_{R̄/V̄}` induces almost isomorphisms `Ω_{R/V} ⊗_R R̄[1/p] ≅ Ω_{R̄/V̄}` and `Ω_{R/V} ⊗_R (R̄[1/p]/R̄) ≅ Ω_{R̄/(R⊗_V V̄)}`"*。したがって**完備化の枠組みは不要**で、通常の Kähler 微分と局所化 `R̄[1/p]` だけで書ける——必要なのは `R∞`(単元の `p` 冪根を添加した塔)の設定である。 |
...
| **`Theorem 4.1(i)` の抽象的な内容** | **✅証明済み**(第 1565・1566、非空虚性つき)——`localizedModule_map_bijective`(**almost 同型は水準を反転すると本物の同型になる**。局所化は完全なので**平坦性の議論は要らない**)＋ `thm_4_1_i_localized`(それを `Theorem 2.4(i)` に当てたもの)。★`Section4.lean` 冒頭の「(i) は `p` 進完備化の枠組みが要る」という記述は**誤りだった**(bar は `Ω` ではなく添字)——訂正済み。残るのは `R∞` の設定と明示計算 | `AlmostDerivation.lean` + `Section4.lean` |
| **§4(b) の「`dlog(uᵢ)` を基底とする自由加群」** | **✅証明済み**(第 1567、非空虚性つき)——`mapBaseChange_bijective_of_formallyEtale`(**形式的エタールなら微分は底変換で同型**。Jacobi–Zariski の 2 本の完全列で両端が消える)＋ `kaehlerBasisOfEtale`/`_apply`(`Ω[R⁄V]` は `d(uᵢ)` を基底とする自由 `R`-加群) | `Section4.lean` |
| **§3(a) の `Rₙ` を実際に構成** | **✅証明済み**(第 1569-1571、非空虚性つき)——原文が *"It is clear that `Rₙ ≅ (R⊗_V Vₙ)[X₁…X_d]/(Xᵢ^{p^n}−uᵢ)`"* と書く環を **`kummerAlgebra` としてそのまま定義**し、`kummerGen`/`kummerGen_pow`/`kummerGen_adjoin`・**`kummerAlgebra_kaehler_annihilated`**(`Rₙ` の微分は `N` で消える。`uᵢ` が単元であることだけを使う)・`kummerTransition`(塔の遷移 `Rₙ → R_{n+1}`、`xᵢ ↦ yᵢ^M`)。一般形は `kaehler_annihilated_of_roots_of_units`、その入力 `hspan` は `kaehler_span_of_adjoin`(★`kaehler_cyclic_of_adjoin` の一般形)が与える | `Section3.lean` + `Section1.lean` |
| **almost étale は塔で合成できる** | **✅証明済み**(第 1562)——`relDiag`(相対対角イデアル)・`relDiag_mul_eq_zero`(★`C⊗_A C → C⊗_B C` の核の**記述を使わずに**済む)・`witness_comp`(`w = w_C·ι(w_B)`、水準 `c_B·c_C`)・`hasAlmostWitnesses_comp`(塔版)。§3 が常用する基礎 | `Section3.lean` |
| **底変換と almost 性** | **✅証明済み**(第 1563)——`almostSurj_lTensor`(余核側は任意の底変換で保たれる、平坦性不要)・`isAlmostZero_tensor`。★核側は保たれない(テンソルは左完全でない)——`Theorem 4.1(ii)` が捩れ無しを別途要求する理由 | `AlmostDerivation.lean` |
| **補正した積から本物の環** | **✅証明済み**(第 1561、非空虚性つき)——`commRingOfBilin`・`algebraOfBilin`・`commRingOfCorrectedMul`。`associativity_correction`(既証)の帰結を**環**にする道具で、原文の `B_ε` の構成そのもの。mathlib に「双線形写像から環を作る」道具が無いので置いた | `AlmostDeform.lean` |
...
| `Theorem 2.3` 第5段 | **✅全段証明済み**——`ρ : B_ε ↠ B` の構成(`exists_quot_map`)・乗法の持ち上げ(`thm_2_3_mul_lift`)・結合子が核に入る(`associator_mem_ker`)・five-term identity・**H³ の almost 加群版**(`hochschild_H3_almost_coboundary_act`——核 `J` の `B`-加群構造が almost にしかならない問題を、作用を外から与えて「結合律がスカラー `t` を除いて成立」で足りる形に一般化して解決。ずれは恒等式の 4 箇所に現れるが全て `t` で消える)・結合律への補正は**倍化なしで厳密**・交換子は導分。**第5段の最終部**も形式化済み——`scaledSubalgebra`(`C_σ = A + c·B` が部分代数)・`scaledSubalgebra_mono`(`σ < ε/2` のとき `C_ε ⊆ C_σ`)・`mem_iSup_scaledSubalgebra`(増大列の合併)。★これで**原文の5段すべて＋最終部の代数的骨格が揃った**——`etale_lift_existsUnique`/`etale_lift_iso`(*"all `B_ε[1/p]` are isomorphic to a fixed algebra"*、mathlib の `FormallySmooth.exists_lift`+`FormallyUnramified.ext` から。★`Theorem 2.2` はこれの **almost 版**)・`tensor_range_of_range`(*"the image of `C_σ` in `B` contains `p^{6σ}B`. It follows that `p^{7σ}e_{B/Ā}` lies in `C_σ ⊗_A C_σ`"*)。残るのは、これらを実際の `B_ε`・`C_σ`・`e_{B/Ā}` に当てはめる配線 | `AlmostDeform.lean` + `HochschildLowDegree.lean` |
...
AlmostLifting.lean` の `thm_2_2`)で、原文に無い仮定を4つ足した。
いずれも「原文が almost mathematics の *basic setup*(`(V, m)` with
`m² = m`)として暗黙に置いている前提」を Lean の言葉で明示したもので、
後続(`Theorem 2.3`・`Theorem 3.1`)は同じ setup の中で使うため影響しない。

1. **`PDivTower A q`(塔の明示化)**。原文は `ε ∈ ℚ_{>0}` で添字づけた
   `p^ε` を自由に使う。Lean では `p^ε` に意味を与える必要があるので、
   `ϖ : ℕ → A` with `(ϖ(k+1))^q = ϖ k`(= `p^{1/q^k}` の列)を構造体に
   した。`m := span (range ϖ)` は `m² = m`(`m_sq_eq`)を満たす——
...
(その2)`A = ℤ`、`B = C = Fin 2 → ℤ`、`ϖ ≡ 1`——`B/A` が**非自明な
階数2の有限エタール拡大**。どちらも `∃!` の結論が実際に成り立つ。
