...
## §9-436 座標環の単元は定数である(第 128 ブロック)

層 3 の最終段で要る。★イデアルの言葉で `g_P` を作ると、
まず「`g_P^n` と `μ(f_P)` が同じイデアルを生成する」ところまで出て、
両者は**単元倍で一致する**。★★その単元が定数でなければ `n` 乗根に吸収できない。
...
| 道 | 内容 | 状態 |
|---|---|---|
| A: 因子 | `IsDedekindDomain` を入れて mathlib の素分解を使う | 残り `IsIntegrallyClosed` のみ(10-25) |
| B: イデアルの引き戻し | `[n]^* I := μ(I) の生成する分数イデアル`。乗法的で `[n]^*((f)) = (μ f)` なので `J := [n]^* I_P` が `J^n = (μ f_P)` を満たす | 因子群を経由しない。残るのは `[J] = 0` |

★★どちらの道でも**最終段の単元処理は第 128 ブロックで済んでいる**。

★義務の数は動いていない(Galois 4/8)。

## §9-437 「道 B は因子群を経由しない」は過剰な主張だった——訂正

§9-436 で層 3 の道 B(イデアルの引き戻し)を
「`[n]^* I := μ(I) の生成する分数イデアル` と定めれば因子群を経由しない」と書いた。
★**これは誤りである。**

`FF` の `CR`-加群構造は `algebraMap` によるもので `μ` によるものではない。
したがって `μ(''(f))` の張る `CR`-部分加群は `(μ f)` にならない。
★★正しくは `μ` を体の埋め込みに延ばし、`μ(CR)` の `FF` における整閉包を取る必要があり、
それが `CR` と一致するには**結局 `IsIntegrallyClosed CR` が要る**。

★★★**2 つの道は `IsIntegrallyClosed` で合流する**——これが層 3 の唯一の入口である。
...
★内訳:
(i) 各極大イデアルが `XYIdeal W x y` の形であること(代数閉体上の零点定理)、
(ii) 非特異点では `x − x₀` か `y − y₀` が局所化で極大イデアルを生成すること、
(iii) 局所 Noether 整域で極大イデアルが単項 ⟹ DVR(mathlib の `DiscreteValuationRing.TFAE`)。
...
本セッションで記録の訂正が 2 件出た:
1. §9-434「mathlib に因子群が無い」→ 実は完備していた(探索範囲の敷衍が原因)
2. §9-437 本項「道 B は因子群を経由しない」→ 加群構造の取り違え

★★どちらも**先に進もうとして初めて分かった**。
...
## §9-444 分岐点の局所構造(第 132 ブロック)

§9-443 で「零でない素イデアルでの局所化がすべて DVR」を示せば Dedekind が出ると分かった。
★極大イデアルは `x` 座標 `c` の上にあり、2 つの場合に分かれる:

| 場合 | 局所構造 |
|---|---|
| `Ψ₂Sq(c) ≠ 0`(不分岐) | `x − c` が極大イデアルを生成 |
| `Ψ₂Sq(c) = 0`(分岐、= 2 等分点) | `z` が極大イデアルを生成 |

★★本ブロックは**後者の中身**を出した:

    Ψ₂Sq = (x − c)·e   で   e(c) ≠ 0     (squarefree だから単根、第 131)
      ⟹  z² = (x − c)·e(x)              (`genZ_sq_factor`)
      ⟹  e(x) は局所化で単元だから x − c = z²/e(x) ∈ (z)
      ⟹  極大イデアル = (z)
...
| 段 | 見積もり |
|---|---|
| 極大イデアルが `x` 座標の上にあること(整拡大から) | 3-8 |
| 不分岐の場合の局所生成 | 5-10 |
| `IsDiscreteValuationRing` への接続(`iff_pid_with_one_nonzero_prime`) | 5-10 |
| 因子計算・`n` 乗根の取り出し | 30-75 |

★義務の数は動いていない(Galois 4/8)。

## §9-445 不分岐点の局所構造(第 133 ブロック)——2 つの場合が揃った

第 132 で分岐点(`Ψ₂Sq(c) = 0`)を出した。★本ブロックは**不分岐**の側:

    s² = Ψ₂Sq(c)  (s ≠ 0)  とすると
      (z − s)(z + s) = z² − s² = Ψ₂Sq(x) − Ψ₂Sq(c) = (x − c)·g(x)

★★局所化では `z + s` が単元(`z ≡ s` かつ `2s ≠ 0`)なので
`z − s ∈ (x − c)`、すなわち極大イデアルは `x − c` で生成される。

### ★★★★DVR 判定の材料が揃った

| 場合 | 一意化元 | ブロック |
|---|---|---|
| `Ψ₂Sq(c) = 0`(分岐 = 2 等分点) | `z` | 第 132 |
| `Ψ₂Sq(c) ≠ 0`(不分岐) | `x − c` | 第 133 |

★残るのは (i) 極大イデアルが `x` 座標の上にあること(整拡大から、3-8)、
(ii) `IsDiscreteValuationRing` への接続(5-10)である。

★★**平方完成(第 129)→ squarefree(第 131)→ 局所構造(第 132・133)**という
一本の筋で来た。どの段も 1 ブロックで済んでいる。

★義務の数は動いていない(Galois 4/8)。

## §9-446 素イデアルの生成元が出た(第 134・135 ブロック)——零点定理は要らなかった

§9-442 では「極大イデアルが `XYIdeal` の形であること」に**古典的零点定理**
(mathlib に直接の形では無い、5-15 ブロック)が要ると見積もっていた。
★**それは過大であった。零点定理は使わずに済んだ。**
...
| `F[X]` の 0 でない素イデアルは `X − c` を含む | `F[X]` が PID + `F` 代数閉 |
| `algebraMap(X − c) = x − c` | 第 116 ブロック |

★整拡大(第 116 `coordinateRing_isIntegral`)さえあれば、
**零点定理を経由せずに** `x` 座標が取れる。

### ★★★★★第 135 —— 残りの生成元

1. `z² = Ψ₂Sq(x) ≡ Ψ₂Sq(c) = s²` と第 133 の分解から `z ≡ ±s`。
2. `z = 2y + a₁x + a₃` を解いて `y ≡ y₀ = (s − a₁c − a₃)/2`(`2` が単元)。
3. **`P = (x − c, y − y₀)`**。

★★★3 の機構が肝である。`AdjoinRoot.mk` が全射なので任意の `α` は `mk(p)`
(`p ∈ F[X][Y]`)であり、`Q := (x − c, y − y₀)` で割ると
**環準同型の一致**(`Polynomial.ringHom_ext`)から `α ≡ p(c,y₀) ∈ F`。
`α ∈ P` なら `p(c,y₀) ∈ P` だが `F` の 0 でない元は単元なので `p(c,y₀) = 0`、
すなわち `α ∈ Q`。

★★★★**`Q` が極大であることを示す必要すら無い**——`P ⊆ Q` が直接出る。
`IsField`・商体・基底のどれも使っていない。

### ★★DVR 判定の材料が全部揃った

| 材料 | ブロック |
|---|---|
| `IsNoetherianRing F[W]` | mathlib |
| `Ring.DimensionLEOne F[W]` | 第 127 |
| **`P = (x − c, y − y₀)`** | **第 135** |
| 分岐(`Ψ₂Sq(c) = 0`)で `z` が一意化元 | 第 132 |
| 不分岐(`Ψ₂Sq(c) ≠ 0`)で `x − c` が一意化元 | 第 133 |
| `極大イデアルが単項 ⟺ DVR` | mathlib(`IsDiscreteValuationRing.TFAE` 項 0 ⟺ 項 4) |
| `局所化が全部 DVR ⟹ Dedekind` | mathlib(`IsDedekindDomainDvr.isDedekindDomain`、instance) |

★残るのは**局所化への移送**だけである(`P.map` が極大イデアルであること、
生成元が単項に落ちること)。
...
## §9-448 高さ 1 の素イデアルとアフィン点の対応(第 138 ブロック)

第 137 で因子機構が開いた。★因子を**幾何の言葉**で読むための座標系を敷く。
...
★★点が曲線上にあることは、生成点の方程式(第 114 `equation_gen`)を `P` で割り、
`F ↪ F[W]/P` の単射性で `F` に降ろせば出る。

★★★mathlib の `XYIdeal W c (C y₀) = span {XClass, YClass}` は
第 135 の `span {x − c, y − y₀}` と**同じもの**である
(`XClass W c = mk W (C (X − C c)) = x − c`)。

★これで `div(f)` の各項を「どの点での位数か」として読めるようになった。

★義務の数は動いていない(Galois 4/8)。

## §9-449 層 3 が 2 節点に還元された(第 139 ブロック + `WeilDivisor.lean`)

### ★★★★★★`WeilRoot` 自身の `sorry` が消えた

第 139 ブロック(`Found`)で最後の一歩を出した:

    J^n = (μ f_P)   かつ   J = (g)     ⟹   ∃ g', g'^n = μ f_P

★機構は「単項分数イデアルが等しい ⟹ 単元倍で一致」+「第 128: 単元は定数」
+「`F` 代数閉だから定数の `n` 乗根が取れる」である。

★★これにより `exists_nthRoot_comp_mulByN` は **D1・D2 から証明される**ようになり、
`sorry` は新設した 2 節点に移った:

| 節点 | 内容 | 見積もり |
|---|---|---|
| D1 `exists_fractionalIdeal_pow` | `∃ J, J^n = (μ f_P)` | 20-50 |
| D2 `fractionalIdeal_isPrincipal` | その `J` が単項(Abel–Jacobi) | 10-25 |

### ★★★★★★見通しが変わった——**不分岐性は要らない**

当初は「`[n]` が不分岐だから `ord_Q(μ f) = ord_{[n]Q}(f)`」を経由する予定で、
不分岐性(`deg[n] = n²` と `#E[n] = n²` が要る)を勘定に入れていた。★**不要である。**

一般の付値論で `ord_Q(μ f) = e_Q · ord_{[n]Q}(f)` であり、
`div(f_P) = n(P) − n(O)` から `ord_{[n]Q}(f_P) ∈ {n, −n, 0}`。
★★したがって **`e_Q` が何であっても `n` で割れる**(D1)。

★★★D2 でも `e_Q` は消える。`[n]∘τ_T = [n]`(`T ∈ E[n]`)から `e_Q` は
各ファイバー上で一定であり、共通因子 `e` を括り出すと

    Σ_{Q ∈ [n]⁻¹(P)} Q = n²Q₀ = n·P = 0,      Σ_{T ∈ E[n]} T = 0

により類は 0 になる。★★★★**`e` の値を知る必要はない。**

### ★逸脱の記録

`exists_nthRoot_comp_mulByN` に `[IsAlgClosed F]` を足した。
第 139 が単元(= 定数)の `n` 乗根を取るために使う。
★消費側((G5) `det_cyclotomic`)は `[IsAlgClosed L]` の下で述べられているので
後続の証明に影響しない。★★現時点で本定理の消費者は 0 件(全文検索で確認)。

★全体ビルド成功(5097 jobs)、ゲート PASS。
★義務の数は動いていない(Galois 4/8)。

## §9-450 D2 の部品(第 140 ブロック)

D2(`J` が単項であること)の計算に要る 2 つを先に出した。

### ★★★部品 1 —— `Σ_{T ∈ E[n]} T = 0`

一般に **`B` が有限可換群なら `Σ_{p ∈ B × B} p = 0`** である
(第 1 成分は `Σ_x Σ_y x = |B|·Σ_x x = 0`、Lagrange)。
★`B × B` に同型な群なら同型で送って同じ結論が出る
(`sum_univ_eq_zero_of_addEquiv`)。
★★`E[n] ≅ (ℤ/n)²`((G1) で取得済)に当てれば `Σ_{T ∈ E[n]} T = 0`。

### ★★★部品 2 —— 類が自明なら単項

`ClassGroup.mk_eq_one_iff`(mathlib)と `FractionalIdeal.isPrincipal_iff`(mathlib)を
...
## §9-453 ★★★★★★D1' 場合 A が閉じた(第 143 ブロック)

D1'(`∀ v, n ∣ count_v((μ f_P))`)は 2 つの場合に分かれる。
★本ブロックは**アフィンの場合**(引き戻しが `v` で極を持たない、`w(μ ·) ≤ 1`)を閉じた。
...
* `≤`: **`{r | w(μ r) ≤ γ}` はイデアルである**(`w(μ ·) ≤ 1` だから `R` 倍で閉じる)。
...
両者を結ぶ補題は無かった(2026-08-20 実測)。★本ブロックで

    v(x) = exp(−count_v((x)))          (x ≠ 0)

を出した。★★これにより**超距離不等式が `Valuation` API から無料**になり、
結論は `count` の言葉(整数、加法的)で述べられる。
★★★この橋は D1' 場合 B でもそのまま使える。

### ★残るのは場合 B と D2

| 節点 | 内容 | 見積もり |
|---|---|---|
| D1' 場合 B(`[n]Q = O`) | `ν(x) < 0` の側。`ν(x)` が偶数であることから | 8-20 |
| D2 | `J` が単項——Abel–Jacobi | 10-25 |

★義務の数は動いていない(Galois 4/8)。

## §9-454 D1' 場合 B の入口(第 144 ブロック)

第 143 で場合 A(`∀ r, w(μ r) ≤ 1`)を閉じた。★残るのは**その否定**、
すなわち `∃ r, 1 < w(μ r)` の場合である。

★★本ブロックは**そこから `1 < w(μ x)` が出る**ことを示した:

    r は `F[x]` 上整(第 116)。モニック `p` で `p(r) = 0`。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| B2 | `2·ν(z) = 3·ν(x)` ⟹ `ν(x)` は偶数(`deg Ψ₂Sq = 3`、第 142) | 2-5 |
| B3 | `ν(p + qz) = ν(p − qz)`(偶奇で一致しないから min) | 4-10 |
| B4 | `f_P·ι(f_P) = c(x−x_P)^n` から `2ν(f_P) = n·ν(x)` | 3-8 |
| D2 | `J` が単項——Abel–Jacobi | 10-25 |

★義務の数は動いていない(Galois 4/8)。

## §9-455 場合 B —— `ν(x)` は偶数(第 145 ブロック)

第 144 で場合 B の入口(`1 < w(μ x)`)を出した。★本ブロックはそこから

    w(μ z)² = w(μ x)³        ⟹        `count_v((μ x))` は偶数

を出した。★★根拠は **`deg Ψ₂Sq = 3`(奇数)**である(第 142)。
...
**主係数の付値が 1 の場合**に広げた。`Ψ₂Sq` の主係数は `4` で、
標数 ≠ 2 なら単元だから付値は 1 である。

### ★残っている段

| 段 | 内容 | 見積もり |
|---|---|---|
| B3 | `ν(p + qz) = ν(p − qz)`(偶奇が違うので min が一致) | 4-10 |
| B4 | `f_P·ι(f_P) = c(x−x_P)^n` から `2ν(f_P) = n·ν(x)` | 3-8 |
| D2 | `J` が単項——Abel–Jacobi | 10-25 |
...
## §9-457 ★★★★★★超楕円対合(第 147 ブロック)

D1' 場合 B の残りは `count(μ f_P) = count(μ f_{−P})` である。★これは

    ι(f_P) と f_{−P} は同伴

から出る。★★そのために**超楕円対合** `ι : F[W] → F[W]`(`y ↦ negY(x,y)`)を作った。

### ★★★★構成——`pointHom` の一般化

第 118 の `pointHom` は終域が `F(W)` に固定されていた。
★**任意の `F`-代数へ**一般化すると(`curveHom`)、`ι` は `F[W]` 自身への写像として得られる。
仮説は mathlib の `equation_neg`(`Equation x (negY x y) ↔ Equation x y`)と
...
★★`n` 乗すると `ι((f_P)) = (f_{−P})`、すなわち **`ι(f_P)` と `f_{−P}` は同伴**。

### ★残っている段

| 段 | 内容 | 見積もり |
|---|---|---|
| B7 | `count(μ ι a) = count(μ a)`(基底 `{1,y}` と偶奇) | 4-8 |
| B8 | B7 + 第 141 + 第 145 から `n ∣ count(μ f_P)` | 2-5 |
| D2 | `J` が単項——Abel–Jacobi | 10-25 |

★`deg N` を経由する必要はなくなった——`ι` があれば直接繋がる。

★義務の数は動いていない(Galois 4/8)。

## §9-458 ★★★★★★対合は付値を保つ(第 148 ブロック)
...
(`w(μ ι y) = w(μ y)`)**同じ max になる**。

★★★これが「`ν` は `F(x)` 上への制限で一意に決まる」ことの Lean での正体である。
場所の一意性定理を経由せず、基底と偶奇だけで済んだ。

### ★残っている段

| 段 | 内容 | 見積もり |
|---|---|---|
| B8 | 第 141・145・147・148 を繋いで `n ∣ count(μ f_P)`(場合 B) | 2-5 |
| D1' | 場合 A(第 143)と場合 B を合流させる | 2-5 |
| D2 | `J` が単項——Abel–Jacobi | 10-25 |
...
## §9-460 ファイバーの総和(第 150 ブロック)——D2 の心臓
...
当てるだけで `Σ_{T ∈ E[n]} T = 0` が出た。`#E[n] = n²` も同じ同型から。

★★**G1 が G5 の中で初めて実際に消費された。**これまで G5 は G1 に依存すると
記録していたが、依存が具体的な形で現れたのはここが最初である。

### ★★★ファイバー

`n·Q₀ = P` なら ファイバーは `Q₀ + E[n]` であり
...
| 段 | 内容 | 見積もり |
|---|---|---|
| `count_v(μ f_P)` の台の同定(どの点で 0 でないか) | 幾何の側 | 8-20 |
| 類の計算(`toClass` で点の和として読む) | 第 138・140・150 で材料は揃った | 4-10 |
...
`count_v(μ f_P) ≠ 0` となる `v`(= 曲線上の点 `Q`)を決めること。
★場合 A では `count_v(μ f_P) = n·min(count_v(μ a), count_v(μ b))`(第 143)なので

    count ≠ 0  ⟺  μ a と μ b が共に v で消える  ⟺  [n]Q = P

★★これを言うには**点の還元(特殊化)写像**が要る——`μ` を剰余体 `κ(v) ≅ F`(第 138)へ
落として `([μ x], [μ y]) = [n]Q` を示す(8-20 ブロック)。

### ★★★2026-08-20 の実測 2 件

1. mathlib の `AlgebraicGeometry/EllipticCurve/Reduction.lean` は**曲線**の還元
   (極小モデル・good/multiplicative/additive reduction)であって、
   **点の還元写像ではない**。点の特殊化は自前で積む必要がある。
   ★ただし `Point.map`(環準同型に沿った関手性)は mathlib にあるので、
   剰余体への環準同型を作れば流せる。★★この測定は **(G7) 半安定モデルでも効く**。
2. `Point.toClass` には `toClass_injective` と
   **`toClass_eq_zero : toClass P = 0 ↔ P = 0`** があるが、**全射性は無い**。
   したがって `[J]` を `toClass(R)` の形で得るには `J` を `XYIdeal'` の積として
   **明示的に構成する**必要がある(= 台の同定)。
   ★`toClass_eq_zero` があるので、構成できれば結論は 1 行である。

### ★D2 の内訳(更新)

| 段 | 状態 | 見積もり |
|---|---|---|
| `Σ_{T ∈ E[n]} T = 0`、`#E[n] = n²`、ファイバー総和 | ✅ 第 150 | 0 |
| 類が自明 ⟹ 単項 | ✅ 第 140 | 0 |
| 高さ 1 素点 ↔ 曲線上の点 | ✅ 第 138 | 0 |
| **点の還元(特殊化)写像** | ❌ | 8-20 |
| `J` を `XYIdeal'` の積として構成 | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-462 ★★★★★★因子の台の同定(第 151 ブロック)——剰余体は要らなかった

§9-461 では台の同定に**点の還元(特殊化)写像**が要ると見積もった(8-20 ブロック)。
★**剰余体を経由しない書き方があった。**
...
場合 A では `count ≥ 0` なので
    ...
    count ≠ 0 ⟺ count(μ a) > 0 かつ count(μ b) > 0
             ⟺ a, b ∈ P'  ⟺  I_P ⊆ P'  ⟺  I_P = P'
...
★台の同定そのものは本ブロックで済んだ(見積もり 8-20 → 実際 1 ブロック)。

★義務の数は動いていない(Galois 4/8)。

## §9-463 D2 に残るのは 1 段だけ——点の還元(`PointReduction.lean` を新設)

第 151 で台の同定が済んだので、D2 に残るのは

    P' = XYIdeal(n·Q_v)        ただし v.asIdeal = XYIdeal(Q_v)

すなわち **「点の還元が群準同型である」**(Silverman VII.2)だけである。
★スケルトン節点 `pullbackPrime_eq_xyIdeal_nsmul` として立てた。

### ★★★2026-08-20 実測——mathlib には無い

* `Point.map` は**体の間の単射代数準同型** `f : F →ₐ[S] K` にしか使えない
  (`W'.baseChange_nonsingular f.injective` を使う)。
  還元 `O_v → κ(v)` は**単射でない**ので適用できない。
* `AlgebraicGeometry/EllipticCurve/Reduction.lean` は**曲線**の還元
  (極小モデル・good/multiplicative/additive)であって点の還元ではない。

### ★★★★良い還元であることは自動である

`Δ ∈ F^×`(定数)なので `w(Δ) = 1`、すなわち **`v` では常に良い還元**。
★悪い還元の場合分けは不要。★★残るのは加法公式の場合分け
(`x₁ ≡ x₂ mod m_v` のとき傾きが `O_v` に入らない)である。
...
| 高さ 1 素点 ↔ 曲線上の点 | ✅ 第 138 |
| 類が自明 ⟹ 単項 | ✅ 第 140 |
| `Σ_{T∈E[n]} T = 0`、`#E[n] = n²`、ファイバー総和 | ✅ 第 150 |
| 台の同定 | ✅ 第 151 |
| **点の還元が群準同型** | ❌ 20-45 |
| 類の計算(`toClass_eq_zero` で 1 行) | ❌ 4-10 |

★全体ビルド成功(5112 jobs)、ゲート PASS。
★義務の数は動いていない(Galois 4/8)。

## §9-464 ★★★★★★還元写像(第 152 ブロック)——剰余体を構成せずに済んだ

D2 の最後の段(点の還元)の土台が取れた:

    w t ≤ 1   ⟹   ∃! d ∈ F,  w(t − d) < 1

★**剰余体 `κ(v)` を構成する必要は無かった。**

### ★★一意性

`d ≠ d'` なら `d − d'` は `F` の単元だから付値は 1 である。★易しい。

### ★★★存在——mathlib の `valuationSubringAtPrime` が効いた
...
★★`evalPt` は**第 117 ブロックに既にあった**——重複を作らずに済んだ。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| 還元が加法を保つこと(加法公式の場合分け) | ❌ | 15-30 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-465 ★★★★★★還元は環準同型(第 153 ブロック)

第 152 の `w t ≤ 1 ⟹ ∃! d ∈ F, w(t − d) < 1` の `d` を `red(t)` と書くと、
`red` は付値環の上で**環準同型**である:
...
★★**`w t = 1` なら `red t ≠ 0`** も出た——加法公式の場合分けで
「傾きの分母が還元後も 0 でない」ことを言うのに使う。

### ★★★★これが加法公式の場合分けの土台である

`slope = (y₁ − y₂)/(x₁ − x₂)` に `red` を当てると、
`w(x₁ − x₂) = 1`(= `red x₁ ≠ red x₂`)の場合には
...
| 段 | 内容 | 見積もり |
|---|---|---|
| 加法公式の場合分け(`red x₁ = red x₂` の側) | ❌ | 12-25 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-466 ★★★★★★点の還元が定義できた(第 154 ブロック)

第 153 で `red` が環準同型だと分かったので、
...
が定義できた。★★座標が付値環に入らない点は `0` に送る(無限遠へ落ちる点)。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| `redPoint` の加法性(`red x₁ = red x₂` の側) | ❌ | 12-25 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-467 ★★★★★★加法公式の各部品が還元と可換(第 155 ブロック)
...
そのまま還元と可換になった。★★付値が 1 以下であることも保たれる。

### ★★★★これで残るのは傾き `ℓ` だけ

    red(slope x₁ x₂ y₁ y₂) = slope (red x₁) (red x₂) (red y₁) (red y₂)

★`red x₁ ≠ red x₂` の場合は `slope = (y₁−y₂)/(x₁−x₂)` で分母の付値が 1
(第 155 の `val_eq_one_of_redConst_ne_zero`)だから
...
| 段 | 内容 | 見積もり |
|---|---|---|
| 傾きの還元(`red x₁ ≠ red x₂` の側) | ❌ | 3-6 |
| 傾きの還元(`red x₁ = red x₂` の側) | ❌ | 10-20 |
| `redPoint` の加法性の組み立て | ❌ | 4-8 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-468 ★★★★★★還元は加法を保つ(弦の場合、第 156 ブロック)
...
2. したがって `slope = (y₁ − y₂)/(x₁ − x₂)` の付値は 1 以下で、
...
そこでは還元先で **2 倍公式**に切り替わる。★`x₁ ≠ x₂` なのに `red x₁ = red x₂`
(`w(x₁ − x₂) < 1`)の場合が本質的に難しい——傾きが `O_v` に入らないことがある。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| 還元の加法性(`red x₁ = red x₂` の側) | ❌ | 10-20 |
| `redPoint` が群準同型であることの組み立て | ❌ | 3-6 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-469 ★★★★★★傾きの統一表示(第 157 ブロック)——弦と接線が同じ式になる

mathlib の `slope` は場合分けで定義されているが、★**曲線上の 2 点に対しては
どちらも次の 1 つの式に等しい**:
...
で、**`linear_combination h₁ − h₂` の 1 行**で出る(任意の可換環で成立)。
...
★★★`red(D) = 0` の場合は `red y₁ = negY (red x₂) (red y₂)` が直ちに出るので
還元先で `red S₁ + red S₂ = 0`。上でも傾きが極を持つので `redPoint = 0` になる。

★★★★見積もり 10-20 ブロックだった `red x₁ = red x₂` の場合が、
この 1 つの恒等式で**場合分けなしに**扱えるようになった。

★★★★★これは (G7) 半安定モデルでも効く。

★義務の数は動いていない(Galois 4/8)。

## §9-470 ★★★★★★★還元は加法を保つ(分母が消えない場合、第 158 ブロック)

第 157 の**統一表示** `slope = N/D` により、**弦と接線を区別せずに**還元が扱えた:
...
3. 還元先でも `¬(red x₁ = red x₂ ∧ red y₁ = negY(red x₂)(red y₂))` が成り立つ
   (成り立たなければ `red D = 0`)ので、統一表示が使える。
4. `addX`・`addY` は第 155 で還元と可換。

### ★★★残るのは `red D = 0` の場合だけ

そこでは `red y₁ = negY(red x₂)(red y₂)` が直ちに出るので
**還元先で `red S₁ + red S₂ = 0`**。上でも傾きが極を持つので `redPoint = 0`。

★★★★見積もり 10-20 ブロックだった `red x₁ = red x₂` の場合が、
統一表示のおかげで**独立した場合ではなくなった**。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| `red D = 0` の場合(両辺 0) | ❌ | 4-8 |
| `redPoint` が群準同型であることの組み立て | ❌ | 3-6 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-471 `red D = 0` の場合の部品(第 159 ブロック)

第 158 で `red D ≠ 0` の場合が閉じた。★残るのは
`red x₁ = red x₂` かつ `red D = 0` で、そこでは**両辺がどちらも 0** になる。

### ★★★右辺(還元先の和)

`red D = 0` は `red y₁ = negY (red x₁) (red y₂)`。★`red x₁ = red x₂` と合わせて
mathlib の `Point.add_of_Y_eq` がそのまま効く(`redPoint_add_eq_zero_of_redD`)。

### ★★★左辺(和の還元)

`w(D) < 1` なので傾き `slope = N/D` は極を持つ(`w(N) = 1` のとき)。
★そのとき `addX = ℓ² + a₁ℓ − a₂ − x₁ − x₂` の付値は `w(ℓ)² > 1` になる
——`ℓ²` の項が**単独で最大**だからである(`one_lt_val_addX`)。
★★座標が付値環に入らないので `redPoint = 0`(`redPoint_eq_zero_of_not_le`)。

### ★残っている段

| 段 | 内容 | 見積もり |
|---|---|---|
| `red D = 0` のとき傾きが極を持つこと(`w(N) = 1` の確認) | ❌ | 4-8 |
| `redPoint` が群準同型であることの組み立て | ❌ | 3-6 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |

★`w(N) = 1` は還元先の点の**非特異性**から出る——`red N = −polynomialX(R₂)` であり、
`red D = 0` のとき `polynomialY(R₂) = 0` になる場合は非特異性から `polynomialX(R₂) ≠ 0`。

★義務の数は動いていない(Galois 4/8)。

## §9-472 ★★★★★★★点の還元が加法を保つ(第 160 ブロック)

    redPoint (S₁ + S₂) = redPoint S₁ + redPoint S₂

★場合分けは 4 つで、いずれも既存のブロックで閉じた:

| 場合 | 出所 |
|---|---|
| `S₁ + S₂ = 0` | 第 155 `redConst_negY` |
...
* `red y₁ = red y₂` なら還元先の点が **2 等分点**になるので、
  **非特異性**から分子 `N` の還元が 0 でない
  (`polynomialY = 0` なら `polynomialX ≠ 0`)。したがって `w(N) = 1 > w(D)`。

★したがって `w(addX) = w(slope)² > 1` となり `redPoint = 0`(第 159)。
★★右辺も `red y₁ = negY(red x₂)(red y₂)` から 0(第 159)。

★★★**非特異性がここで初めて本質的に効いた。**これまでは
「楕円曲線では方程式を満たす点は自動的に非特異」としか使っていなかった。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| `redPoint` の `n • ` との可換性(帰納法) | ❌ | 3-6 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |
| 類の計算(`toClass_eq_zero` で 1 行) | ❌ | 4-10 |

★義務の数は動いていない(Galois 4/8)。

## §9-473 ★★★★★★無限遠に落ちる点の付値(第 161 ブロック)

第 160 の `redPoint` の加法性は**両方の点の座標が付値環に入る場合**である。
★`n • S` との可換性には「無限遠に落ちる点の全体が部分群である」ことが要り、
その入口が本ブロックである:
...
が成り立つ(`linear_combination 4·heq` の 1 行、任意の可換環)。
★`w(x) > 1` なら右辺は `4x³` が単独で最大なので `w(RHS) = w(x)³`。
★★左辺から `w(2y+a₁x+a₃)² = w(x)³ > w(x)²` となり `w(2y+a₁x+a₃) > w(x)`、
したがって `a₁x + a₃` の項は落ちて `w(y) = w(2y+a₁x+a₃)`。

★★★これは第 145(生成点の場合、`z² = Ψ₂Sq(x)` と `deg Ψ₂Sq = 3`)と**同じ形**である。
**平方完成が 2 度目に効いた。**
...
## §9-474 ★★★★★★★還元の核が部分群になった(第 162 ブロック)

D2 に残っていた最大の段——**無限遠に落ちる点の全体 `E₁` が加法で閉じていること**——が
閉じた。★見積もり 6-15 に対して **1 ブロック**である。

### ★★★★★★★打ち消しを完全に回避できた

素朴に `addX = ℓ² + a₁ℓ − a₂ − x₁ − x₂` の付値を評価しようとすると、

    w(ℓ)² = w(x₁)   (両方が無限遠のとき、片方が大きい場合)

でちょうど**打ち消しが起きる**。★これが通常の証明が形式群 `z = −x/y` を
持ち出す理由である。

★★**Vieta の第 2 基本対称式に移ると打ち消しが起きない**:

    x₁x₂ + x₁x₃ + x₂x₃ = −(2ℓm + a₁m + a₃ℓ − a₄),   m := y₁ − ℓx₁

* **左辺**: `w(x₁x₂) = w(x₁)·w(x₂)` が単独で最大(`w(x₃) ≤ 1` だから)
* **右辺**: `m = y₃ − ℓx₃` とも書けるので `w(m) ≤ max(1, w(ℓ))`、
  よって `w(右辺) ≤ max(1, w(ℓ), w(ℓ)²) ≤ max(w x₁, w x₂)`

★★★`w(x₁)·w(x₂) ≤ max(w x₁, w x₂)` は `w(x₁), w(x₂) > 1` に矛盾する。

**値群が ℤ であることも、平方根も、形式群も使っていない**——乗法的な付値のまま完結する。

### ★★★Vieta は mathlib にあった(2026-08-20 実測)

`addPolynomial_slope`(3 次式の**完全な因数分解** `−(X−x₁)(X−x₂)(X−x₃)`)と
`addPolynomial_eq`・`Cubic.prod_X_sub_C_eq`・`Cubic.toPoly_injective` を繋ぐと
第 2 基本対称式が **4 行**で出る(`vieta_e2`)。★弦・接線を問わない。

### ★★★★★★混合の場合は第 160 から**無料で出る**

`S₁ ∈ E₁`、`S₂ ∉ E₁` の場合は直接評価すると同じ打ち消しに突き当たる。★しかし

1. 本ブロックの `val_addX_le_of_mixed` で `S₃ := −(S₁+S₂)` が付値環に入る
2. `S₂ + S₃ = −S₁ ∈ E₁` に**第 160 の整な加法性**を当てる
3. `red S₂ + red S₃ = red(−S₁) = 0`、すなわち `red(S₁+S₂) = −red S₃ = red S₂`

★★**混合の場合を直接評価する必要はない**。既に積んだものが効いた。

### ★本ブロックで取れたもの

| 定理 | 内容 |
|---|---|
| `val_y_le_of_val_x_le` | ★★★`w x ≤ 1 ⟹ w y ≤ 1` |
...
「無限遠に落ちる点の全体が部分群」6-15 → **1**。
★原因: 形式群を要ると見込んでいたが、**Vieta の第 2 基本対称式**に移れば
打ち消しが起きず、しかも mathlib が 3 次式の因数分解を持っていた。
...
| `redPoint` の完全な加法性(3 つの場合の合流) | ❌ | 2-4 |
| `redPoint` の `n • ` との可換性(帰納法) | ❌ | 1-3 |
| `P' = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |
| 類の計算(`toClass_eq_zero` で 1 行) | ❌ | 4-10 |

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-475 ★★★★★★★★点の還元が群準同型になった(第 163 ブロック)

第 162 で残りが埋まったので、3 つの場合が合流した。

| 場合 | 判定 | 根拠 |
|---|---|---|
| 整 + 整 | `w x₁ ≤ 1`、`w x₂ ≤ 1` | 第 160 |
| 無限遠 + 無限遠 | `1 < w x₁`、`1 < w x₂` | 第 162 |
| 無限遠 + 整 | 混合 | 第 162 + **第 160 の再利用** |

### ★★★★★★混合の場合を直接評価せずに済ませた

`S₁ ∈ E₁`、`S₂ ∉ E₁` を素直に評価すると `w(ℓ)² = w(x₁)` の打ち消しに突き当たる
(E₁ の閉性と同じ難所)。★しかし

1. 第 162 の `val_addX_le_of_mixed` で `T := S₁ + S₂` が付値環に入る
2. `T + (−S₂) = S₁` に**第 160 の整な加法性**を当てる
3. `red T + red(−S₂) = red S₁ = 0` から `red T = red S₂`

★★**同じ道具を 2 度使うだけで済んだ**。

### ★★★★`n • ` との可換性は mathlib から来る

`redHom` を `AddMonoidHom` として束ねると `map_nsmul`・`map_zsmul` がそのまま効く
(`redPoint_nsmul`・`redPoint_zsmul`、各 1 行)。
...
「`redPoint` の完全な加法性 + `n • ` との可換性」2-4 + 1-3 → **1**。
★原因: 混合の場合に**既に積んだ第 160 を再利用できる**ことを見落としていた。
...
|---|---|---|
| 無限遠に落ちる点の全体が部分群であること | ✅ **第 162** | 0 |
| `redPoint` の完全な加法性 | ✅ **第 163** | 0 |
| `redPoint` の `n • ` との可換性 | ✅ **第 163** | 0 |
| `P'  = XYIdeal(n·Q_v)` への組み立て | ❌ | 4-10 |
| 類の計算(`toClass_eq_zero` で 1 行) | ❌ | 4-10 |
...
## §9-479 ★★★★★★`hnn` は仮定でなく定理だった(第 167 ブロック)
...
座標環は `AdjoinRoot` なので `AdjoinRoot.mk` が全射。★`AdjoinRoot.lift_mk` により
`curveHom W heq r` は **2 重の `eval₂`** になる。★★第 144 の `valuation_eval₂_le_one`
を 2 回当てるだけ(`valuation_curveHom_le_one`)。
...
を示した(`pullbackPrime_curveHom`)。★★平行移動 `τ_T` による素点の輸送で
そのまま使える形である。

### ★★★実測: 環準同型の一意性は既にあった

「座標環からの環準同型は `genX`・`genY`・定数で決まる」を新しく書こうとしたが、
**第 119 ブロックに既にあった**(`FieldHomExt.lean` の `coordinateRing_hom_ext`)。
★`lake build` が重複宣言で落ちて気付いた。★★既存在庫の確認を先にすべきだった
(この種の取りこぼしは今セッション 2 件目——`evalPt` に続く)。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| `hnn` の自動化・引き戻し素点の一般形 | ✅ **第 167** | 0 |
| `τ ∘ μ = μ`(`n·T = 0`、第 119 の一意性 + `Point.map`) | ❌ | 1-3 |
| 素点の輸送 `count_v ∘ τ = count_{v'}` | ❌ | 2-5 |
| `e_v` の一定性 → `Σ e_v · Q_v = 0` の組み立て | ❌ | 2-4 |

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-480 ★★★★★★★`μ f_P` が平行移動で不変になった(第 168 ブロック)

`[n] ∘ τ_T = [n]`(`T ∈ E[n]`)を Lean の言葉にした:

    τ_T ∘ μ = μ        (μ = [n]^*)

★これが `e_v` の一定性の**唯一の源**である。

### ★★★★★機構は 4 段

1. **`τ_T` は生成点を `生成点 + T` に送る**——mathlib の `Point.map`
   (**加法群の準同型 `→+`**)に第 124 の `τ(coordX) = translateX` を当てる。
   ★`translateX`・`translateY` が「生成点 + 定数点」であることは `Point.add_some`
   そのもの(`generic_add_toFF`)。
2. **`n • ` と可換**なので `τ_T(n·生成点) = n·生成点 + n·T = n·生成点`。
3. したがって `τ(x_n) = x_n`、`τ(y_n) = y_n`(`Point.some` の単射性)。
4. **環準同型は生成元で決まる**(第 119)ので `τ ∘ μ = μ`。

★★**`Point.map` が `→+` として mathlib にあった**ことが効いた——群法則との
可換性を自前で示す必要はなかった。

### ★★躓いた点: `DecidableEq` の実体ずれ

`slopeFF_eq`(第 120)は `@slope _ _ _ (Classical.decEq _) ...` と**実体を固定**して
述べられている。★`Point.add_some` が使う実体と食い違い、`rfl` が
200000 heartbeat で落ちた。★★傾きを `slopeFF` 側に書き換えてから合わせると通る
(`simp only [hs, translateX, translateY]`——`rw` は従属モチーフで失敗する)。
...
|---|---|---|
| `τ ∘ μ = μ` | ✅ **第 168** | 0 |
| 素点の輸送 `w_v ∘ τ = w_{v'}` | ❌ | 2-5 |
| `e_v` の一定性 → `Σ e_v · Q_v = 0` の組み立て | ❌ | 2-4 |

★素点の輸送は次の形で閉じる見込み:
`ν := count_v ∘ τ` は `F[W]` 上非負で、中心は第 167 の `pullbackPrime_curveHom` により
`XYIdeal(Q_v + T)`。★DVR の一意化元 `π` を取ると `ν = m · ord_{v'}`(`m ≥ 1`)。
★★`T` と `−T` で 2 回使えば `m·m' = 1`、すなわち `m = 1` が出る
(全射性を示す必要がない)。

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-481 ★★★★★★★素点の輸送が閉じた(第 169 ブロック)

D2 の最後の道具である

    count_v(e z) = count_{v'}(z)      (e : K ≃+* K、中心が v')

が取れた。

### ★★★★★★中心が `v'` の付値は `v'`-進付値の冪

`W'` を `K` 上の付値で `W'(a) ≤ 1`(∀a ∈ A)かつ `W'(a) < 1 ⟺ a ∈ v'` を満たすものとすると、
一意化元 `π` を取って

    W'(z) = W'(π) ^ ord_{v'}(z)

★鍵は **`ord_{v'}(u) = 0` なら `u = c/d`(`c, d ∉ v'`)**。これは mathlib の
`valuationSubringAtPrime_eq_valuationSubring` と
`IsLocalization (v'.asIdeal.primeCompl) (valuationSubringAtPrime K v')` から出る。
★★この 2 つが揃っていたのが効いた(自前で DVR の構造を組む必要がなかった)。

### ★★★★★`m = 1` は対称性から——全射性を経由しない

上の表示から `count_v(e z) = m · count_{v'}(z)`。★`e.symm` にも当てて
`count_{v'}(e.symm w) = m' · count_v(w)`。★★`w := e π` と置くと `1 = m'·m`、
そして `π ∈ v'` から `m > 0` なので **`m = 1`**。

★★★通常の証明は「`W'` が全射」を経由するが、**`e` が同型であることを 2 回使えば
全射性は不要**である。これは今回の発見で、記録に残す価値がある。
...
★これは一般の Dedekind 環に対する定理なので、(G7) 半安定モデルでもそのまま使える。
...
|---|---|---|
| `τ ∘ μ = μ` | ✅ **第 168** | 0 |
| 素点の輸送 `count_v ∘ τ = count_{v'}` | ✅ **第 169** | 0 |
| `e_v` の一定性(輸送の仮説を第 167 で満たす) | ❌ | 1-3 |
| `Σ e_v · Q_v = 0` の組み立て(第 150・166) | ❌ | 2-4 |

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-482 ★★★★★★★★`e_v` の一定性が出た(第 170 ブロック)

第 168(`τ ∘ μ = μ`)と第 169(素点の輸送)を繋いだ:

    count_v(z) = count_{v'}(z)      (τ z = z、Q_{v'} = Q_v + T)

★`z := μ f_P` に当てれば **`e_v` はファイバー上で一定**である。

### ★★★★★輸送の仮説を第 167 でそろえた

第 169 の `count_comp_eq` が要求する 4 つの仮説は、

| 仮説 | 出どころ |
|---|---|
| `w_v(τ(a)) ≤ 1` (∀a ∈ F[W]) | 第 167 `valuation_curveHom_le_one` |
| `w_v(τ(a)) < 1 ⟺ a ∈ v'` | 第 167 `pullbackPrime_curveHom` |
| `τ.symm`・`v'` について同じもの | 本ブロック `autFF_symm_coord` |

でそろう。★`τ ∘ algebraMap` が `curveHom` であることは第 119 の
`coordinateRing_hom_ext` から。★★`w_v(τ coordX) ≤ 1` は第 163 の
`redPoint_eq_zero_iff` と `red(τ(生成点)) = Q_v + T` から出る
(**`Q_v + T ≠ 0` すなわちアフィン点であることが効く**)。

### ★★★★`τ.symm` は `−T` の平行移動である

`τ.symm ∘ τ = id` を点の側で読むと `τ.symm(生成点) = 生成点 − T`。
★`exists_translateAut_all` を `−T` に当て直す必要はなかった。
...
★台はファイバー `[n]⁻¹(P)` と `E[n] ∖ {O}` の 2 つに分かれる。
★★**2 つの係数が一致する必要はない**——

    S = α·(Σ_{ファイバー} Q) − β·(Σ_{T ∈ E[n]∖{O}} T) = α·(n·P) − β·0 = 0

であり、`Σ_{ファイバー} Q = n·(n·Q₀) = n·P = 0`(第 150 + `n·P = 0`)と
`Σ_{T ∈ E[n]} T = 0`(第 150)がそれぞれ 0 だからである。
★★★これは**分岐指数の大域的な一定性を示さずに済む**ということであり、
見積もりを押し下げる。
...
|---|---|---|
| `e_v` の一定性 | ✅ **第 170** | 0 |
| 台の分解(ファイバー / `E[n]∖{O}`)と総和 | ❌ | 4-10 |

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-483 ★★★★★★★★差が `n` 等分点なら指数は等しい(第 171 ブロック)

第 170 の `count_translate_eq` は平行移動 `τ` を仮定として要求していた。★本ブロックで
**「2 つの点の差が `n` 等分点である」だけを仮定にした形**にした:

    n·(Q_{v'} − Q_v) = 0   ⟹   count_v(μ r) = count_{v'}(μ r)

★★差が 0 の場合は `v = v'`(`XYIdeal` が点を決める、第 164)。0 でなければ
...
第 167 で `hnn` が定理になったので、さらに
    ...
    n·Q_v ≠ 0   ⟹   ∀ r, w_v(μ r) ≤ 1        (`valuation_mulByN_le_one`)

が言える。★`redPoint(n·生成点) = n·Q_v`(第 163・164)と `redPoint_eq_zero_iff`
(第 163)を繋ぐだけである。

★★**場合 A / 場合 B の区別が「`n·Q_v` が 0 かどうか」になった。** これで台が

* `[n]⁻¹(P)`(場合 A、`count > 0`)
* `E[n] ∖ {O}`(場合 B、`count < 0`)

の 2 つに分かれることが**点の言葉で**見える。§9-461 で「場合 B は無限遠」と書いていた
ものが、`n·Q_v = 0` という判定に置き換わった。
...
| 段 | 内容 | 見積もり |
|---|---|---|
| `e_v` の一定性(点の言葉で) | ✅ **第 171** | 0 |
| 素点と点の再添字づけ、台の分解、総和 | ❌ | 3-8 |

★残るのは `Finset.sum_image`・`Finset.sum_subset`・`Finset.sum_union` による
...
## §9-485 ★★★★★★係数が一致する必要はない(第 173 ブロック)

D2 の残りの組み立てのうち、**抽象的な部分と 2 つの総和**を閉じた。

    S = α·(Σ_{[n]⁻¹(P)} Q) + β·(Σ_{T ∈ E[n]∖{O}} T) = α·0 + β·0 = 0

★**2 つの係数 α, β が一致する必要はない**。ファイバーの総和
`Σ_{[n]⁻¹(P)} Q = n·(n·Q₀) = n·P = 0`(第 150 + `n·P = 0`)と
`Σ_{T ∈ E[n]} T = 0`(第 150)が**それぞれ独立に** 0 だからである。

★★これは **`[n]` の不分岐性(`e = 1`)を示さずに済む**ということであり、
通常の証明(Silverman III.8.1)と違う道である。分岐指数はここまで一度も
値を知る必要がなかった。

### ★躓いた点: シェルのバッククォート展開

`python -c "..."` の中でバッククォートを**エスケープし忘れた**ため、
記録の一部が空文字に置き換わった(今セッション 2 回目)。★以後、記録は
スクラッチパッドにスクリプトを書いてから実行する。
...
## §9-487 ★★★★★`τ(g)/g` は定数である(第 176 ブロック)

D2 が閉じたので `g_P^n = μ f_P` なる `g_P ∈ F(W)` が取れる。★Weil 対は

    e_n(P, Q) := τ_Q(g_P) / g_P        (Q ∈ E[n]、τ_Q は平行移動)

で定める。★★これが **`F` の元**であることが定義の前提であり、本ブロックで示した。

### ★★★機構は 2 段

1. **`(τ_Q(g_P)/g_P)^n = 1`** —— 第 168 の `τ ∘ μ = μ` から
   `τ_Q(g_P)^n = τ_Q(g_P^n) = τ_Q(μ f_P) = μ f_P = g_P^n`。
2. **1 の `n` 乗根は定数** —— `h^n = 1` なら `h` は `X^n − 1` の根なので `F[W]` 上整。
   ★第 137 の**整閉性**から `h ∈ F[W]`、そこで `h^n = 1` は単元なので、
   第 128 の**単元は定数**から `h ∈ F`。

★★★**第 137(整閉性)と第 128(単元は定数)がここで効いた。** どちらも D2 の
ために積んだものであり、追加の在庫は要らなかった。
...
| 節点 | 内容 | `sorry` |
|---|---|---|
| `WeilPairingDef.lean` | `e_n` の定義(全域関数として)と 5 性質 | 6 |
| `WeilPairing.lean` | `det_galRep_eq_cyclotomic` | 1 |
| `WeilFunctionField.lean` | `x([n]P) = Φ_n/ΨSq_n`(**臨界路外**) | 1 |

★次の設計上の課題: スケルトンの `weilPairing` は
`W.Point → W.Point → F` という**仮定なしの全域関数**である。
`f_P`・`g_P`・`τ_Q` の選択を含むので、「特徴づける性質」を書いてから
`Classical.choose` で定義し、選択に依らないことを示す必要がある。
★★選択に依らないことの根拠は既にある——`g_P` を `ζ g_P`(`ζ^n = 1`、定数)に
替えても `τ_Q(ζ g_P)/(ζ g_P) = τ_Q(g_P)/g_P`(τ は定数を固定する)。

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-488 ★★★★★★Weil 対が選択に依らないこと(第 177 ブロック)

`e_n(P, Q) := τ_Q(g_P) / g_P` の定義に入る選択は 3 つある。★どれを取っても
値が変わらないことの根拠を出した。

| 選択 | 依らない理由 | 定理 |
|---|---|---|
| `f_P`(`XYIdeal(P)^n` の生成元) | 生成元は定数倍の差(第 128) | `aut_div_eq_of_const_mul` |
| `g_P`(`μ f_P` の `n` 乗根) | `g'/g` は定数 | 同上 |
| `τ_Q`(平行移動) | 座標での値が決まれば一意 | `aut_ext` |

### ★★★★★定数の `n` 乗根も定数である

`h^n = c`(`c ∈ F^×`)なら、`F` が代数閉なので `c = e^n` と書けて `(h/e)^n = 1`。
★第 176 の `const_of_pow_eq_one` から `h/e` は定数、よって `h` も定数
(`const_of_pow_eq_const`)。
...
| `aut_div_eq_of_const_mul` | ★★★★★★**値が `g` の定数倍に依らない** |
| `aut_ext` | ★★★★★**自己同型は座標での値で決まる** |

### ★次の段

残るのは `weilPairing` の**定義そのもの**である。スケルトンは
`W.Point → W.Point → F` という仮定なしの全域関数なので、

    WeilSpec W n P Q c := ∃ (μ) (f_P) (g) (τ), (条件) ∧ τ g / g = algebraMap c

を存在形で書き、`if h : ∃ c, WeilSpec c then h.choose else 1` で定義する。
★存在形にすると、データが無いとき(`P` や `Q` が `n` 等分点でないとき)は
既定値 1 になるので、`e_n(P,Q)^n = 1` などが**退化した場合でも成り立つ**。
...
## §9-490 ★★★★★★★★Weil 対の存在と第 1 性質(第 179 ブロック)

第 178 で定義した `weilPairingVal` が「本物の」値になるための **witness** が取れた。
★材料はすべて既に積んであった:

| 部品 | 出どころ |
|---|---|
| `f_P`(`XYIdeal(P)^n` の生成元) | 第 126 |
...
★`τ (μ f_P) = μ f_P` には `n·Q = 0` が要る——スケルトンの述べ方
(`weilPairing_pow_eq_one` の仮定 `hQ : n • Q = 0`)と一致する。
★★データが無い場合は既定値 1 なので `1^n = 1`。**仮定の少ない述べ方のまま通る**。

### ★★残る仮定: `μ` の単射性

`μ`(= `[n]^*`)の単射性は仮定に置いたままである。★これは既知の未解決事項で、
`Skeleton` の `dvd_count_pullback` の `.needs` に
「`x([n]·)` の超越性を別途示す必要がある(3-8 ブロック)」として記録済み。

★★2026-08-20 の見通し: `μ` の核は素イデアルで、0 でなければ極大(第 138)なので
`XYIdeal(R)` の形。★するとその像は `F` に入り、`x_n = μ(genX)` が定数になる。
★★しかし第 171 の `redPoint(n·生成点) = n·Q_v` から、定数点なら**すべての** `v` で
`n·Q_v` が同じ点になってしまう。`E(F)` は無限で `E[n]` は有限だから矛盾。
★★★つまり **第 171 を使えば単射性が出る**見込みである(3-6 ブロック)。
...
| `weilPairingVal_pow_eq_one` | ★★★★★★**`e_n(P,Q)^n = 1`**(5 性質のうち 1 つ目) |

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-491 ★★★★★★★★`[n]` の引き戻しが単射になった(第 180 ブロック)

`Skeleton` の `dvd_count_pullback` の `.needs` に

> ★★`μ` の単射性を仮定に置いた。……`x([n]·)` の超越性を別途示す必要がある(3-8 ブロック)

と記録していた項目が**定理になった**。★**超越性を経由せずに済んだ**。

### ★★★★★★機構——第 171 の還元を使う

1. `μ` の核は素イデアル。0 でなければ第 138 の `prime_eq_xyIdeal` で `XYIdeal(R)` の形。
...
4. `v` は `F` のすべての点を走る(第 172 の `placeOf`)。★★したがって任意の 2 点の差が
   `n` 等分点になり、`E(F)` が `E[n]`(有限)に単射する。
5. `F` は代数閉なので各 `x` に対して点がある(第 122)。★`F` は無限なので矛盾。

★★★**D2 のために積んだ第 171・172 が、そのまま別の未解決事項を解いた。**
見積もり 3-8 → **1**(18 件目の見積もり修正)。

### ★本ブロックで取れたもの

| 定理 | 内容 |
|---|---|
| `yOf` / `somePt` | ★各 `x` に対する曲線上の点 |
...
| 段 | 内容 |
|---|---|
| 5 性質の残り 4 つ | 双線型性・交代性・非退化性・Galois 同変性 |
| `det_galRep_eq_cyclotomic` | `WeilPairing.lean` |
...
## §9-494 ★★★★★★★双線型性の中核(第 183 ブロック)

3 つのブロックが合流して、`e_n(P₁,Q)·e_n(P₂,Q) = e_n(P₁+P₂,Q)` の中核が出た。

| 材料 | 出どころ |
|---|---|
| `f₁ f₂ = u · h^n · f₃` | 第 182 |
| `μ̃`(`μ` の関数体への延長)と `τ ∘ μ̃ = μ̃` | 第 181 |
| `(g₁g₂/(μ̃(h)·g₃))^n = 定数 ⟹ 定数` | 第 177 |
| `g₁g₂ = ζ·z·g₃ ⟹ (τg₁/g₁)(τg₂/g₂) = τg₃/g₃` | 第 181 |
...
### ★残る段(双線型性を `weilPairingVal` の言葉にする)

`WeilSpec` の witness(第 179)を `P₁`・`P₂`・`P₁+P₂` の 3 つで取り、
**同じ `μ`・同じ `τ`** を使うように揃えてから本ブロックを当てる。
★`μ` は第 178 の一意性から自動的に揃う。★★`τ` は `Q` だけで決まる(第 177 `aut_ext`)。
...
## §9-495 ★★★★★★★★双線型性が出た(第 184 ブロック)

    e_n(P₁, Q) · e_n(P₂, Q) = e_n(P₁+P₂, Q)

★5 性質のうち **2 つ目**である(1 つ目は第 179 の `e_n(P,Q)^n = 1`)。
...
`(τg₁/g₁)(τg₂/g₂) = τg₃/g₃` を当てると `c₁ c₂ = c₃` が出る。

### ★★★`hμinj` が仮定から消えた

第 180 の `mulByN_injective` を内部で呼ぶので、単射性を外から渡す必要が無くなった。
...
## §9-496 交代性に向けた足場(第 185 ブロック)

交代性 `e_n(P,P) = 1` の古典的な証明は

    F := ∏_{i=0}^{n-1} τ_{iP}^*(f_P)   は定数

を経由する。★これを**指数の計算**で言うための部品を 2 つ置いた。
...
が直ちに出る。★専用の補題を作らずに mathlib の 2 行で済むのは、
第 172 の `pointSpectrum` が `XYIdeal` をそのまま `asIdeal` に持っているからである
(`rfl` で繋がる)。

### ★★★★★★素点の輸送(一般形)

第 170 の `count_translate_eq` は `τ z = z` を仮定していた——D2 では
`z = μ f_P` で、これは `τ` で動かないからである。
★しかし交代性では `z = f_P` そのものを動かす。★★そこで**仮定を外した形**を切り出した:

    count_v(τ z) = count_{v'}(z)        (Q_{v'} = Q_v + T)

★★★中身は第 169 の `count_comp_eq` に第 170 の仮説をそのまま渡すだけで、
新しい数学は入っていない。★★第 170 の時点で一般形で書いておけば良かった——
仮定を早く入れすぎたという記録である。

### ★この先の指数計算

    count_v(F) = Σ_i [ n·[Q_v + iP = P] − n·[Q_v + iP = O] ]

`Q_v ∈ ⟨P⟩` なら各和が 1 項ずつで打ち消し、そうでなければ両方 0。
★どちらでも 0 なので `F` は単元、すなわち**定数**(第 128)。
...
## §9-497 ★★★★★★★分割多項式を迴回できた(第 186 ブロック)

交代性 `e_n(P,P) = 1` の古典的な証明は、`P` の **`n` 等分点** `P'`(`n·P' = P`)を取る。
★これは `[n] : E(F̄) → E(F̄)` の全射性で、普通は**分割多項式**
`Φ_n(x) − a·Ψ_n(x)² = 0` の根の存在(次数 `n²`、`F` は代数閉)で示す。

★★しかし `Skeleton/GaloisRep/WeilFunctionField.lean` の `x([n]P) = Φ_n/ΨSq_n` は
**臨界路の外**として残してある。★★★そこで、**位数だけで迴回した**。

### ★★★★★★★数え上げで出る

`#E[n] = n²`(`torsion_card`、既にある)を `n` と `n²` の 2 回使うだけで、
`[n] : E[n²] → E[n]` の全射が出る:
...
| 3 | Lagrange と第一同型で `#E[n²] = #image · #ker φ` |
| 4 | `n⁴ = #image · n²` すなわち `#image = n² = #E[n]` |
| 5 | `image ≤ E[n]` で位数が等しい ⟹ `image = E[n]` |

★★★**分割多項式は要らない**。★「臨界路の外」と印を付けた項目を
**迴回できた**という記録である。★★印を付けたときは「後で必要になるかも」と思っていたが、
別の道があった。
...
| `exists_nsmul_eq_point` | ★★★★★★★**捻れ点は `n` で割れる** |

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-498 ★★★★★★★★無限遠の極の位数が決まった(第 187 ブロック)

交代性を止めていたのは **1 つの量**だけだった。

    count_v(τ_T f_P) = n·[Q_v + T = P] − n·[Q_v + T = O]

の第 2 項、すなわち**平行移動で無限遠に落ちる素点での極の位数**である。

### ★使えなかった道具 2 つ

| 道具 | なぜ使えないか |
|---|---|
| 第 185 `count_translate_gen` | `Q_v + T = O` のとき `v'` が**存在しない** |
| 第 166 Abel–Jacobi | 類群には**次数の情報が無い**——`(n + a)·T = 0` しか出ない |

★★Abel–Jacobi では**原理的に決まらない**。これは記録しておく価値がある——
類群を回しても出ない量がある。

### ★★★★★★★★値群の全射性で決まる

`w := count_v ∘ τ` は `F(W)` 上の**全射**な離散付値である(`τ` は自己同型)。
★一方、第 143-149(場合 B)の道具立てから `count_v(τ x) = 2k` と置くと:

| 材料 | 出どころ |
|---|---|
| `count(τ p(x)) = deg p · 2k` | `count_algebraMap_poly`(第 145) |
| `count(τ y) = 3k` | `count_genY_relation` |
| `2 deg p ≠ 2 deg q + 3` で 2 項は決して打ち消さない | `valuation_poly_ne_genY_term` |

より **`count_v(τ a) ∈ k·ℤ`**。★分数体に延ばしても同じ。
★★`w` は全射なので `k ∣ 1`、`count_v(τ x) < 0` から `k < 0`、よって **`k = −1`**。

### ★★★★★★そこから `−n` が出る

第 146-148 の場合 B の計算は、実は**等式**を与えていた:

    2·count(τ f_P) = n·count(τ x)

★第 149 は可除性だけを返していたが、中身は等式だった。
★★`count(τ x) = −2` を入れて **`count(τ f_P) = −n`**。

### ★★★場合 B に入ることは幾何で判定できる

`Q_v + T = O` なら `redPoint(autFF τ (generic)) = 0`、第 163 の `redPoint_eq_zero_iff` で
`1 < v(τ x)`——すなわち**場合 B**。★第 170 の `aut_transport_hyps` の中で
使われていた補題が、そのまま**逆向きに効いた**。
...
## §9-499 ★★★★★★★★平行移動の積が定数になった(第 188 ブロック)
...
★★`IsTranslate` という述語で `T = O` を含めて一つの形にまとめた。
...
| `IsTranslate` | `τ` が点 `T` による平行移動(`T = O` 込み) |
| `count_translate_fP_gen` | ★★★★★★★**平行移動した `f_P` の指数(全素点)** |
| `prodTranslate_const` | ★★★★★★★★**積は定数** |

### ★残る段(交代性)

1. `H := ∏_{i<n} τ_{iP'}^*(g)`(`nP' = P`、第 186)を作る
2. `H^n = μ(∏_i τ_{iP}^* f_P) = μ(定数) = 定数` —— `τ_T ∘ [n]^* = [n]^* ∘ τ_{nT}`
3. 第 177 で `H` は定数
...
## §9-501 ★★★★★★★★★交代性が出た(第 190 ブロック)

    e_n(P, P) = 1

★5 性質のうち **3 つ目**である。★★第 186-189 で積み上げた 4 つがここで合流した。

| 材料 | 出どころ |
|---|---|
| `n·P' = P` なる `P'` | 第 186(位数からの割り算、分割多項式不要) |
...
`hchar` の範囲が `k ≤ n²`(第 186 が `#E[n²] = n⁴` を使うため)。
...
## §9-504 ★★★★★★★★★Galois 同変性が出た(第 193 ブロック)

    σ(e_n(P, Q)) = e_n(σP, σQ)

★5 性質のうち **4 つ目**である。

### ★★★★★★★mathlib の `Point.map` では足りない

`Point.map` は `f : F →ₐ[S] K` を要求する——**半線型写像は表せない**
(`σ` が底体 `L` に非自明に効くから)。
★底を `K`(固定体)に取れば形式的には乗るが、`W₀.baseChange` との型の同定で
インスタンスの手入れが必要になる。
★★そこで**加法公式を直接輸送して** `semiPoint` を作った:
...
### ★★★★★★★★要は「`Σ_F` が `n·生成点` を固定する」

`Σ_F` は `coordX`・`coordY` と係数を固定するので、`semiPoint` は生成点を固定し、
加法準同型だから `n·生成点` も固定する。
...
## §9-507 ★★★★★★★非退化性を止めている 1 行を特定した(第 196 ブロック)

非退化性(5 性質の最後)の古典的な証明は:
...
★★★等号 `F(E)^{E[n]} = [n]^* F(E)` を出すには最後の 1 行だけが要る。

### ★★★★残る 1 行の中身

`deg[n] = n²` であり、道は 2 つしか見えない:

| 道 | 中身 |
|---|---|
| 分割多項式 | `x([n]P) = Φ_n/ΨSq_n` の次数が `n²`(`Skeleton/GaloisRep/WeilFunctionField.lean`) |
| 双対同種 | `deg` が 2 次形式であること |

★どちらも**本プロジェクトがこれまで避けてきた量**である。
★★第 186 では分割多項式を迴回できたが、ここでは臨界路に戻ってくる。
★★★Abel–Jacobi や位数の数え上げでは**原理的に出ない**——拡大次数の情報だからである
(第 187 で記録した「類群には次数の情報が無い」と同じ型の壁)。
...
## §9-509 ★★★★★★残る 1 行の形が定まった(第 198 ブロック)
...
である。★★mathlib の `natDegree_Φ_le` と `coeff_Φ` から
**`Φ_n` はモニックで次数ちょうど `n²`**(仮定は何も要らない——実測)。
★★★`ΨSq_n` の次数は `n² − 1` 以下なので、**差もモニックで次数 `n²`**。

### ★★★★★★★これで「約分」の心配が消えた

当初は `finrank_eq_max_natDegree`(Lüroth の副産物)で
`max(deg num, deg denom)` を使うつもりだったが、
それには **`Φ_n` と `ΨSq_n` が互いに素**であることが要った。
★モニック多項式による上からの評価に切り替えたので、**互いに素性は要らない**。
★★下からの評価は Artin が与えるので、上下が挟んで等号になる。
★★★**測ってから道具を選ぶと、要求が軽くなる**という例である。
...
| `monic_mulByNPoly` | ★★★★★★**`Φ_n − c·ΨSq_n` はモニックで次数 `n²`** |

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-510 ★★★★★★★分点多項式と群法則が実際に噂み合うことを確かめた(第 199 ブロック)

残る 1 本は **`Φ_n(x) = x_n · ΨSq_n(x)`** である(第 198)。
★その **`n = 2` の場合**を証明した。
★★これは帰納法の底であると同時に、**mathlib の `Φ`・`ΨSq` の定義が
我々の群法則(`addX`・`slope`)と本当に噂み合うことの確認**でもある。
...
★★★係数は手で出した: `N·(N + a₁d) = A² + AB − a₁²(y²+a₁xy+a₃y)` と
`d² = ΨSq₂ + 4E` から `E` の係数を集めたものである。

### ★Lean 上の注意(記録)

`field_simp` は `d = 2y + a₁x + a₃` を**展開してしまう**と `d ≠ 0` と噂み合わなくなる。
★`obtain ⟨d, hdd⟩ : ∃ d, d = …` で **`d` を不透明な変数にしてから** `field_simp` し、
そのあと `subst` して `linear_combination` する。★★これは今後も使う手である。

### ★★帰納法の見通し(測定)

一般の `n` に進むには `y` 側の多項式 `ω_n` が要るが、
**mathlib は `ω_n` をまだ持たない**(docstring に `TODO: the bivariate polynomials ωₙ`)。
★したがって道は 2 つ:

| 道 | 中身 |
|---|---|
| `ω_n` を自分で定義して帰納法 | 典型的だが重い |
| `x` だけの(Kummer)公式で帰納法 | `x(P+Q)` と `x(P−Q)` の対称式を使う |

★★どちらも 30-80 ブロック規模である。★★★本ブロックで
**底の計算が実際に通ること**が確かめられたので、道の存在は確実である。
...
## §9-511 ★★★★★★★Kummer の 2 公式で `ω_n` を回避できる(第 200 ブロック)

残る 1 本 `Φ_n(x) = x_n·ΨSq_n(x)` を一般の `n` へ進めるには、
古典的には `y` 側の多項式 `ω_n` が要る。
★**mathlib はまだ `ω_n` を持たない**(docstring に `TODO`)。
★★そこで **`x` だけの(Kummer)公式**で帰納法を回す道を測り、**両方取れた**。
...
和の式は `linear_combination 2h₁ + 2h₂` で 1 行。
★積の式を直接やると `y` の 4 次になって係数が見つからない。
★★そこで**差**を先に出した:
...
(`ring` で出る)を合わせるだけ。★★★★★**4 次の `linear_combination` を
2 次の `ring` 2 本に分解した**という手である。
...
`x(nP)` の帰納法は Montgomery 梯子で回る:
`x(2mP)` は 2 倍公式(第 199)、`x((2m+1)P)` は積の公式と `x(P)` から。
★あとはこれを mathlib の `preΨ` の EDS 漸化式と突き合わせる。

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-512 ★★★★★★★梯子が消費する形に直した(第 201 ブロック)

第 200 の 2 公式は `addX`・`slope` の言葉で書いてある。
★Montgomery 梯子は**点の加減**(`A + B` と `A − B`)で回るので、その形に直した。
...
### ★mathlib の EDS 漸化式(実測)

`preΨ` には `preΨ_even`・`preΨ_odd` が `n : ℤ` で揃っている:

    preΨ(2m)   = preΨ(m−1)²·preΨ(m)·preΨ(m+2) − preΨ(m−2)·preΨ(m)·preΨ(m+1)²
    preΨ(2m+1) = preΨ(m+2)·preΨ(m)³·(…) − preΨ(m−1)·preΨ(m+1)³·(…)

★これと梯子を突き合わせるのが次の作業である。
...
## §9-513 ★★★★★★梯子と EDS が噂み合うことを `n = 3` で確かめた(第 202 ブロック)
...
★`ring` で通った(2.6 秒)。★★**mathlib の `preΨ₄` の定義が群法則と噂み合っている**
ことの確認でもある——`Φ₃ = X·Ψ₃² − preΨ₄·Ψ₂Sq` の `preΨ₄` がここで効く。

### ★Lean 上の注意(記録)

`ring` は `C 4` と数字 `4` を区別する。★`map_ofNat, C_add, C_sub, C_mul, C_pow` を
simp に入れてから `ring` する(mathlib の `C_simp` マクロと同じ手)。
...
## §9-514 ★★★★★★★★壁の所在を特定した——EDS 恒等式は mathlib 自身の TODO

`x(nP) = Φ_n/ΨSq_n` の一般 `n` を `n → 2n` 側から測った。

### ★★★★★★★★実測(2026-08-20)

| 試したこと | 結果 |
|---|---|
| `n = 4` の段を `b₂ b₄ b₆ b₈` を自由変数にして `ring` | **124 秒かけて失敗**——`b₂b₆ − b₄² = 4b₈` が要る |
| `a₁ … a₆` に展開して `ring` | 既定の heartbeats を超える(28 秒で打ち切り) |

★つまり**一般の `n` を多項式展開で押すのは実際的でない**。
★★`n = 3`(第 202)は 2.6 秒で通ったが、`n = 4` でもう限界である。

### ★★★★★★★★構造的には EDS 恒等式が要る

梯子を一般の `n` で回すには、分点多項式列が**楕円列**であること

    W(m+n)W(m−n)W(r)² = W(m+r)W(m−r)W(n)² − W(n+r)W(n−r)W(m)²

が要る。★★★**これは mathlib 自身の TODO である**——
`Mathlib/NumberTheory/EllipticDivisibilitySequence.lean` の

    * TODO: prove that `normEDS` satisfies `IsEllDivSequence`.

(2026-08-20 実測)。★★★★`IsEllSequence` の定義はあるが、
`normEDS` がそれを満たすことは未証明である。

### ★見積もりの修正

当初 10-25 ブロックと見ていたが、**30-80 ブロック**に上振れする。
★本筋は**上流(mathlib)に `IsEllDivSequence (normEDS …)` を入れる**ことである。
★★それが入れば、梯子の各段は恒等式の特殊化として出る。

### ★実測で確定したこと(両方とも良い知らせ)

1. **`y([n]P)` の式(`ω_n`)は Weil 対には要らない**——消費されるのは `x` の側だけである
   (`Φ_n(X) − x_n·ΨSq_n(X)` がモニックで次数 `n²`、第 198)。
   ★mathlib も `ω_n` を持たないので、これは大きな回避である。
2. **mathlib の `preΨ₄` は群法則と噂み合っている**(第 202 で `n = 3` で確認)。

★義務の数は動いていない(Arakelov 9/9、Galois 4/8)。

## §9-515 ★★★★★★★★行列式の公式が出た(第 203 ブロック)

`det_galRep_eq_cyclotomic` が使うのは、基底 `P, Q` について

    e_n(aP + cQ, bP + dQ) = e_n(P, Q)^{ad − bc}

である。★引き算を避けるため、**両辺に `e_n(P,Q)^{bc}` を掛けた形**で述べた:
...
(= 非退化性)を足せば出る。
★非退化性は第 197 で `F(E)^{E[n]} = [n]^*F(E)` の 1 つに絞れており、
それは `deg[n] = n²`、すなわち EDS 恒等式(**mathlib 自身の TODO**)に帰着している。

★★★つまり **(G5) 全体が単一の葉に帰着した**——
`normEDS` が楕円列であること、その 1 本である。
