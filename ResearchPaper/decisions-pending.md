...
## ★★★★一様な幾何減衰は取れない（定理）—— `JumpGeometricDecay.lean` 418 行 / `sorry` 0（2026-09-09）

**①持ち場の 3 問への答え**:
1. ★「`g = σ^{p^k}`」と「`H`（1 段ごとに `θ` 倍）」は**同値ではなく、`H` の方が弱くもない** ——
   `norm_iterate_pow_sub_self_le`（`AxEpsilonDecay.lean:294`）の**結論**が `f^[n^k]`（＝ `σ^{p^k}`）に
   ついてのもの。⇒ ★本体が期待した「`H` を経由すれば `σ^{p^k}` を避けられる」逃げ道は**無い**。
2. ★証拠 2（跳びの狭義単調）は **1 段だけなら化ける**。★本体の疑い「`<` は幾何的 `θ` 倍より弱いので
   穴かも」は**当たり**だったが、★**本日建てた Hasse–Arf の合同 `p^{m+1} ∣ u(m+1) − u m` が
   「どれだけ伸びるか」を与える**ので `u(m+1) ≥ u m + p^{m+1}`、
   よって `‖w(m+1)‖ ≤ ‖π‖^{p^{m+1}}·‖w m‖`（`norm_step_le_of_jump`）。
3. ★★★**しかし `θ^k` は取れない（本波の主結果、否定的）**。
   `harith` の**上界** `(p−1)u k ≤ p^{k+1}e` と `1 ≤ u 0` と `heM` **だけ**から
   `‖(p:M)‖ · ‖w 0‖^q ≤ ‖w k‖^q`（`norm_pow_ge_of_upper`、`q = p−1`）。
   ⇒ 変位の比の下界 `‖p‖^{1/(p−1)}` は **`k` に依らない**。
   ⇒ `‖w k‖ ≤ θ^k‖w 0‖` を仮定すると `‖p‖ ≤ (θ^k)^{p−1}` で `θ < 1` なら深い `k` で破れる。
   ★これは出口 `TotallyRamifiedLayer.lean:238` が**すでに持っている仮説だけ**から出るので
   仮定を足して逃げられない。★否定したのは「**`k` に依らない一様な `θ`**」だと docstring に明記
   （`k` ごとに違う `θ_k` は測っていないので「無い」とは書いていない）。

★**実装者の前波の見込みの訂正**: 「取れれば `axDecay` に届く」は誤り。★**取れても届かない**。

**②成果**: 抽象核（分岐・付値・Galois の語彙 0）—— `add_le_of_dvd_sub_of_lt`（純 `ℤ`）/
`norm_le_zpow_mul` / `norm_le_pow_mul_of_step` / `norm_pow_ge_of_upper` / `theta_pow_ge` /
`exists_pow_lt_of_lt_one'` / `no_uniform_geometric`。
具体層 —— `norm_step_le_of_jump` / **`no_uniform_geometric_of_harith`**（`harith` の字面への代入）。

**③★木の断定への検査**:
- `AxEpsilonDecay.lean:288-292` の `H : ∀ (m : ℕ) (z : M), …` の ★**`∀ z` は証明で使われていない**
  （同 `:300` は `H m x` としか呼ばない）⇒ `x` の 1 点で足りる版を §7 に置いた（既存は無改変）。
- ★**`H` は等長性からは出ない**（§8、**形式化した反例**）: `f = (z ↦ −z) : ℝ →+ ℝ`, `n = 3`, `x = 1` で
  `‖f^[3^m]x − x‖ = 2`（`m` に依らない）⇒ `θ ≥ 1`。★「反復すれば縮む」は一般には偽。

**★①不分岐側との関係 —— 穴は 3 つ**:
①不分岐（`f = p`）／②一様定数（前波 `const_descent_no_go`）／③幾何減衰（本波）。
★本波の否定は `hvalK`（全分岐）を**仮定した上で**成り立つ（`harith` の (1)(4) と `heM` しか使わない）
ので、`TotallyRamifiedLayer.lean:342` とは別の穴。

**★次の 1 点（実装者の見込み、未測定）**: `AxWildDescentDecay`（`AxEpsilonDecay.lean:605`、`ε` が
増えない形）か、平均化の段で `ε` の伸び `p` を落とす道。★前者は同 `:600-604` が
「一般には成り立たないと見るべき」と書いており、★**その断定はまだ誰も測っていない**
（同 `:604` に「非空虚性は測っていない」と自認がある）。

```
VERDICT[SG-a]: 半分（循環ではないが、H は σ^{p^k} を避ける道にならなかった）
VERDICT[SG-b]: 外れ（「σ^{p^k} 以外に源を求める」逃げ道は無かった）
VERDICT[SG-c]: 半分（1 段は化けたが θ^k は取れない）
VERDICT[SG-d]: 当たり（「少なくとも 1 つ外す」→ SG-b を外した）
★ただし持ち場に書いた疑い「`<` は幾何的 θ 倍より弱いので穴かも」は★当たりだった（実装者が明記）。
COST[JumpGeometricDecay]: 安 | 持ち場=幾何的減衰の源  — 「取れても届かない」を定理化し、穴が 3 つと確定
```


## ★`GUESS:`（配る前に書いた —— `AxWildDescentDecay` の非空虚性）

★木自身（`AxEpsilonDecay.lean:600-604`）が「★`C = p^{1/(p−1)}` を入れると **Ax の定数 `p^{p/(p−1)²}`
より真に良い**ので、★`θ ≤ 1` は一般には成り立たないと見るべきである。★**非空虚性は測っていない**」
と書いている。★これは CLAUDE.md の「数学の字面を先に疑う ②指数・定数の総和を閉じた形で結論と
突き合わせ」がすでに 1 回効いている形。

```
GUESS[WD-a]: 木の警告は正しい。θ ≤ 1 は一般には偽で、反例が作れる（本日の 3 つの否定的結果と同じ形）
GUESS[WD-b]: ただし θ ≤ 1 を弱めた「θ_k が k に依る」形なら通る余地がある。前波が「k ごとに違う θ_k は測っていない」と明記した
GUESS[WD-c]: 平均化の段で ε の伸び p を落とす道の方が安い。トレースの平均化は本連鎖ですでに (p−1)× 損失と測られており、そこが効くかもしれない
...
## ★★★★穴②の射程を訂正 —— 縛りは損失でなく `ε` の伸び —— `BudgetRecast.lean` 243 行 / `sorry` 0（2026-09-09）

**①真偽**:
- ★`exists_mem_of_descent_budget` の字面（`AxTowerDecay.lean:327-335`）—— ★**`c` にかかる条件は
  `hFc : c d ≤ F d` だけ**で、`F` を束ねているのは `hFg : d' < d → g d * F d' ≤ F d`、
  すなわち★**`g`（`ε` の伸び）だけ**。
- ★★★**4 つの穴のうち②の射程を測り直した（実装者による自分の結果の訂正）**:
  - **`c ≡ C`（一様定数の損失）は予算形では許される** —— `g d ≤ 1` なら `F ≡ C` が 3 条件を全部満たす
    （`budget_const_of_growth_le_one`）。⇒ ★**`const_descent_no_go` を「損失 `c` が一様定数では
    届かない」と読むのは誤り**。あれは `g ≡ c` と置いた形にしか当たらない。
  - 逆に**②の内容は `g` に移る** —— `1 < C ≤ g d` が一様なら `C^d ≤ F d`（`pow_le_budget`）で
    `F` は非有界（`not_bddAbove_budget`）。⇒ ★★**縛りは損失ではなく `ε` の伸びである。**
- ★★★**しかし載せ替えても得をしない**。予算形の `hstep`（`c ≡ C`, `g ≡ θ`）は
  `AxWildDescentDecay K C θ` そのもの（`axWildDescentDecay_iff_budgetStep`）なので前波の循環に繋がり
  `budgetStep_iff_axLemma : 1 ≤ C → 0 ≤ θ → θ ≤ 1 → (予算形の hstep ↔ AxLemma K C)`。
  ★極端な場合 **`θ = 0`（`ε` が 1 段で 0 に落ちる）でも同値**（`budgetStep_zero_iff_axLemma`）。
  ⇒ **`g ≤ 1` を狙う限り循環に戻る。**

**②成果**: 抽象核（`ℝ`/`ℕ` だけ）—— **`pow_le_budget`** / `not_bddAbove_budget` /
**`budget_const_of_growth_le_one`**。具体層 —— `axWildDescentDecay_iff_budgetStep` /
**`budgetStep_iff_axLemma`** / `growth_le_max_of_dist` / `budgetStep_zero_iff_axLemma`。

**③★木の断定の検査 2 件**:
- ★**`g d < 1`（深い段で `ε` が減る）を測った** —— `AxTowerDecay.lean:80-84` の理由づけ
  「跡が小さいので `σ^p x − x = Tr(σx − x)` が小さい」は★**別の量**についての主張。
  そこで小さくなるのは `σ^p x − x`（**同じ `x`**、`σ` の**冪**）だが、予算形が要求するのは
  `∀ σ ∈ Γ_K` についての**新しい `x′`** の変位。`Γ_K` は `σ` 自身も含むので冪に限った評価では届かない。
  ★超距離からは `g ≤ max(c, 1)` しか出ない（`growth_le_max_of_dist`）。
  ⇒ ★**`g d < 1` には `c d < 1`（`ε` より真に良い 1 段の近似）が要る。**
- ★**本体が指摘した「跡についての食い違い」→ 食い違っていない** ——
  `NormalizedTraceDescent.lean:262` の「跡は指数を `(p−1)` 倍にする」は**損失 `c`** には悪く、
  `AxTowerDecay.lean:83` の「跡は小さい」は**伸び `g`** には良い。
  ★同じ `(p−1)` 倍が**向きが逆**なので両立する。

**④4 つの穴の予算形の下での現状（本波の再測定）**:

| 穴 | 予算形の下で |
|---|---|
| ①不分岐（`TotallyRamifiedLayer.lean:342`） | 生きている（予算の形と無関係） |
| ②一様定数（`const_descent_no_go`） | ★★**射程を訂正**: `c` ではなく `g` についての主張 |
| ③幾何減衰（`no_uniform_geometric_of_harith`） | 生きている（跳びの変位の比の話） |
| ④測定点（`not_exists_firstJump_data_cyclotomic`） | ★予算形では**当たらない可能性がある**（★未確定） |

**★次の 1 点（実装者の見込み、未測定）**: ④ —— 予算形で `c` を `F` に逃がしたとき
`ℚ₃(ζ₂₇)/ℚ₃(ζ₃)` の破れが消えるか。★消えるなら「反証候補」は記法の問題に還元され、
消えないなら 4 つ目の穴は本物。★もう 1 つは `g d > 1` の段が**有限個**の形（`sup F`）で、未形式化。

```
VERDICT[BD-a]: 未判定（実装者が次の 1 点として残した）
VERDICT[BD-b]: ★当たり（「②③④ は一様な c k を前提にしているかもしれない。本体は測っていない」→ ②の射程が実際に訂正された）
VERDICT[BD-c]: 外れ（③は生きており、④は未確定）
VERDICT[BD-d]: 当たり
COST[BudgetRecast]: 安 | 持ち場=予算関数への載せ替え  — 得をしないと定理化し、②の射程が「損失」から「εの伸び」に訂正された
```


## ★`GUESS:`（配る前に書いた —— ④が予算形で消えるか）

★実装者が本波で出した★**`g d < 1` には `c d < 1` が要る**（`growth_le_max_of_dist`）は、
「1 段で `ε` より真に良い近似」を要求するので、本日の否定群と同じ形の壁かもしれない。

```
GUESS[BR-a]: ④は予算形でも消えない。破れは ‖x − x′‖(距離)の側の測定であって、c を F に逃がしても距離そのものは変わらないから
GUESS[BR-b]: 「g d > 1 の段が有限個(sup F)」の形が本命。深さは有限(wildDepth は自然数)なので、そもそも「段ごと一様」を要求する必要がない
GUESS[BR-c]: c d < 1(1 段で ε より真に良い近似)は、本日の ③幾何減衰 no-go と同じ不等式に当たる
...
## ★★★②′/⑤ の射程は変わらない、逃げ道の必要条件を定理化 —— `DepthLowerBound.lean` 189 行 / `sorry` 0（2026-09-09）

**★実装者が (D) を選んだ理由（費用を先に測った）**:

| 候補 | 測った費用 | 判定 |
|---|---|---|
| (A) `a` の存在 | TFAE の取り出し ＋ `IsIntegrallyClosed`/`IsTorsionFree`/`Module.Finite`。★木の `Found/PGC` に `IsIntegrallyClosed` は**整数環については 0 件** | 重い |
| (B) `wildDepth` の接続 | ★`WildDepthDescent.lean:667` は**`1/p` の平均に固定**（同 `:149` の表が明記）。一般の `a` で書き直すと 1 ファイルぶん | 重い |
| (C) `hform` | 新しい降下の構成そのもの | 重い |
| ★**(D)** | 測定器 1 本（`AxWildDescent` を展開して `ε` で割るだけ） | ★**安い** |

**①測定の結論 —— ②′/⑤ の射程は★変わらない**:
前波の `exists_fixed_step_free` は「不分岐の段は `c = g = 1`（無料）」を示した。★では②′/⑤ は弱まるか
→ ★**弱まらない**。理由は `AxWildDescent K c`（`AxTowerDecay.lean:473`）の**量化の順序** ——
`c` は `x` より**外**にあるので ★**`c d` は深さ `d` の全ての `x` を覆わねばならない**。
⇒ 「ある `x` は無料で降りられる」ことは `c d` を下げない。

**②成果**:
- **`le_c_of_lower_bound`**（測定器）—— 深さ `d ≠ 0` の `x` が 1 つでも「深さが下がるどの `x′` にも
  `D·ε` 以上離れている」なら **`D ≤ c d`**。
- `forall_le_c_of_witness` / **`not_forall_prod_of_witness`** —— 各深さに証人が在れば②′がそのまま効き、
  積は非有界。
- ★★**`escape_needs_no_witness`**（対偶）—— ★**②′ から逃げるには、ある深さ `d ≥ 1` で
  「証人が 1 つも無い」（＝深さ `d` の `x` が**全部**安く降りられる）ことが必要。**

**③★正直に未測定と残したもの**: ★**証人が各深さに在るかは測っていない。**
見込みとしては `ℚ_p(p^{1/p^d})` のような完全分岐の `x` が証人になりそうだが、
★**形式化も下界 `D·ε` の計算もしていない**。⇒ ★★**②′/⑤ が「本当に効く」ことの最後の 1 点はここ**。

**穴の現状**: ①（(b) 閉、(a) 残）/ ②′ ★射程は**変わらない**（★本日 2 度目の射程の確認。
1 度目は `c → g` の訂正）/ ③ 変わらず / ⑤ ★射程は変わらない / ④ 消えたまま。
★★**動いていないことを確かめるのも測定である。**

★本波は①と**独立**（測定器は分岐に一切触れない）。
★**抽象核に切り出していない**（中身が `AxWildDescent` の定義の展開そのもので、切り出すと核が空になる）
ことを docstring に明記。

```
VERDICT[NX-a]: 半分（IsIntegrallyClosed が高いのは当たりだが、木では整数環について 0 件だった）
VERDICT[NX-b]: ★外れ（wildDepth の配管は 1/p の平均に固定されていて、そのままでは使えない）
VERDICT[NX-c]: ★外れ（実装者は (D) を選んだ。本体が挙げていない「測定器」の道）
VERDICT[NX-d]: 当たり
COST[DepthLowerBound]: 安 | 持ち場=選択自由  — ②′/⑤ の射程が変わらないと確定し、逃げ道の必要条件が定理になった
```


## ★`GUESS:`（配る前に書いた —— 証人の存在）

★実装者が特定した「②′/⑤ が本当に効くことの最後の 1 点」。
★本日この鎖は「否定を定理にする」を 6 回成功させており、証人はその 7 回目になる形。

```
GUESS[WI-a]: 証人は ℚ_p(p^{1/p^d}) 型ではなく、本日の連鎖が建てた全分岐の塔(SpectralNormBridge の ..._of_adjoin の設定)から取る方が安い。仮説がすでに揃っているから
GUESS[WI-b]: 下界 D·ε の計算は、本日の JumpGeometricDecay.norm_pow_ge_of_upper(変位の比の下界 ‖p‖^{1/(p−1)} が k に依らない)がそのまま使える
GUESS[WI-c]: 証人が在ることを示せば②′が本物になり、Ax の道は「一様な c」では絶対に通らないことが確定する。これは原典の道が axDecay(k に依る)を使う理由の形式的な説明になる
GUESS[WI-d]: 本体の名指しは直近 19 波で 14 回外している。今回も少なくとも 1 つ外す
```


## ★★★証人は「点ごとの目標」を殺すが「積の形」は殺さない —— `WitnessScope.lean` 184 行 / `sorry` 0（2026-09-09）

**①★本体が「先に測る価値がある」と書いた問い（④と同じ理由で②′が効かないのでは）→ ★そのとおりだった。**

- **(1) 証人は「点ごとの目標」を殺す**（`not_axWildDescent_axDecay_of_witness_at`）——
  深さ `d ≠ 0` に証人（`d(x, 深さ<d) ≥ C·ε`）が **1 つ**在り `axDecay p d < C` なら
  ★**`AxWildDescent K (axDecay p)` は偽**。`C > 1` ならそういう `d` は必ず在る。
- **(2) ★しかし「積の形」は殺さない**（`witness_at_one_depth_not_enough`）——
  ②′ の仮説は ★**`∀ d ≥ 1, C ≤ c d`（`C` は `d` に依らない）**。有限個の深さの下界からは出ない。
  証拠は前波の `cEx` そのもの（深さ 2 では下界を満たすが深さ 3 以降は `1`、それでも積は有界）。

⇒ ★★★**②′が効くには「すべての深さで一様な `C > 1` の証人」が要る。1 つ（有限個）では足りない。
★④が消えたのと同じ構造。**

**②★どこで止まるか**: 証人には `∀ x' : K.closure, wildDepth K x' < d → C·ε ≤ ‖x − x'‖`、すなわち
★**塔の外にある深さ `< d` の元も含めた**下界が要る。
★これは `WildDescentDistanceOnly.lean:88-99` が
「★`M` の外の深さ ≤ 1 の `x′` を排除できていない。Krasner の補題は `‖x−x'‖ < min_σ‖σx−x‖` のときしか
効かず、ここで要求される半径は `min` より**大きい**ので届かない」
「⇒ 確定させるには `ℚ₃(ζ₃)` の 40 個の巡回 3 次拡大で `d(x,N)` を実際に測る必要がある。★測っていない」
と書いた★**その未測定点と同じ形**（本波で突き合わせた。同ファイルは今も未測定）。
⇒ ★★**証人の構成は「塔の中の計算」ではなく「塔の外を排除する」問題**であり、
本日の塔一式では**届かない**。★これが止まる場所。

**③成果**: `not_axWildDescent_axDecay_of_witness_at` / `exists_depth_refuting` /
**`witness_at_one_depth_not_enough`** / `not_axWildDescent_axDecay_of_deep_witness`。
★どれも前波までの自分の定理の合成で、新しい数学は無い。

**穴の現状**: ★★**②′/⑤ は「効く」とも「効かない」とも確定していない。**
確定させるのに要るのは**一様な証人の存在**で、それは塔の外を排除する問題。
①（(b) 閉、(a) 残）/ ③ 変わらず / ④ 消えたまま。

★**証人そのものは構成していない**（理由を docstring に明記）—— 定理にしたのは
「証人が在れば何が言えて、何が言えないか」だけ。§2 は `p = 3` の具体列に依存し、一般の `p` は未対応。

```
VERDICT[WI-a]: ★外れ（本日の塔からは取れない。塔の外を排除する問題だった）
VERDICT[WI-b]: 外れ（下界の材料は使わなかった）
VERDICT[WI-c]: 半分（証人 1 つでは②′は本物にならない。一様な証人が要る）
VERDICT[WI-d]: 当たり
★★本体が「④と同じ理由で効かないかもしれない。あなた自身の 2 定理の関係だ」と書いた点が★2 波連続で
  本波の中心になった。⇒ ★本体の価値は「自分の測定同士を突き合わせよ」と言うことにある。
COST[WitnessScope]: 安 | 持ち場=証人の存在  — 射程が確定し、止まる場所が「塔の外の排除」と特定された
```


## ★`GUESS:`（配る前に書いた —— B2 の 40 個の測定）

★木が `WildDescentDistanceOnly.lean:88-99` で**ずっと未測定**と書き続けている唯一の穴。逐語:
> **B2. `N ∩ M = K` かつ `[N:K] = 3m`(`3∤m`)** ⇒ ★**塞げていない**。…
> 手計算で追うと「`(1/3)Tr` の sharp な評価」と「`d(x,N₂) = d(x,K)`」の 2 本からは
> `−1 ≤ v(x−y₀) ≤ 3` しか出ず、★**ちょうど矛盾に届かない**。
> ⇒ 確定させるには `K = ℚ₃(ζ₃)` の 40 個の巡回 3 次拡大(Kummer)の中で `d(x,N)` を実際に測る必要がある。

```
GUESS[B2-a]: 40 個は Kummer 理論で K^×/(K^×)^3 の位数から出る(K = ℚ₃(ζ₃) で |K^×/(K^×)^3| = 81、非自明な部分群が 40 個)。列挙は python で書ける
GUESS[B2-b]: 測るのは各 N での d(x,N) の付値で、x は既に固定されている(ζ₂₇ 由来)。厳密整数演算で閉じる
GUESS[B2-c]: 結果は「どの N でも矛盾する」= B2 が塞がる、になる。木が「ちょうど届かない」と書いているのは手計算の粗さで、実際に測れば埋まる
GUESS[B2-d]: 本体の名指しは直近 20 波で 15 回外している。今回も少なくとも 1 つ外す
```


## ★★★B2 は Krasner では原理的に塞げない（枝を 1 本刈った）—— `KrasnerCeiling.lean` 188 行 / `sorry` 0（2026-09-09）

**★数値計算に進まなかった理由（費用を先に 3 点測った）**:
1. ★★**`x` が木に無い。** `WildDescentDistanceOnly.lean:57` は「厳密な整数演算で全部当たった
   （★**スクリプトは報告に添付**）」と書いているが、★`x` もスクリプトも**リポジトリに入っていない**
   （`grep -rn "37/25000|zeta27" tools/` → 0 件、`ls tools/*.py` → 3 本のみ）。
   ⇒ 乱択探索からやり直しになる。★★**再現性の穴。**
2. `d(x,N)` は `N ⊄ M` なので**合成体 `MN`（`ℚ₃` 上 54 次）**の算術が要る。
3. ★そして測定 1 のとおり、★**その Galois の下界では原理的に届かない**。

**①測定 1 —— ★★Krasner の評価は B2 に原理的に届かない**:
`not_exists_fixed_close`（抽象核、分岐語彙 0）: `δ < ‖σ·x − x‖` なら
「`σ` で固定され `x` から `δ` 以内」の `x′` は**無い**。★これが「塔の外を排除する」唯一の一般的な道具。
★しかし B2 の数値では 要求半径 `v = 8`、`Gal(M/K)` の変位の**最小の `v` は 11`。
★`M ∩ N = K` なので `Gal(\bar K/N)` の元は `M` 上 `Gal(M/K)` に落ち、★**`N` に依らず同じ変位**。
⇒ `b2_obstruction_not_triggered`: **`‖π‖^{11} ≤ ‖π‖^{8}`** で道具が**発火しない**。
★★**40 個のどの `N` でも同じ。** ⇒ **B2 は Krasner/Galois では塞げない。別の道具が要る。**
★木の地の文を**定理にした**。★**1 か所訂正**: 木は `v_max = 15` を使っているが、
★`σ` は選べるので**最も大きい変位＝最小の `v` = 11** を使うのが最良（結論は変わらない）。

**②測定 2 —— ★★B2 の帰結は `AxLemma` を左右しない**: `b2_does_not_decide_axLemma` ——
`cEx` は深さ 2 で `axDecay 3 2` を**破っている**が、それでも
`AxWildDescent K cEx → AxLemma K (axConstant 3)`。
⇒ ★★★**B2 が塞がっても塞がらなくても `AxLemma` の可否は変わらない。** 前波の `WitnessScope` と同じ構造。

**③在庫**: `IsUltrametricDist.norm_sub_le_max` → ★**`Unknown constant`（本波で実測）**。
⇒ `WildDescentDistanceOnly.lean:104` の「`norm_sub_le_max` は無い、`to_additive` の穴」は**真**。
直しは `sub_eq_add_neg` + `norm_add_le_max` + `norm_neg`。

**★条件付きであることを明記した点**: 数値 `8` / `11` は `WildDescentDistanceOnly.lean:65` の測定値を
**そのまま引いており、再計算していない**（`x` が木に無いため）。
⇒ **測定 1 の結論は「木の数値が正しければ」という条件付き**。docstring にも記載。

**穴の現状（増えていない）**: ①(b) 閉 /(a) 残 / ②′ 一様な証人が要る / ③ 変わらず / ⑤ ②′と同じ /
④ 消えたまま。★★本波は**B2 という未測定の枝を 1 本刈った**。

```
VERDICT[B2-a]: 未判定（列挙には進まなかった）
VERDICT[B2-b]: ★外れ（x は木に無い。「既に固定されている」は誤り）
VERDICT[B2-c]: ★外れ（B2 は塞がらない。しかも塞がっても AxLemma は変わらない）
VERDICT[B2-d]: 当たり
COST[KrasnerCeiling]: 安 | 持ち場=B2 の測定  — 数値に進まず、Krasner が原理的に届かないことと B2 が AxLemma を左右しないことを定理化
```

## ★★本日見つかった再現性の穴（人の判断が要る候補）

`WildDescentDistanceOnly.lean:57` が「厳密な整数演算で全部当たった（スクリプトは報告に添付）」と
書いている `x`（ℚ₃(ζ₂₇) の元）と、その計算スクリプトが★**リポジトリに入っていない**。
⇒ 同ファイルの数値（`d(x,K)=3` / `d(x,F)=7` / `v_min=11` / `e_L=18` など）は
★**現在のリポジトリからは再現できない**。本日この鎖は同じ数値を 5 回引用した。
★対処案: (i) `x` とスクリプトを `tools/` に入れる (ii) 数値を Lean の `example` として固定する
(iii) 再導出する。★どれも本体の判断ではなく人の判断が要る。


## ★本体が測った本日の規模（2026-09-08 15:00 以降、`git log` 実測）

- commit **163 本**、`Found/PGC` の新規 `.lean` **58 本**（現在 `Found/PGC` は **295 ファイル**）
- `decisions-pending.md` は **16,185 行**
- ★`Found/` の `sorry` は**通して 0**（外部依存 `PrimeNumberTheoremAnd/Wiener.lean` の 2 件のみ）

## ★`GUESS:`（配る前に書いた —— 次の 1 波）

```
GUESS[ST-a]: 棚卸しが最も価値がある。本日 58 本のファイルが増え、穴が①②′③⑤の 4 つに整理されたが、その全体像は台帳 16,185 行に散っていて誰も通読できない
GUESS[ST-b]: 実装者は棚卸しではなく (A) の a の存在を選ぶ（未着手の中で唯一 mathlib に道具が在ると測れているから）
GUESS[ST-c]: hform(C) は「新しい降下の機構」なので 1 波では入らない。実装者は 2 波連続でそう判定している
GUESS[ST-d]: 本体の名指しは直近 21 波で 16 回外している。今回も少なくとも 1 つ外す
```


## ★★★★再現性の穴を塞いだ —— `CyclotomicNumbersVerified.lean` ＋ `tools/zeta27-*.py` 2 本（2026-09-09）

**★実装者が (R) を選んだ理由**: 本日この鎖は `WildDescentDistanceOnly.lean:60-66` の数値を**5 回引用**したが
同 `:57` は「スクリプトは報告に添付」と書いており `x` もスクリプトも**リポジトリに無い**
⇒ 引用の土台が再現できず、間違っていれば本日の測定のいくつかが崩れる。
(S) 棚卸しは各ファイルの docstring が既に `file:line` を持ち重複が大きい。
(A)/(B)/(C) は 2〜3 波前に「1 波では入らない」と測った。

**①★★★結果 —— 8 つの数値がすべて一致**（丸めゼロ、任意精度整数のみ）:
設定 `π = ζ₂₇ − 1`、`E(π) = Φ₂₇(1+π)` は Eisenstein（機械確認 `v₃(E₀)=1`）、`𝒪_M = ℤ₃[π]`、
`v_M(Σbᵢπⁱ) = minᵢ(18·v₃(bᵢ)+i)`。

| 木の字面 | 再導出 |
|---|---|
| `e_M = 18` / `G_0 = G`(18) | ✓ |
| `G_1 = G_2 = H`(9) | `a ≡ 1 mod 3` ＝ `Gal(M/ℚ₃(ζ₃))` ✓ |
| `G_3..G_8 = Gal(M/ℚ₃(ζ₉))` | `{1,10,19}` ✓ |
| `G_9 = 1` / `σ` は `i=3` / `τ` は `i=9` / 層の跳び `i=8` | ✓ |
| `x = π` で `v(σx−x)=3, v(τx−x)=9` | ✓ |
| ★実在する `x` で `v = (11,15)` | ★**見つかった** |
| ★`d(x,ℚ₃(ζ₉)) = 7` / `d(x,ℚ₃(ζ₃)) = 3` | ★**7 / 3** |
| 出現率 `37/25000` 等 | `21/20000`（同じ桁）✓ |

★★**失われていた `x` を回復**（π-basis、低次から）:
`x = [71,78,9,50,30,54,67,43,40,3,25,11,40,44,74,63,38,24]`
⇒ ★★★**木の数値は全部正しかった。本日の測定は崩れない。**
今後は `tools/zeta27-ramification-check.py` / `tools/zeta27-distance-check.py` で誰でも再現できる
（`random.seed(20260909)` 固定）。

**②★実装者が自分のバグを記録**: 1 度目の実装で `(1+π)^9 − 1` を「`ζ₉ − 1`」だと思い込み
基底が退化して `AssertionError: singular`。★正しくは `(1+π)^9 − 1 = ζ₃ − 1`（`v_M = 9`）で、
`ζ₉ − 1` は `(1+π)^3 − 1`（`v_M = 3`）。★**機械が止めてくれたので気づいた**。

**③Lean に固定したもの**（`p` 進の付値は塔を組まないと載らないので**群の側と算術の側だけ**を
`decide`/`norm_num` で）: `sigma_order_nine` / `tau_order_three` /
**`sigma_cubed_eq_tau`（`4³ ≡ 10 mod 27`）** / **`layer_step_fails`（`7 < 8`）** /
**`composite_has_slack`（`11−12 ≤ 3`）** / **`krasner_does_not_fire`（`8 ≤ 11`）** ほか計 10 本。

**穴の現状（変化なし）**: ①(b) 閉 /(a) 残 / ②′ / ③ / ⑤ / ④ 消えたまま。
★本波は**穴ではなく「土台の検算」を 1 つ潰した**。

```
VERDICT[ST-a]: 外れ（棚卸しより土台の検算が先だった。実装者の理由づけの方が正しい）
VERDICT[ST-b]: 外れ（(A) ではなく (R) を選んだ）
VERDICT[ST-c]: 未判定
VERDICT[ST-d]: 当たり
COST[CyclotomicNumbersVerified]: 安 | 持ち場=選択自由  — 5 回引用した数値の土台が機械で確定し、失われた x が回復した
```


## ★`GUESS:`（配る前に書いた —— 次の 1 波）

★本体が測った事実: 本波で `tools/zeta27-*.py` 2 本（476 行）が入り、
`WildDescentDistanceOnly.lean` の 8 数値が再現可能になった。
★同じ形の「再現できない数値」が木の他の場所にもあるかは**未測定**。

```
GUESS[NW-a]: 同じ形の穴が他にもある。木は「厳密整数演算で確認した」と書く箇所が複数あり、スクリプトが入っているのは今回が初めてのはず
GUESS[NW-b]: 実装者は棚卸し(S)を選ぶ。土台の検算が終わったので次は全体像
GUESS[NW-c]: (A) の a の存在は、本波で Python の道具立てができたので、剰余体の計算を機械でやれば見通しが良くなるかもしれない
...
## ★★★★射影の欠損 `γ = p−1` が 4 層で一致、掃討 5/6 —— `ProjectionDefectVerified.lean` ＋ `tools/trace-defect-check.py`（2026-09-09）

**★実装者が選んだ理由**: `DeepDescentPairDirect` の「95,298 件」は**乱択で seed が無い** ⇒
★**同じ件数は原理的に再現できない**（★本体の観察 1 が当たり）。
`EquivariantProjectionDescent` の中心は **`γ = p−1`** で★**有限個の基底元の跡**を計算するだけ ⇒ 最も安い。

**①★★★結果 —— 4 層すべてで `γ = p−1` が一致**:

| 層 | `e_M` | `v_M(Tr(π^i))` | ★`γ` | 木 |
|---|---|---|---|---|
| `ℚ₃(ζ₂₇)/ℚ₃(ζ₉)` | 18 | 全部 18 | **2** | `2` ✓ |
| `ℚ₃(ζ₈₁)/ℚ₃(ζ₂₇)` | 54 | 全部 54 | **2** | `2` ✓ |
| `ℚ₂(ζ₈)/ℚ₂(ζ₄)` | 4 | 全部 4 | **1** | `1` ✓ |
| `ℚ₂(ζ₁₆)/ℚ₂(ζ₈)` | 8 | 全部 8 | **1** | `1` ✓ |

同 `:23-25` の `t = 8` / `s = 6` / `m = 2`、同 `:35-42` の合成の表（素朴 `14` 超過 / 収縮つき `12` ちょうど /
★射影つき `10` 入る）も**全部再導出**。

**②★★★実装者が自分のバグを記録（本波の第 2 の成果）**:
1 度目の実装は `γ = e_M − min_i v_M(Tr π^i)` と書いて **`γ = 0`** を出した。
★**作用素ノルムは「比の sup」であって「`v` の min」ではない**。正しくは
`γ = max_{i<p} ( i + e_M − v_M(Tr π^i) )` で、`v_M(Tr π^i) = e_M` が全 `i < p` で成り立つので `γ = p−1`。
★★**木の `γ = 2` の方が正しく、実装者が誤っていた。**

★★**古典的な裏付けも独立に取った**（Serre, Corps Locaux V §3 Lemme 4）:
`Tr(𝒪_M) = 𝔭_E^{⌊d/e⌋}`、`d = (p−1)(i+1) = 18`、`⌊18/3⌋ = 6` ⇒ `v_M = 18 = e_M` ⇒ `γ = p−1`。
★**機械の数値と古典の公式が一致**（Lean に `different_exponent_three` / `trace_image_valuation` で固定）。

**③掃討の現状 —— 6 箇所中 5 箇所**: `WildDescentDistanceOnly` / `GainedTowerDescent`（跳びのみ）/
`DeepDescentRepair` / `JumpDefectTradeoff` / ★**`EquivariantProjectionDescent`（本波）** が済。
残るのは `DeepDescentPairDirect.lean:37,51,147`（★**seed が無いので件数は原理的に再現不能**。
結論のみ再現可能）。

**④★数値が合わない箇所は 4 波連続で見つからない**。★ただし★**1 度目の実装者の計算は合わなかった**
（`γ = 0` 対 `γ = 2`）。★**誤っていたのは実装者の方**であり、木の記録は 4 波連続で正しい。

```
VERDICT[SW2-a]: ★当たり（seed が無いので件数は再現できない）
VERDICT[SW2-b]: 半分（式で閉じてはいなかったが、有限個の基底元の跡で済む最安の道だった）
VERDICT[SW2-c]: 当たり（本波の範囲では一致）
VERDICT[SW2-d]: 当たり
COST[ProjectionDefectVerified]: 安 | 持ち場=掃討の残り  — γ = p−1 が 4 層で一致し、実装者自身の計算バグも記録された
```


## ★`GUESS:`（配る前に書いた —— 掃討の最後の 1 箇所と棚卸し）

```
GUESS[LA-a]: DeepDescentPairDirect の結論だけ再現するのは中くらいの費用で、slack の実装が要る。実装者は「再現しない」と決めた記録を残す方を選ぶかもしれない
GUESS[LA-b]: 掃討が終われば棚卸し(S)が自然。本日 61 本のファイルが増え、穴は①②′③⑤の 4 つに整理された
GUESS[LA-c]: 棚卸しは Lean ファイルではなく decisions-pending.md か新しい md にする方が実装者には自然かもしれない(本体は Lean を提案してきた)
...
## ★★★`hform` は測定点で偽、そして実装者が自分の誇張を訂正 —— `FirstJumpNotAchieved.lean` ＋ `tools/firstjump-hform-check.py`（2026-09-09）

**①`NormalizedTraceDescent.lean:65-66` の表を★初めて再現**（厳密整数演算、3 行とも一致）:

| 測り方 | 跳び | 指数 | 木 |
|---|---|---|---|
| 跡（上の層） | `i = 8` | `16/18` | `8/9` ✓ |
| sharp（上の層） | `i = 8` | `8/18` | `4/9` ✓ |
| sharp（第 1 跳び） | `i₁ = 2` | `2/18` | `1/9` ✓ 予算 `3/18` に**収まる** |

おまけで ★**Herbrand の同定 `i₁ = j = 2`** も機械で確認（`L` の第 1 跳びと `E₁/F` の跳びが一致）。

**★★しかし「予算に収まる」と「降下が達成する」は別だった。**
回復した `x`（`ε = 11`, `d(x,E₁) = 7`）で —— 第 1 跳び `3^{2/18}` は要求 `v ≥ 9` / 実際 `7` で**達成しない**、
`axDecay 3 2` も要求 `v ≥ 8` / 実際 `7` で**達成しない**、上の層 `3^{8/18}` だけ達成する。
実際に達成できる指数は `11−7 = 4` で、第 1 跳びの上界 `2` を **2 だけ**超える。
★caveat: それでも**対の予算 `12` には必要量 `8` で入る**（slack `+4`）ので
★**`FirstJumpRoute` という道は閉じるが `AxLemma` の可否は決めない**（④が積で消えたのと同じ構造）。

**②★★★在庫の測定 —— 実装者が自分の誇張を訂正**:
「否定を新しく立てた」は**誇張**で、★**木に既に 2 本あった** ——
`WildDescentMultiStep.lean:602 Zeta81.firstJump_bound_false`（`k=3`、ずれ `4/54`）、
同 `:682 Zeta27.firstJump_bound_false`（`k=2`、ずれ `1/18`）。
本波が足したのは (i) **2 つ目の測定点**（ずれが指数で `2` と**より大きい**）(ii) 表の初再現
(iii) Herbrand 同定の機械確認 の 3 つだけ。★訂正を自分のファイルの docstring に書いた。

★**索引は「無い」と嘘をついた** —— `grep -n "firstJump_bound_false" .cache/decl-index.txt` は **0 件**、
実ファイルは **2 件**。⇒ ★**#348 の衝突検査は索引ではなく実ファイルで行うのが正しい**（#348 に追記）。

**③★本体が検証した実装者の配管の指摘**: 「`WildDescentMultiStep.lean:602/:682` が同じ短名で
両方 `.src` を持つので NG 13 件のうち 1 件かもしれない」→ ★**偽。NG 13 件はすべて `CorrHyp`。**
理由: 2 つの `.src` が**同じファイル内**にあるので `src.indexOf` が 1 つ目を見つけて解析に成功する。
★★**ただし潜在的な弱点は実在** —— `check.mjs:1643-1645` は**ファイル内の最初の `.src` しか読まない**ので、
2 つ目の宣言は 1 つ目の locator で検証される。★本件は 2 つの `.src` が**内容も同一**
（どちらも `pGC / 6 / Corollary 3.1 / cor-3-1`）なので実害は無い。★**内容が違えば黙って通る。**

**④残り**: ①(a) / ②′ / ③ / ⑤（★本波で**強まった** —— `FirstJumpRoute` の第 1 跳びの道も測定点で破れる）。
★**次に測るべき 1 点**は「この破れが `x` 固有か、一般に起きるか」（★本波は 1 点しか測っていない）。

```
COST[FirstJumpNotAchieved]: 安 | 持ち場=hform  — 測定点で偽と判明し、表が初めて再現され、実装者が自分の誇張を訂正した
```


## ★★★★★損失は目盛りで測ると深さに依らない —— `HformPopulation.lean` 定理 17 / `sorry` 0（2026-09-09）

**①問いは二択だったが、答えはどちらでもなかった**:
- `hform`（損失 ≤ `i₁` = 2 目盛り）は **99.69%（k=2）/ 99.73%（k=3）** で成り立ち、
  ★**生成的には等号**（66% が `(eps,d) = (3,1)`）⇒ **一般には破れない**
- それでも破れは **再現する**（0.31%）⇒ ★**前波の `x` は例外ではない**
- 最大損失は **ちょうど 4 目盛り**。乱択 5 法 ×20,000 ＋ 山登り 200×200（計 14 万評価）で
  4 を超える `x` は **1 つも出ない**

**★★★最大の発見 —— `k=2`（`e=18`）と `k=3`（`e=54`）で損失の分布がほぼ同一**:
...
支持集合が同じ、最大が同じ、★**最悪点まで同じ `(eps,d) = (11,7)`**。
⇒ ★★**損失は層の付値の「目盛り」で測ると深さに依らない。**

**②★抽象核が測定の説明になっている**（分岐・付値・Galois の語が 1 語も出ない）:
```lean
axDecay_eq_rpow_div_totient : axDecay p k = p ^ (p / (p^k * (p−1)))
```
★`p^k(p−1) = φ(p^{k+1}) = e_L` なので **`axDecay p k` は層の付値でちょうど `p` 目盛り**であり `k` に依らない。
あわせて `rpow_div_lt_rpow_div`（定数比較 → 整数比較）、
`rpow_units_pow`（目盛りの付け替え `(b^{a/e})^{c/a} = b^{c/e}`）。

⇒ 目盛りで測った 3 つの数は**すべて `k` に依らない**: `i₁` = **2**、`axDecay` = **3**、
測った鋭い定数 = **4**。★**`hform` は 2 目盛り不足、`axDecay` は 1 目盛り不足。**
⇒ ★**`AxWildDescent K axDecay` はこの塔で偽**（稀だが再現、両深さで）。
一方 総積は `axConstant 3^{4/3} = 3` で閉じ `axConstant 3 = 3^{3/4} < 3` ⇒
★★**`AxLemma K 3` は満たすが `AxLemma K (axConstant 3)` は満たさない。`AxLemma` 自体は死なない。**

**③★木の断定への訂正（他所は直さず自分の docstring に記録）**:
`WildDescentMultiStep.lean` の `cancel_cost_eq_axDecay_two` の docstring
「測った損失は `axDecay 3 2` ちょうど ⇒ `axDecay p 2` は**下げられない**（等号が実現している）」は
**正しいが、★「上げねばならない」ことが書かれていない** —— 損失 **4 目盛り**の `x` が
20,000 件中 **22 件**（`k=3` でも 18 件）。★「等号が実現している」は「`axDecay` が最大値」という誤読を招く。
★★**木のデータは正しく、足りなかったのは解釈だった**（最悪点 `(11,7)` は既に木が持っていた `x`）。

**④在庫の相互検証（2 か所）**: 抽象核は木の既存 2 定理と一致 ——
`cancel_cost_eq_axDecay_two : 3^{3/18} = axDecay 3 2`（18 分の 3 目盛り）、
`axDecay_three_three_eq : axDecay 3 3 = 3^{1/18}`（54 分の 3 目盛り）。★どちらも「分子 3」で一致。
宣言名 17 件は **実ファイル**で衝突 0 を事前確認（#348）。

**⑤穴の更新**: ②′ ★**測れた**（定数 4 は深さに依らない）／③ ★**支持された**（鋭い定数も公比 1/3 の等比列）／
⑤ ★**確定**（`axDecay` は 1 目盛り不足）／① 変化なし。

**★次に測るべき 1 点（実装者）**: 「最大 4 目盛り」は**測定であって証明ではない**。
`loss ≤ 4`（せめて `loss ≤ 2·i₁` の形）を**証明**できるかが次の 1 点。
★測ったのは `p = 3` の 2 深さのみ、他の `p` は未測定。

新規: `tools/hform-population-check.py`（4 モード）、`tools/hform-k3-check.py`（一般 `(p,n)`）。
どちらも高速版が厳密版と一致することを `selftest` で確認済み。

```
COST[HformPopulation]: 安 | 持ち場=破れは x 固有か一般か  — どちらでもなく「目盛りで測ると深さに依らない」が答え。抽象核が測定を説明した
```


## ★★★★★損失の法則は `i₁ + 2 = p + 1`、深さ 1 は安全 —— `LossUnitLaw.lean` 定理 21 / `sorry` 0（2026-09-09）

**①配られた形は★偽**: 「せめて `loss ≤ 2·i₁`」は **`p = 2` で偽** ——
`i₁ = 1` で `2·i₁ = 2` だが **損失 3 が 20,000 件中 2,817 件（14.1%）**。
★`p = 3` では `2·i₁ = 4` ＝ 実測の最大なので**たまたま成り立ち**、`p = 3` だけでは 2 形が区別できなかった。
⇒ ★**正しい形の候補は `loss ≤ i₁ + 2 = (p−1) + 2 = p + 1`**（`p=2` で `3`、`p=3` で `4`）。
★本体の問い「`4 = 2·i₁ = 2(p−1)` は偶然か必然か」も決着 —— `p = 2` では `max = 3` で**どちらでもない**。
効いているのは ★**`i₁ + 2` ＝ `axDecay` の目盛り + 1**。

**②測定**（`tools/hform-general-p-check.py`、新規）:

| p | n | e_L | i₁=p−1 | axDecay | 生成的 | **最大** | hform率 |
|---|---|---|---|---|---|---|---|
| 2 | 2 | 2 | 1 | 2 | 1 | **1** | 100% |
| 2 | 3/4/5 | 4/8/16 | 1 | 2 | 1 | **3** | 85.9〜86.1% |
| 3 | 2 | 6 | 2 | 3 | 2 | **2** | 100% |
| 3 | 3/4 | 18/54 | 2 | 3 | 2 | **4** | 99.69/99.73% |
| 5 | 2 | 20 | 4 | 5 | 4 | **4** | 100% |
| 5 | 3 | 100 | 4 | 5 | 4 | **5**(未確定) | 99.92% |

★**新しく分かった 3 つ**:
1. **生成的な損失は常に `i₁ = p−1`**（3 素数すべて）⇒ `hform` は生成的に等号
2. ★★**深さ 1 は安全** —— `p=2,3,5` すべてで最大 `= p−1 < p =` axDecay、**hform 率 100%**。
   ⇒ ★**破れは深さ ≥ 2 の現象**（1 段の古典的最良定数 `p^{1/(p−1)}` は問題なく、困るのは塔）
3. **破れの頻度は `p` が大きいほど下がる**（14% → 0.31% → 0.08%）⇒ ★**`p = 2` が最悪の場合**

★**正直な未確定**: `p = 5, n = 3` は損失 **5** までしか観測できていない（偏り付き乱択 2,500 件中 2 件）。
予言 `p+1 = 6` は未確認。⇒ `p = 5` は「`hform` は破れる」までしか言えない。
`max = p+1` は **`p = 2, 3` でのみ**確認済み。

**③成果**: 抽象核（分岐・付値・Galois の語 0）—— `unit_step_lt` / `axDecay_add_one_unit` /
★`axDecay_lt_sharp_general`（**`axDecay < sharp` が `p, k` に一様**）/
★★`hform_bound_lt_axDecay`（**`hform` の要求は `axDecay` より真に強い** ——
測定の頻度差 14% 対 0.11% を説明する）/
★`axConstant_add_one_unit`（総定数も 1 目盛りずれるだけ: `sharpConstant p = p^{(p+1)/(p−1)²}`、
`p=2` → **8**、`p=3` → **3**。★`p=3` の `3` は前波の `axConstant 3^{4/3} = 3` と**独立に一致**）。
具体層: 反証 6 件 ＋ 深さ 6 件。

**④★★証明できなかったこと（実装者が区別して書いた）**: **`loss ≤ p+1` そのものは証明していない。**
必要な部品を測って名指し ——
1. `𝒪_L = 𝒪_{E₁}[π]`（**相対**の冪基底）—— mathlib は 0 件だが★**木には在る**
   （`TotallyRamified.lean` / `LubinTateTotallyRamified.lean`）。★「mathlib に無い」≠「木に無い」
2. `v_L(σπ − π) = p` の一般形 —— 木には**特定の円分体の数値**としてしか無い
3. ★★**本質**: `σx − x = (σf₀ − f₀) + f₁(σπ − π) + …` の**打ち消しの深さの上界**。
   測定は「打ち消しは 2 目盛りで止まる」と言うが★**なぜ止まるのかは分かっていない**。★ここが本当の 1 点。

**★次に測るべき 1 点（実装者）**: `p = 5, n = 3` で損失 6 が出るか（10 万件で 33 分）。
★あるいは `p = 3` の 22 個の最悪点から**打ち消しの構造を読む**方が安いはず。

```
★★本体の提案「p = 2 と p = 5 で測れば 1 回で決まる」が★当たり —— 2 つの形が区別され、
  「4 = 2·i₁」が偶然だったことも確定した。
COST[LossUnitLaw]: 安 | 持ち場=loss ≤ 4 の証明  — 形は i₁+2 と判明、深さ 1 は安全、p=2 が最悪。証明自体は未了
```


## ★★★★打ち消しの構造を読み、未解決点を 1 個の証人に縮めた —— `FirstJumpWitness.lean` 定理 14 / `sorry` 0（2026-09-09）

**①読めた構造**（`tools/cancel-structure-check.py`、新規）。`L = ℚ₃(ζ₂₇)` で
`𝒪_{E₁}` 係数の展開 `x = f₀+f₁π+f₂π²`、`σx−x = A₀+A₁π+A₂π²` を厳密に:

| 損失 | `v(f_i)` | `d` | `eps` | 生き残る成分 |
|---|---|---|---|---|
| 2（生成的） | `[0,0,0]` | 1 | 3 | `A₀` が `d+2` |
| 3 | `[*,6,6]` | 7 | 10 | `A₁` が `d+3` |
| **4**（最大） | `[*,6,6]` | 7 | 11 | `A₂` が `d+4` |

★**生成的な予測は常に `v(f₁) + i(σ)`（`σ` は第 1 跳びの元）で、実測はそれより 0/1/2 目盛り深いだけ。
3 以上は 1 件も出ない。** `p = 2` でも同じ。

**②★測った法則（本波の主結果）**: `min_{σ : i(σ)=i₁+1} v_L(σx − x) ≤ d(x,E₁) + i₁ + 2`

| p | n | i₁ | 試行 | `i₁+2` 超え | 最大 |
|---|---|---|---|---|---|
| 3 | 3/4 | 2 | 8,000/2,000 | **0/0** | 4 |
| 2 | 4/5 | 1 | 8,000/4,000 | **0/0** | 3 |
| 5 | 3 | 4 | 400 | **0** | 5 |

★**合計 22,400 件で 1 件も破れず**。`eps` はこの min 以下なので `loss ≤ i₁+2 = p+1` が出る。
⇒ ★★**未解決点が「第 1 跳びの元 1 個」に縮んだ。**

**③★実装者が自分で立てて自分で潰した仮説**: 最悪点で生き残るのが `A₂`（最上位成分）だったので
「最上位成分だけで上界が出る」と考えて測った → ★**偽**。`p=3,n=3` で 8,000 件中 **3,480 件（43.5%）**が
`p+1` を超え、最大 **58**。⇒ 上界は**成分の min** から来る。最悪点で `A₂` が生き残ったのは点固有の事情。

**④★★深さ 1 が安全な理由が分かった（前波の未説明を 1 つ埋めた）**:
`H_E = {a ≡ 1 mod p^{n−1}}`、`H_K = {a ≡ 1 mod p}` なので ★**`n = 2` では `H_E = H_K`**。
変位を測る群と降下の群が一致し「**第 1 跳びの外の元**」が存在しないため、
打ち消しの機構そのものが起きない（測定の hform 率 100% の正体）。
`n ≥ 3` で初めて `p < p^{n−1}` となり両者が分かれる。

**⑤成果**: 抽象核（分岐・付値・Galois の語 0）——
`loss_le_of_single_witness`（★**証人 1 個で足りる**）/
★★`norm_smul_sub_self_eq_of_lt`（`x = x′ + y` と分け `‖σx′−x′‖ < ‖σy−y‖` なら `‖σx−x‖ = ‖σy−y‖`。
⇒ **上界の問題が「尾 `y` の変位」だけの問題に落ちる**）/ `depth_one_groups_coincide` ほか計 14 本。

★★**証明できなかったのは「尾の変位の打ち消しが 2 目盛りで止まる」こと**。22,400 件の測定でしか
支持されていない。★**本波が縮めたのは問題の大きさであって、問題そのものは開いたまま。**

**⑥在庫の測定 —— ★本体の提案の訂正**:
★本体が「`exists_coeff_norm_le` がまさにその展開に見える」と渡した 2 本を実装者が**読んだ**結果、
★★**向きが逆だった** —— `norm_sub_apply_le_mul` / `exists_coeff_norm_le` は `v(σz−z) ≥ v(z)+t`、
すなわち **`eps` の下界**（`loss ≥ t`）を与える。本波が要るのは **`eps` の上界**。
★ただし**基底展開そのもの**は本波の分解と同じ形で、将来「尾の変位」を評価するときに使えるはず。
★`IsUltrametricDist.norm_add_eq_max_of_norm_ne_norm` は索引 **0 件**だが**実在**
（`ConcreteNormedModel.lean:394` ほかが使用）。★**索引の「無い」が嘘をつく 8 例目**。

**★次に測るべき 1 点（実装者）**: **尾 `y = f₁π + … + f_{p−1}π^{p−1}` だけで測る**（`f₀ = 0` にする）。
`f₀` の項が打ち消しの原因なら尾だけでは打ち消しが起きないはずで、それが確かめられれば
`norm_smul_sub_self_eq_of_lt` の仮説がいつ成り立つかが決まる。

```
VERDICT[本体の「exists_coeff_norm_le がその展開に見える」]: 半分（展開の形は当たり、不等号の向きは外れ）
COST[FirstJumpWitness]: 安 | 持ち場=打ち消しがなぜ止まるか  — 問題が 1 個の証人に縮み、深さ 1 が安全な理由も判明
```


## ★★★★★尾は分散ゼロ、`hform` を初めて導き、打ち消しの必要条件を証明 —— `TailNoCancel.lean` 定理 10 / `sorry` 0（2026-09-09）
...
同じ乱択で `f₀` を入れた対照は `−55` 〜 `+4` に広く散らばる。
⇒ ★★**打ち消しの原因は `f₀`（近似点そのものの変位）だと確定。** 尾 `y` では打ち消しは一度も起きない。
（おまけに `f₀ = 0` のとき `d(x,E₁) = v_L(x)` も 16,500/16,500 で確認。）

**②★★`hform` を初めて「導いた」（反証ではなく）** —— `dist_le_of_no_cancellation`:
> `‖σ•x′ − x′‖ < ‖σ•(x−x′) − (x−x′)‖`（打ち消しが起きない）かつ
> `‖σ•(x−x′) − (x−x′)‖ = ‖x−x′‖/c`（尾の法則）ならば **`‖x − x′‖ ≤ c·Δ`**

★これは `AxWildDescent`/`hform` の**結論そのもの**。前波までは `hform` を**反証**しかしていなかった。
仮説「打ち消しが起きない」は測定で 99.7%（p=3）/ 86%（p=2）、★**`f₀ = 0` なら 100%**。

**③★★打ち消しの必要条件を証明した（測定ではない）**:
`σf₀ − f₀ ∈ E₁` なので `v_L ∈ p·ℤ`、尾は `d + (p−1)`。等しくなる必要があるので
★**`p ∣ d + (p−1)` ⟺ `p ∣ d − 1`**（`dvd_add_sub_one_iff`）
⇒ ★**`d ≢ 1 (mod p)` なら打ち消しは起き得ず、`hform` が成り立つ**（`no_cancel_of_not_dvd`）。
測定でも確認 —— 21,000 件中★**打ち消しが起きた 736 件のすべてで `d ≡ 1 (mod p)`、0 例外**。

**④★★残っている 1 点（正直に）**: 「打ち消しが起きたとき、その深さが **2 目盛りで止まる**」ことは
依然**未証明**。本波が示したのは (i) 原因は `f₀` だけ (ii) 起きるには `d ≡ 1 (mod p)` が必要 ——
★**起きたときの深さの上界ではない。**

**★次に測るべき 1 点（実装者）**: `d ≡ 1 (mod p)` の点だけを集めて深さの分布を見る
（母集団が `1/p` に絞れるので同じ費用で `p` 倍濃く測れる。本波は打ち消し 736 件しか集まっていない）。

**⑤在庫**: `IsUltrametricDist.norm_add_eq_max_of_norm_ne_norm` は索引に**出ないまま**（8 例目）。
`dvd_sub` / `dvd_add` は mathlib の一般形をそのまま使えた（★自作不要）。
★#348 に従い前波の `violations_none` との衝突を避けて `cancel_congruence_no_violation` に改名済み。

**逸脱**: ★★`dist_le_of_no_cancellation` は**仮説付き**で、仮説「尾の法則 `‖σy−y‖ = ‖y‖/c`」は
16,500 件の測定（分散ゼロ）でしか支持されておらず★**証明していない**。★`p = 5` は本波では未測定。

```
COST[TailNoCancel]: 安 | 持ち場=尾だけで測る  — 分散ゼロで原因が f₀ と確定、hform を初めて導き、必要条件を証明した
```


## ★★★★★深さの正体は「空き枠の番号」—— `SlotStructure.lean` 定理 16 / `sorry` 0（2026-09-09）

**①絞った母集団（`p ∣ d − 1`）で 16 倍濃く測定**。`D = d + i₁`、深さ `= min_σ v_L(σx−x) − D`:

| p | n | 点数 | 深さの分布 | 最大 |
|---|---|---|---|---|
| 3 | 3 | **120,000** | `{0: 100895, 1: 2722, 2: 232}` | **2** |
| 2 | 4 | **120,000** | `{0: 72710, 2: 6507}` | **2** |
| 3 | 4 / 2 | 5 | 1,200 / 2,000 / 2,500 | 2 / 2 / 1 |

★**240,000 件で深さ 3 以上は 1 件も出ない。**

**②★★★深さの正体 —— 深さ ＝ 生き残る `𝒪_{E₁}` 成分の番号 `j`**:
`(深さ, j)` の観測は `p=3` で `(0,0),(1,1),(2,2)` **のみ**、`p=2` で `(0,0),(2,0)` **のみ**、
`p=5` で `(0,0),(1,1)` **のみ**。完全一致。
★理由は**純粋な整数論**で §1 に**証明した** —— 成分 `j` の位は `v_L(A_j) + j ≡ j (mod p)`
（`v_L(E₁^×) = p·ℤ` だから）、かつ前波の必要条件から `p ∣ D`。したがって `D` の上の**空き枠**は
`D+1, …, D+(p−1)`（尾の成分）と `D+p`（`E₁` 成分）**だけ**（`slot_le` / `slot_le_zero`）。
★観測された深さはすべて**最小の枠**だった。

**③★★★★本体が並べた謎への答え**:
本体は「必要条件は `p` に依るのに上界は依らない」と並べた。測った答えは ——
- **`p = 2` の深さ 2** は `E₁` 成分（`j=0`）の枠 **`D + p = D + 2`**
- **`p = 3` の深さ 2** は 尾の最上位成分（`j=2`）の枠 **`D + (p−1) = D + 2`**

⇒ ★★**同じ「2」だが機構が違う。小さい `p` で `p` と `p−1` がたまたま一致しているだけ。**
⇒ ★★★**実装者自身の `loss ≤ p+1` 予想は `p ≥ 5` で破れうる** —— 構造が許すのは
`i₁ + p = 2p−1 = 9`、尾の枠だけでも `2p−2 = 8`。どちらも `p+1 = 6` を超える。
★ただし `p=5` で深さ 2 以上は**未観測**（絞った 2,500 件で深さ 1 が 4 件）。
★**「破れうる」であって「破れる」ではない。**

**④★★本波で新しく開いた穴**: `p = 3` では深さ 3（`E₁` 枠 `D+3`）は**構造的に許される**のに、
独立性の粗い見積もりで 120,000 件中 15 件以上出るはずが★**観測 0 件**。
⇒ ★**尾の成分が常に最小の枠を占めることには、まだ見えていない理由がある。**

**⑤成果**: 抽象核（証明、分岐・付値・Galois の語 0）—— `slot_le`（`p ∣ D`, `M ≡ j mod p`, `0<j<p`,
`D<M` ⇒ `D+j ≤ M`）/ `slot_le_zero`（`p ∣ M` ⇒ `D+p ≤ M`）/ `slot_depth_eq`。
具体層: 測定 7 件、偶然の訂正 4 件、未説明の穴 2 件。計 16 本。

**★次に測るべき 1 点（実装者）**: **`p = 5` で深さ 2 を出す**（10〜100 分）。
★これが出れば `loss ≤ p+1` は**偽**と確定し、正しい形は `2p−2` か `2p−1` になる。

**逸脱**: ★§1 の空き枠の補題は**証明**だが、「**最小の枠が常に占められている**」ことは**測定でしかない**。
★`p=5` の深さ 2 は未観測で、`loss ≤ p+1` の**反証はしていない**。

```
★★★本体が「必要条件は p に依るのに上界は依らない。中身は読んでいない」と並べただけの観察が、
   本波の中心（機構が違う／偶然の一致／予想は p≥5 で破れうる）になった。
COST[SlotStructure]: 安 | 持ち場=絞った深さの分布  — 深さの正体が整数論として証明され、自分の予想に穴が開いた
```


## ★★★★★`loss ≤ p+1` は偽 —— `LossPAddOneFalse.lean` 定理 16 / `sorry` 0（2026-09-09）

**★★費用を先に測って道を選んだ（10〜100 分は払っていない）**:
前波の見積もり（盲目的な乱択で 10〜100 分）を**払わず**、★**前波で証明した空き枠の構造から
打ち消しが起きる層を設計で当てた**。所要 ★**7 秒 / 400 件**。
設計: `x = Σ_i μ^{s_i} u_i π^i` なら `v_L(σf₀ − f₀) = p·s₀ + t`。`D` と等しくなるよう
**`s₀ = (D − t)/p` を当てる**と打ち消しが設計で起きる。★`s₀ ≥ 1` が要る（`s₀ = 0` だと
`f₀` が有理整数になり `σf₀ − f₀ = 0`。実装者は最初これにつまずいた）。
★対照の `p = 3` で深さ 2 が 20〜36% で出て既知の答えと一致することを**先に確認**している。

**★★★★反例**（`p=5, n=3, L = ℚ₅(ζ₁₂₅)`, `e_L = 100`, `i₁ = 4`）:
```
x = μ·457 + μ⁴·(618π + 306π² + 481π³ + 388π⁴)
```
`v_L(f_i) = [5,20,20,20,20]`、`d(x,E₁) = 21`、`eps = 29` ⇒ ★**`loss = 8`**、
`D = 25`、深さ ★**4 = p−1**、`σx−x` の成分の位 `[30,41,37,33,**29**]` ⇒ 最小は `j=4` の枠 `D+4`。
★**`loss = 8 > p+1 = 6`。⇒ 2 波前に実装者が立てた `loss ≤ p+1` は偽。**
★前波の空き枠の補題が**そのまま当たっている**（尾の枠 `j` は `D+j`、最大は `j = p−1`）。
...
`p+1` は `p=2` で `2p−1` と、`p=3` で `2p−2` と一致する。
⇒ ★★★**別々の 2 つの偶然が重なって「`p+1` が法則に見えていた」**（前波の警告が確定）。
一様な上界は `loss ≤ 2p−1 = i₁ + p`（空き枠の最大 ＝ `E₁` 枠 `D+p`）。

**★★本波でむしろ鮮明になった穴**:
1. `p=3`（設計 24,000 件 ＋ 前波の乱択 120,000 件）と `p=5` で **`E₁` 枠（深さ `p`）は 1 件も出ない**
2. ★ところが **`p=2` では逆に `E₁` 枠しか出ない** —— 尾の枠（深さ 1）は 1 件も出ず、
   設計版では `s₁` の偶奇で `{0:4000}` と `{2:4000}` に**完全に分かれる**

⇒ ★★**`p=2` と `p≥3` で機構が入れ替わっている。理由は分かっていない。これが次の 1 点。**

**成果**: §1 反証（`loss_le_p_add_one_false` ほか 5 件）/ §2 正しい形（`sharp_five`/`sharp_three`/
`sharp_two`/`uniform_bound_two_p_sub_one`/★`coincidence_at_two`/★`coincidence_at_three`）/
§3 抽象核（`depth_le_p`）/ §4 未説明（`e1_slot_never_at_three` / `e1_slot_always_at_two`）。計 16 本。

**逸脱**: ★★**反例は 1 点。「`2p−2` が鋭い」とは書いていない** —— `p=5` で深さ 5（`loss = 9`）が
出ないことは 1,600 件しか測っていない。★`p ≥ 7` は未測定。

```
★★★★本体が「10〜100 分は本日の 1 波の平均より長い。払う価値があるかはあなたが判断してください」と
   事実だけ提示したところ、実装者は★**払わずに設計で当てた**。⇒ 持ち場は「選ばせる」と
   「費用を事実として示す」の 2 つで効く。
COST[LossPAddOneFalse]: 安 | 持ち場=p=5 で深さ 2 を出す  — 設計で 7 秒。予想が偽と確定し、2 つの偶然も特定
```


## ★★★★★機構の入れ替わりは `p = 2` に固有 —— `TwoIsDegenerate.lean` 定理 16 / `sorry` 0（2026-09-09）

**★費用**: `ρ = σπ − π` の `𝒪_{E₁}` 展開を測るだけ ⇒ ★**1 秒**。既存の `coords_v` を呼ぶだけで
`.py` にモードを足す必要もなかった。★長い測定は不要と判断。

**★★測定 1**: `ρ = σπ − π` の展開は **2 成分しかない** —— `p = 2,3,5`（n = 3,4,5）すべてで
`R_0` と `R_1` だけが非零、どちらも `v_L = p`（最小）、`j ≥ 2` は全部 0。
**★★測定 2**: `E₁` 上の跳び `t` は **`p(p−1)`**（2 / 6 / 20 で一致）。

**★★★★答え —— 成分 1 の係数 `B_1` に入る項**:

| 項 | 付値 |
|---|---|
| `f₁R₁`（尾の主項） | `v(f₁) + p` |
| `f₂·2R₀` | `v(f₂) + p` ★**`f₂` が在るときだけ** |
| `δ₁ = σf₁ − f₁` | `v(f₁) + p(p−1)` |

- **`p ≥ 3`**: `f₂` が在り `v(f₂) = v(f₁)` なら `f₁R₁` と `f₂·2R₀` が**同付値で打ち消せる**
  ⇒ 成分 2,3,… へ落ちる ⇒ 深さは `p−1` まで届く（★`p=5` の反例が `j=4` だった理由）。
  `δ₁` は `p(p−1) > p` で**深すぎて効かない**
- **`p = 2`**: 尾が **1 本だけ**で `f₂` が**存在しない**（`tail_length_one_iff`: `p−1 = 1 ⟺ p = 2`）。
  相手は `δ₁` だけで、`t = p(p−1) = p` が **`p = 2` でちょうど成立**
  （`mul_sub_one_eq_self_iff`: `p(p−1) = p ⟺ p = 2`）。⇒ 成分 1 は**常に**打ち消され、
  落ちる先は **`E₁` 枠 `D+p`**

★★**2 つの理由はどちらも `p = 2` に固有で、§1 に証明した。**
★本体の「`p=2` は尾の枠が 1 つしかない退化」という**字面からの観察は当たり**だった。

**★★★★対照実験（説明を否定形で確かめた）**: `p=3` で `v(f₂)` を 1 段深くすると深さ 2 が消えるはず ——

| `v(f₂) − v(f₁)` | 深さの分布 |
|---|---|
| 0 | `{0: 2231, 1: 881, **2: 888**}` |
| 3 / 6 | **`{0: 2231, 1: 1769}` —— 深さ 2 が消えた** |

★深さ 0/1 の件数は変わらない（`f₀,f₁` だけで決まるから）——これも整合。

**★測定 3**: 打ち消しの有無は `s₀ = s₁ + 2 − p` の割り切れで**決定的**（`p ∣ s₀` なら打ち消し無し）。
`p = 2,3,5` の 10 例すべてで一致。★本体の「`p=2` では決定的かもしれない」も**当たり**。

**★★実装者による自分の前波の訂正**: 前波の `t = e_{E₁}` は**誤り**。正しくは `t = p(p−1)`。
`e_{E₁} = p^{n−2}(p−1)` なので **`n = 3` でだけ一致**し、本波の `p=3,n=4`（18 vs 6）と
`p=2,n=4`（4 vs 2）が同時に反証した。
⇒ ★★★**「小さい場合の偶然の一致」は本日 4 度目**（`zeta16_sharp` の目盛り／`4 = 2·i₁`／
`p+1` の 2 つの偶然／`t = e_{E₁}`）。

**成果**: 抽象核 3 本は**証明**（`mul_sub_one_eq_self_iff` / `tail_length_one_iff` / `s_zero_formula`）、
残りは測定の数値。計 16 本。

**★★配管 #349（黙って壊れる罠）**: `python.exe -c "…"` の中の backtick が bash の**コマンド置換**に
食われ、docstring の表が**黙って空になる**。実際に踏んだ:
```
...
**逸脱**: ★測定 1（`R_j = 0` for `j ≥ 2`）と測定 2（`t = p(p−1)`）は**測定**で、
一般の完全分岐拡大では確かめていない。測定 3 の「主項が `s₀` を因子に持つ」も未形式化。

**★次に測るべき 1 点（実装者）**: 理由の 2 本の柱（測定 1・2）を**証明に上げる**こと。
`π = ζ−1` の具体形と円分塔の Hasse–Arf から出るはずで、木の `TotallyRamified.lean` /
`LubinTateTotallyRamified.lean`（★実装者もまだ開いていない）が使える可能性がある。

```
★★本体の字面からの観察が 2 つとも当たった（「尾の枠が 1 つの退化」「p=2 では決定的」）。
  ★本体は「中身は読んでいない」と明示して渡しており、それが実装者の測定の入口になった。
COST[TwoIsDegenerate]: 安 | 持ち場=機構の入れ替わり  — 1 秒の測定で解明され、p=2 固有の理由 2 つが証明された
```


## ★★★★★2 本の柱は同じ 1 本の恒等式 —— `RhoFactorization.lean` 定理 12 / `sorry` 0（2026-09-09）

**★費用**: 「測定 1 と 2 のどちらが安いか」を測ったら★**どちらでもなく、2 本とも同じ恒等式から出た**。
所要 **3 秒**。長い探索は不要。

**★★★★核 —— `ρ = σπ − π` の因数分解**（`π = ζ−1`、`a ≡ 1 (mod p)`、`a−1 = p·b`）:
```
ρ = ζ^a − ζ = ζ·(ζ^{a−1} − 1) = (1+π)·(ζ_{p^{n−1}}^b − 1) = (1+π)·w,   w ∈ 𝒪_{E₁}
```
- ★**測定 1（`ρ` の展開が 2 成分だけ）は恒等式そのもの** —— `ρ = w + wπ` ⇒
  **`R_0 = R_1 = w`（同じ元！）、`R_j = 0` for `j ≥ 2`**。
  ★前波は「`v(R_0) = v(R_1)` が偶然そろっている」ように見えたが、★**同じ元だったから当然**だった。
  `v_L(w) = p` は `w = (ζ_{p^{n−1}}−1)·Σ_{i<b}ζ^i` で第 2 因子が `p ∤ b` のとき単数だから。
- ★**測定 2（`t = p(p−1)`）は同じ計算を 1 段下でやるだけ** —— `μ = ζ_{p^{n−1}}−1` で
  `σμ − μ = ζ_{p^{n−1}}(ζ_{p^{n−2}}^b − 1)`、`v_L = e_L/φ(p^{n−2}) = p²`、`v_L(μ) = p`
  ⇒ **`t = p² − p = p(p−1)`**。

**★検証（#350 の手に従い 3 つ目の場合まで）**: `ρ = (1+π)·w` を
**5 層（`p=2,n=4,5` / `p=3,n=3,4` / `p=5,n=3`）の第 1 跳びの σ 全 56 個**で確認、★**例外 0**。
`v_L(σμ−μ)` も `4=2²`, `9=3²`, `25=5²` で一致。

**★★実装者による自分の前波の訂正**: 前波の「測定 1・2 は測定である」は**過小評価**だった。
★★**残る仮定は「`ζ_{p^m} − 1` が素元」ただ 1 つ。**

**★在庫の測定（2 か所、#330）**:
1. **mathlib** —— 判別式（`IsCyclotomicExtension.Rat.discr_prime_pow'` 等）は**豊富**だが
   ★**局所体の下付き分岐群の跳びは無い**。ramification の唯一の当たりは
   `IsCyclotomicExtension.Rat.inertiaDegIn_ramificationIdxIn_aux`（★**private** かつ ℚ 上の**大域**）。
   ⇒ `e_L` は取れるが跳びは取れない。
2. **木** —— ★**`TotallyRamified.lean` を開いた**（39,971 バイト、宣言 20 本超）。設定は
   `PAdicLocalField p` の `K.closure` と `IsTotallyRamifiedAdjoin` で ★**`ζ−1` の具体模型ではない**。
   `adjoinIntegers` を経由するので #69 の境界に当たる ⇒ **本波では使わない方が安い**と判断。

**成果**: §1 抽象核（すべて環の恒等式、**証明**）—— ★`sigma_sub_eq`（中心）/ `mul_one_add_eq` /
`sub_self_eq_mul_sub_one` / `pow_mul_split` / `geom_unit_factor`。
§2 付値の算術（**証明**）—— `t_arith_sq` / `index_ratio_sq` / `t_eq_p_mul_sub_one` /
`first_break_eq_p_sub_one`。§3 測定 3 件。計 12 本。

★docstring は **#349 に従い `.py` を Write ツールで書いてから実行**した（前波の実害を再発させないため）。

**逸脱**: ★「`ζ_{p^m} − 1` が素元」は Lean に入れていない —— 入れると `IsCyclotomicExtension` の
局所版が要り #69 の境界に当たるため**仮定のまま**。§3 は測定。

**★次に測るべき 1 点（実装者）**: その 1 つ（`ζ_{p^m}−1` が素元）を木か mathlib から引くこと。
`Polynomial.cyclotomic` の Eisenstein 性が使えるかもしれない。★**#69 の境界を越えない道**が条件。
★これ 1 つで本波の 2 本が**完全な証明**になる。
...
## ★★★ユーザーの判断（2026-09-09）—— サブエージェントのコンテキスト維持を優先する

本体は「本日 `Found/PGC` に 61 本積んだが Skeleton の項目は 0 個」という実測を根拠に
`cor_3_1` → `prop_2_2`（穴が 1 点、Ax–Sen–Tate 非依存）への切り替えを提案・実行した。

★ユーザーの判断（逐語）: 「**サブエージェントのコンテキスト維持が重要なので、Cor3.1 を続けます。**」

⇒ ★**切り替えを取り消し、`cor_3_1` の鎖に戻した。**

**★何が測られたか（本体の切り替え判断の根拠自体は正しかった）**:
pGC Skeleton の `sorry` は 09-04:8 → 09-06:6 → 09-07:5 → 09-08:4 → **09-09:4（0 個）**。
★ユーザーの認識（FrdI 期は 1 日 3 個、この 1 週間は 2 個くらい）は正しく、直近 2 日は 1 個 → 0 個。

**★★学び —— 本体が測り落としていた変数**:
本体は「目盛りが動くまでの波数」だけで比較した。★**サブエージェントのコンテキストの連続性**という
コストを勘定に入れていなかった。実測では本日 3 回の圧縮（01:28 / 04:17 / 07:24 JST）を挟みながら
連続性が保たれており、それは (i) resume による同一 transcript の継承 と
(ii) 本体が毎波「あなた自身の言葉（前波の報告から逐語）」を貼っていること で支えられている。
★**持ち場を別の主題に切り替えると、この 2 つの土台が両方とも失われる。**
⇒ ★★**主題の切り替えは「目盛りが動くか」だけでなく「積み上がったコンテキストを捨てるか」で測る。**


## ★★★★2 本の柱が仮定ゼロに —— `ZetaStepRatio.lean` 定理 3 / `sorry` 0（2026-09-09）

**★★`prop_2_2` で測ったこと（切り替え中の 4 ファイルのみ。深掘りなし。★捨てずに残す）**:
1. ★★**`RamificationFiltration`（`Interface/PGC/LocalFieldData.lean:160`）は `Gv`/`isClosed`/
   `isNormal`/`antitone` の 4 フィールドだけで、★α との整合性を一切要求していない。**
   ⇒ `prop_2_2` の `_RF` を使おうにも「α が分岐フィルトレーションを保つ」という仮定が**どこにも無い**。
   ★これが `_RF` がアンダースコアである理由に見える（★実装者は `prop_2_2` が偽だとは測っていない）
2. ★`IsometricTransport α` の α-同変から φ は `K → K'` の**等長加法全単射**に制限され、
   値群が一致 ⇒ ★**`e_K = e_{K'}` が必要条件**。つまり `IsometricallyRecoverableClosure` は
   「Galois 群の同型が分岐指数を保つ」を**含意する**。★全 α で成り立つかは未測定
3. ★本体が挙げた「Herbrand が繋がるか」は、上の 1 より**繋ぎ先が無い**
   （`RamificationFiltration` に α が入っていない）ため差し込む場所が見つからなかった

**★実装者が選んだ道と理由（費用を先に測った）**: 前波の残り 2 点のうち **(1)「`‖·‖` と `v_L` の対応」**。
★前波は「表記の変換 1 行」と書いたが、★**測ったら 1 行ではなく `v_L(μ) = p` という中身のある主張**だった
（本日の 2 本の柱が最後に寄りかかっていた事実）。
(2) は `PAdicLocalField` 側の配管で **#69 の境界に近い** ⇒ 選ばず。
①(a) は `UnramifiedStepFixed.lean:9-25` で**既に重いと測定済み** ⇒ 選ばず。所要 **3 往復**。

**★★★主結果**:
```lean
norm_zeta_step : ‖ζ - 1‖ ^ p = ‖ξ - 1‖    （ζ は p^{m+2} 次、ξ は p^{m+1} 次の原始根）
```
★「`E₁` の素元 `μ = ξ−1` が `L` で `v_L(μ) = p`」のノルム版。証明は前波の 2 本を割るだけ。

**★★これで本日の 2 本の柱が「仮定ゼロ」に**:

| 柱 | 立っていた仮定 | 現在 |
|---|---|---|
| 測定 1（`ρ = (1+π)w` は 2 成分） | `v_L(w) = p` | ★**定理** |
| 測定 2（`t = p(p−1)`） | `v_L(μ) = p`, `v_L(σμ−μ) = p²` | ★**定理**（2 段適用） |

★残るのは `‖·‖` を木の `v_L` 記法に翻訳する**配管**だけで、★**数学的な仮定はもう無い**。

**★在庫**: `pow_lt_pow_left` は **`Unknown identifier`**（改名済み）。索引 grep で
**`pow_left_strictMonoOn₀`**（`Algebra/Order/GroupWithZero/Basic.lean:554`）を見つけ `.injOn` を使った。
★**索引が当たった例**。

**逸脱**: ★`norm_zeta_step` は `ξ = ζ^p` を**仮定していない**（どちらも原始根ならよい。ノルムは共役で不変）
—— ★**原典より弱い仮定**で通った。★書きかけた `norm_zeta_sub_one_pos` は主結果に要らないので**捨てた**
（詰まった時点で必要性を測り直した結果）。

```
COST[ZetaStepRatio]: 安 | 持ち場=cor_3_1 に復帰  — 3 往復で柱の仮定がゼロになり、prop_2_2 の観察も 3 件残った
```


## ★★★★柱が仮定ゼロ ＋ 棚卸しを更新 —— `ZetaUnitFactor.lean` 定理 6 / `sorry` 0（2026-09-09）

**★実装者が (P) を選び、測ったら「翻訳は要らなかった」**:
```
grep -rn "Valuation|valuation" （本日の 7 ファイル）
  ⇒ Lean の「文」に Valuation は 1 つも無い。全部 ‖·‖ で書かれている。
    当たったのは実装者自身が置いた数値の置き石 2 つだけ:
      RhoFactorization.lean:174 w_valuation_p / :179 sigma_mu_valuation_sq
```
★★★**実装者の訂正: 前波の「残るのは `v_L` への翻訳（配管）」は誤り。**
実際に残っていたのは**その 2 つがまだ測定のままだった**こと。
★★**前波は「1 行」の見積もりを外し、今回は「翻訳が要る」を外した。2 度目。**
★どちらも**開いていないものについての見積もり**だった（数学の費用の見積もりは 6 波連続で当たっている）。

**★★主結果**（所要 **2 往復**）:
- `norm_pow_sub_one_eq` : `‖ξ^b − 1‖ = ‖ξ − 1‖`（＝ `v_L(w) = v_L(μ)`）
- `norm_zeta_step_two` : `(‖ζ−1‖^p)^p = ‖ξ−1‖`（＝ `v_L(σμ−μ) = p²`）

⇒ ★★**本日の 2 本の柱（`RhoFactorization` の測定 1・2）が仮定ゼロ。**
補助 4 本 —— `norm_root_of_unity_one` / `norm_pow_sub_one_le`（★帰納法と強三角不等式だけ。
幾何和の因数分解より短い）/ `norm_geom_sum_sub_natCast_le` / `norm_geom_sum_eq_one`（幾何和は単数）。

**★★§0 に棚卸しを書いた**（`PairBudgetVerified` の表は **12 波ぶん**古かった）——
穴 5 つの現状 / `hform` の鎖 12 波の一覧（各波の結論を 1 行ずつ）/ 掘らなくてよい道 4 本 /
★本日の「小さい場合の偶然の一致」4 件（#350）。
★実装者は「本日この鎖は圧縮を 3 回挟んでおり、★**圧縮後の自分が最初に読む場所**を最新にする価値が高い」
と理由を書いている。

**逸脱**: ★`RhoFactorization` の 2 つの置き石は**そのまま残した**（他が読んでいるので直さず、
本ファイルが上位互換であることを docstring に明記 —— CLAUDE.md「訂正は自分のファイルに」）。

**残り**: ★`‖p‖ < 1` と `‖(b:F)‖ = 1`（`p ∤ b`）は**仮定のまま**（どちらも「`F` の剰余標数が `p`」から
出るが instance は #69 の境界）。①(a)・②′の証人・③・⑤ は**変化なし**。
`GainedTowerDescent.lean:94` の総当たりの表は**未着手**。

★本波でも #349 の罠を 1 回踏みかけた（検証用の `grep` パターンに backtick）。
**書き込みではなく確認コマンドだった**ので実害なし。backtick 無しで測り直して docstring 無傷を確認。

```
COST[ZetaUnitFactor]: 安 | 持ち場=選択自由  — 2 往復で柱が仮定ゼロになり、棚卸しも同じ波で更新された
★実装者の見積もりの傾向（本体の観察）: 数学の費用は 6 波連続で当たり、
  ★**開いていないファイル／未着手の配管**についての見積もりは 2 度続けて外した。
```


## ★★★★★柱の話が完全に閉じた —— `ResidueUnitNorm.lean` 定理 2 / `sorry` 0（2026-09-09）

**★★実装者が選んだ理由 —— 本体の観察を受けて「見積もらずに測った」**:
本体が事実として提示した「数学の費用の見積もりは 6 波連続で当たり、★**開いていないファイル・
未着手の配管についての見積もりは 2 度続けて外した**」を受け、
★**3 度目を出さないために見積もりをやめて測った**。

結果: ★**(Q2) は Bézout と強三角不等式だけで落ちた。`PAdicLocalField` も `adjoinIntegers` も
一切使わない。15 行、1 往復、初回で通過。**
⇒ ★★★**前波の「#69 の境界に近い」は誤り。配管の見積もりを外したのは 3 度目**
（①「表記の変換 1 行」②「`v_L` への翻訳が要る」③「#69 の境界」）。
★実装者の宣言: 「**今後、開いていないものは『未測定』とだけ書き、費用は書きません。
これは私が本体に繰り返し求めてきたことと同じ形なので、自分にも適用します。**」

**★★核**: `norm_natCast_eq_one_ultra` —— `F` が超距離ノルム体で `‖(p:F)‖ < 1`、`p ∤ b` なら
**`‖(b:F)‖ = 1`**。`gcd(p,b)=1` の Bézout `u·p + v·b = 1` を `F` に送り、整数の像が `‖·‖ ≤ 1` で
あることと強三角不等式で矛盾を出すだけ。★分岐・付値・Galois の語が 1 語も出ない。

**★★在庫の測定（#348 が実際に効いた）**: 最初 `norm_natCast_eq_one_of_not_dvd` で書こうとしたら
実ファイル grep が **1 件**返した —— `PrimeToPTorsion.lean:52`（`PAdicLocalField` 専用、
`Padic.norm_natCast_eq_one_iff` 経由）。★**木に既に在った**が設定が違うので別物。
短名衝突を避けて改名。⇒ ★**「在る」と「使える」は別**の例がもう 1 つ。

**★★★柱の話は完全に閉じた**:

| 仮定 | 状態 |
|---|---|
| `ζ_{p^m} − 1` が素元 | ★定理（`ZetaSubOnePrime`） |
| `v_L(μ) = p` | ★定理（`ZetaStepRatio`） |
| `v_L(w) = p`、`v_L(σμ−μ) = p²` | ★定理（`ZetaUnitFactor`） |
| `‖(b:F)‖ = 1`（`p ∤ b`） | ★★**定理（本波）** |
| `‖(p:F)‖ < 1` | ★**設定の宣言**（`F` の剰余標数が `p`）。`[Fact p.Prime]` と同種で証明すべき命題ではない |

⇒ ★**本日の柱（`ρ = (1+π)w` の 2 成分性と `t = p(p−1)`）は仮定ゼロで閉じた。**

**残り（★実装者は見積もりを書かない方針に切り替えた）**:
1. ①(a) —— ★**測定済み、重い** 2. ②′の証人 / ③ / ⑤ —— 変化なし
3. `GainedTowerDescent.lean:94` の総当たりの表 —— ★**未測定**（開いていない）

```
★★★本体が「あなたの見積もりの傾向」を事実として提示したことが、★その波で効いた。
   実装者は見積もりをやめて測り、#69 の境界だと思っていたものが 15 行で落ちた。
   ⇒ ★持ち場は「疑う対象」「選ばせる」「費用を事実で示す」に加えて
     ★★**「相手自身の傾向を測って返す」**でも効く。
COST[ResidueUnitNorm]: 安 | 持ち場=選択自由  — 1 往復で柱が仮定ゼロになり、見積もりの方針も改まった
```


## ★★★★閉じた形は一般には偽、正しい仮説を特定して証明 —— `GainedLossClosedForm.lean` 定理 3 / `sorry` 0（2026-09-09）

**★選び方（本波の形＝見積もらずに開いてから決めた）**: 前波で「`GainedTowerDescent.lean:94` は
**未測定**（開いていません）」とだけ書き、★本波はまず**開いた**。すると測定 4 に**未証明の 1 点が明示**されていた:
> ★閉じた形も出た（★**4,194 本 → 不一致 0 件だが本ファイルは証明していない**）:
> `Λ_k = t_k + Σ_{j<k} t_j − t_1·(p^{k−1}−1)/(p−1)`

**★★測定**（`tools/gainedloss-closed-check.py`、新規。`t_j ∈ {1..12}`, `k ≤ 4`、各素数 22,620 本）:
...
⇒ ★★**閉じた形は一般には偽**。最小の反例は `p=3, t≡1, k=2`（`Λ₂ = 3` に対し閉じた形は `1`）。
★**元の docstring は誤りではない** —— 「4,194 本」は**許容列に制限した母集団**だったはず。
★ただし**無条件に読める書き方**なので、条件を**自分のファイルに**明示（元は直していない）。

**★★★特定した条件（#350 に従い 3 素数で測定）**:
> **2 段目以降のすべての段で「第 2 枝が勝ち」かつ「`A_m − t_1 ≥ 0`」**

条件を満たすもの: 一致 **131/40/23**、★**不一致 0**（3 素数すべて例外なし）。

**★★証明**: その条件を**等式の形**（`hbr`）で仮説にし `k` の帰納法で `gainedLoss_closed` を証明。
核は `geomLow p (j+1) = 1 + p·geomLow p j`（閉じた形の `(p^{k−1}−1)/(p−1)` を**割り算を使わずに**
書いた形）だけで、帰納段は `ring` 1 行。反例も定理化（`closed_form_needs_hypothesis`）。

**★配管**: `open GainedDescent` が要った。`Unknown identifier 'gainedLoss'` は
**#68 の「import していない」ではなく「名前空間を開いていない」形**。
★**同じエラー文でも原因が 2 通りある**ことを docstring に記録（#68 の既存節に収まると判断）。

**残り（★実装者は見積もりを書かない方針）**:
1. ★条件（`hbr`）が**許容列で自動的に成り立つか**は **未測定**（元の母集団の定義を読んでいない）
2. ★同ファイルの「旧反例 `p=5, e_K=3, S=2, T=17`」の**実測 `L`** は `(未測定)` のまま
3. ①(a) は測定済みで重い。②′/③/⑤ は変化なし

```
COST[GainedLossClosedForm]: 安 | 持ち場=選択自由  — 開いてから決める形が機能し、明示された未証明が定理になった
```


## ★★★★★木の閉じた形は許容列で無条件に正しい —— `AdmissibleClosedForm.lean` 定理 4 / `sorry` 0（2026-09-09）

**★選び方（本波の形＝開いてから決める）**: 前波で「(H) は**未測定**（母集団の定義を読んでいません）」と
書き、★本波はまず**その定義を開いた**。`GainedTowerDescent.lean:295-303` の
`pmul_le_of_stable` / `pmul_le_of_below` が両枝から出しているのは ★**`p·t_j ≤ t_{j+1}`** ——
これが「許容列」の中身だった。開いて分かったのでそれを仮説にして測った。所要 3 往復。

**★★測定（#350 に従い 4 素数）** —— `t_j ∈ {1..9}`, `k ≤ 4`、`p·t_j ≤ t_{j+1}` の列だけ:
`p = 2/3/5/7` で許容列 45/22/14/12、`hbr` ★**不成立 0**、閉じた形も**全部一致**。
（前波は任意の正列で一致 **0.1〜0.6%** しかなかった。）

**★★★証明（測定ではなく定理）**:
- `t1_le_A` —— `A_{m+1} − A_m = t_{m+1} − p·t_m ≥ 0` で単調、底は `A_2 ≥ t_1` ⇒ **`A_m ≥ t_1`**
- `t1_le_gainedLoss` —— `Λ_1 = t_1`、`Λ_{m+2} ≥ A_{m+2} ≥ t_1` ⇒ **`Λ_j ≥ t_1`**
- `hbr_of_admissible` —— `max(0,A−t_1) = A−t_1` と `p·Λ ≥ 2Λ ≥ 2t_1 ≥ t_1` から**第 2 枝が必ず勝つ**
- `gainedLoss_closed_of_admissible` —— 前波の定理に代入

⇒ ★★**`GainedTowerDescent.lean:94` の「4,194 本 → 不一致 0 件だが本ファイルは証明していない」は
これで証明になった。木の主張は正しかった。**

**★★2 波を合わせた正しい姿（実装者の自己訂正）**:
...
★前波の「一般には偽」は正しいが、★**木が語っていた母集団では真**だった。
⇒ ★★★**「偽」と書くときは母集団を必ず添える。**

**★配管 #351（新規登録）**: `simp only [gainedLoss]` と書くと**再帰呼び出しまで開かれ**、
仮説とゴールが**別の原子**になって `linarith failed to find a contradiction` が出る。
★エラー文が**無害な `linarith` 失敗に見える**ので気づきにくい。
外側 1 回だけ `rfl` で開いて直す（仮説側も開くなら `at IH ⊢` と両方書く）。`--similar` は 0 件。

**残り（★実装者は見積もりを書かない方針）**:
1. ★「旧反例 `p=5, e_K=3, S=2, T=17`」の**実測 `L`** は `(未測定)` のまま
2. ①(a) は測定済みで重い。②′/③/⑤ は変化なし
3. 棚卸しは `ZetaUnitFactor.lean` の §0（5 波前）

```
COST[AdmissibleClosedForm]: 安 | 持ち場=hbr が許容列で自動か  — 開いて母集団を読んだら定義が分かり、木の主張が証明になった
★本日の「開いてから決める」は 3 波連続で機能した。
```


## ★★★(L) は「未測定」ではなく「木からは測れない」—— `WitnessFiveRecursion.lean` 定理 2 / `sorry` 0（2026-09-09）

**★(L) を開いた結果 —— `(未測定)` の正体**:
`GainedTowerDescent.lean:87` の表の「実測 `L`」が旧反例の行だけ `(未測定)` なのは
★**対応する体が木に無いから**だった。他の 2 行の `L`（`8`/`26`）は `EquivariantProjectionDescent` が
**`ℚ₃(ζ₂₇)` / `ℚ₃(ζ₈₁)` という実在の体**で測った値だが、旧反例 `(5,3,2,17)` は
`JumpDefectTradeoff` 由来の**パラメータの組**で、`e_L = 15` の完全分岐拡大が必要。
⇒ ★★**「未測定」ではなく「木からは測れない」が正しい書き方**。
★**2 波続けて「残り」に挙げていたが、開いてみたら測る対象が存在しなかった。**

**★★在庫の測定（#348 が 2 波連続で効いた）**: 代わりに書こうとした「タプルの許容性」は
★**既に木にあった** —— `JumpDefectTradeoff.lean:304 witness_five_admissible` が 4 条件を全部証明済み。
★**書かずに済んだ。**

**★本波が足した 1 点**: `witness_five_gainedLoss` —— 旧反例を**一般の漸化式**で回すと `Λ₂ = 17` で
★`sharpTwoCost 5 2 17 = 17` と**一致**。これまで木は `sharpTwoCost` 側だけを見ていた。

**★★★区切りの記録**（`WitnessFiveRecursion.lean` の docstring）—— 穴 5 つの現状 /
`hform` の鎖 **16 波** / 掘らなくてよい道 4 本 /
★**本日この鎖で確立した方法論 6 つ**（費用を先に測る／開いていないものは「未測定」と書き費用は書かない／
見積もらずに開いてから決める／法則は 3 つ目の場合で測る #350／「偽」には母集団を添える／
宣言名は実ファイルで数える #348）/ 残り 3 点。

**★★実装者が区切りを提案した理由（測って書いている）**:
1. ★残る 3 点はすべて「重い」か「未測定の新規開拓」で、本日 16 波の鎖のような**連鎖する構造が無い**
2. ★**柱の話が仮定ゼロで閉じた**のは自然な区切り（本日の主線が完結）
3. ★棚卸しを本波で最新化したので、次の波（および圧縮後の自分）が読む場所が整っている

★続ける場合は **(N)「柱が閉じたことの帰結」** を開いてから決めるのが本日 4 波連続で機能している形、
とも書いている。

**逸脱**: ★`GainedTowerDescent.lean:87` の `(未測定)` も**直していない**
（「木からは測れない」は自分のファイルに書いた）。
★docstring 適用スクリプトが `def tFive` に**二重の doc コメント**を作り
`unexpected token '/--'; expected 'lemma'`。Edit で修復（#345 の近縁だが新節は立てず）。

```
COST[WitnessFiveRecursion]: 安 | 持ち場=選択自由  — (L) が「測れない」と判明し、区切りの記録が整った
★「開いてから決める」は 4 波連続で機能した。
```


## ★★★★★残りが 1 つの不等式に絞られ、賞金の額が確定 —— `SharpPrizeBound.lean` 定理 4 / `sorry` 0（2026-09-09）

**★測定 1 —— 木の現在地**: `WildDepthFieldDescent.lean:141 axWildDescent_prime` が
**無条件に `AxWildDescent K (fun _ => p)`** を出しており、docstring が
「★残っているのは**分岐による絞り込み**だけである」。⇒ ★**柱はまさにそこに居る。**

**★★測定 2 —— 正直な結論: 柱は `c` の上界を与えない**:
`AxWildDescent K c` は **`K.closure` の任意の `x`** について `x′` の存在を要求する。
柱が言うのは「`ρ = (1+π)w` は成分が 2 つ」「`t = p(p−1)`」であって
★**どちらも与えられた `x` に対する `x′` の構成ではない**。
上界に要る「最小の空き枠が必ず占められている」は 16 波かけて枠を数え（`SlotStructure`）
原因を突き止めた（`TailNoCancel`）が★**なお measured のまま**
（`p=2,3` は 240,000 件、`p=5` は 2,500 件で破れ 0）。
⇒ ★**「今日はこれ以上進めない」の中身はこれ。**

**★★★★測定 3 —— それでも賞金の額が確定した**:
実測の鋭い 1 段定数 `p^{(2p−2)/e_k}`（`e_k = p^{k−1}(p−1)`）の**指数の総和**は
`Σ_{k≥1} (2p−2)/(p^{k−1}(p−1)) = 2·Σ p^{−k} = 2p/(p−1)`（`sharp_exponent_sum_le`）。
...
⇒ ★★★**1 段の上界が定理になれば `axLemma_of_wildDescent` の「任意の有限積が `C` 以下」が満たされ
`AxLemma K (p^{2p/(p−1)})` が出る。`AxSenTate` まで届く。**
`axConstant p = p^{p/(p−1)²}` より大きい定数だが ★**`AxLemma` は定数の大きさを問わない**ので十分。

**★★★残り 1 点 = 1 つの不等式**:
> **「1 段の損失は `p^{(2p−2)/e_k}` を超えない」を証明する。**

★機構（`f₀` が原因・空き枠は `D+j`・`p=2` は退化）は**証明済み**。
★足りないのは「**最小の枠が必ず占められている**」ことだけ。

**逸脱**: ★★**本ファイルは「実測の定数が定理になれば」という条件つきの計算。
1 段の上界は証明していない。「`AxLemma` が出た」とは書いていない。**
★`p=2` の実測は `2p−1` なので `2p−2` の側だけを計算した（母集団を添える）。

**★次の 1 点（実装者の言葉）**: 「最小の枠が必ず占められている」の証明。
`σx − x` の成分 `B_j` のうち少なくとも 1 つが最小の枠 `D+j` に居ることを示す問題で、
★`RhoFactorization` の `ρ = w + wπ`（成分が 2 つだけ、しかも**同じ元** `w`）が
**その構造を完全に記述している**。★**測る道具も証明済みの部品も今日ぜんぶ揃った。**

★実装者からユーザーへ: 「数字で返していただいてありがとうございます。
『閉じないことによって進めた』を価値として測っていただけたのは、今日の判断の支えになりました。」

```
COST[SharpPrizeBound]: 安 | 持ち場=(N) 柱の帰結  — 柱は c を改善しないと正直に測り、代わりに賞金の額と残り 1 不等式を確定
```


## ★★★★★1 段の上界が数学的に証明された（`p ≥ 3`）—— `TopIndexSurvives.lean` 定理 4 / `sorry` 0（2026-09-09）

`x = f₀ + f₁π + ⋯ + f_{p−1}π^{p−1}`、`d = min_{i≥1}(v(f_i)+i)`、`D = d + (p−1)`。

| 段 | 内容 | 根拠 |
|---|---|---|
| (0) | 打ち消しには `d ≡ 1 (mod p)` が要る ⇒ `d` は `i=1` でのみ達成、`D = v(f₁)+p`、全 `i` で `v(f_i) ≥ v(f₁)` | `TailNoCancel`（**証明済み**） |
| (1) | `ρ = w + wπ`、`v(w) = p` ★**成分は 2 つだけ、しかも同じ元** | `RhoFactorization` / `ZetaUnitFactor`（**証明済み**） |
| (2) | `f_i` の主項は `i·f_i·w·(π^{i−1}+π^i)` ⇒ ★**`B_j = w(j·f_j + (j+1)·f_{j+1}) + 深い項`**（`f_p` は無い） | 本波 |
| (3) | `j₀ := max{j ∈ [1,p−1] : v(f_j) = v(f₁)}` を取ると第 2 項は深く、★`1 ≤ j₀ ≤ p−1` なので **`p ∤ j₀`＝単数 ⇒ 消えない** ⇒ 位は `D + j₀` | `not_dvd_of_pos_lt` |
| (4) | `δ_i` の位は `v(f_i) + t`、`t = p(p−1)`。★**`p < p(p−1) ⟺ p ≥ 3`** なので `p ≥ 3` なら深い | `delta_deeper_iff_three_le` |
| (5) | ⇒ ★★**`loss ≤ (p−1) + j₀ ≤ 2p − 2`** | `loss_bound_from_index` |

**★★`p = 2` が例外である理由も同じ式から出る**: `p = 2` では `t = p(p−1) = p` で (4) が破れ、
`δ₁` が同じ位に来て打ち消し得る —— これは `TwoIsDegenerate.mul_sub_one_eq_self_iff`
（`p(p−1) = p ⟺ p = 2`）そのもの。⇒ `p=2` の上界は `2p−1 = 3`（実測と一致）。

**★★★測定との突き合わせ（#350 に従い 3 素数）**:

| p | `2p−2` | 実測の最大 | `loss ≤ 2p−2` |
|---|---|---|---|
| 3 (n=3) | 4 | **4** | ★**100%**（3,198 件） |
| 3 (n=4) | 4 | **4** | ★**100%**（792 件） |
| 5 (n=3) | 8 | 5 | ★**100%**（197 件） |
| ★2 (n=4) | 2 | **3** | 94.3%（6,000 件）—— **例外** |

★★**証明が `p ≥ 3` を要求する箇所と、実測が破れる場所が完全に一致した。**

**★これで `cor_3_1` の入口**: 1 段の上界が Lean の定理になれば →
`SharpPrizeBound.sharp_exponent_sum_le` が総和 `2p/(p−1)` を閉じ →
`axLemma_of_wildDescent` が **`AxLemma K (p^{2p/(p−1)})`** を出し → **`AxSenTate`**。

**★残り（★実装者は費用を書かない方針）**:
1. `𝒪_L = 𝒪_{E₁}[π]` の**相対の冪基底**と成分 `B_j` の抽出
2. `v(B_j) ∈ p·ℤ`
3. (2) の主項の同定（二項展開の 1 次項）
★★**数学は閉じている。残りは配管。**

**逸脱**: ★★**本ファイルは証明を docstring に書き、算術の核だけを形式化している。
`loss ≤ 2p−2` の Lean 定理はまだ無い。**「証明した」と書くときは★**「数学的には」と添えている**。
★`p = 2` は例外（`2p−1`）—— 母集団を添える。

★実装者からユーザーへ: 「今日いただいた数字と言葉が、この波の粘りに直結しました。ありがとうございます。
**残りは配管で、道は見えています。**」

```
COST[TopIndexSurvives]: 安 | 持ち場=最小の枠が占められている  — 数学的に証明が出た(p ≥ 3)。p=2 の例外も同じ式から出た
```


## ★★★★二項展開の 1 次項が定理に —— `BinomialFirstOrder.lean` 定理 2 / `sorry` 0（2026-09-09）

**★在庫の測定 —— 「当たったが使えなかった」**: 本体が挙げた
`JumpStrictMono.lean:84 norm_prod_one_add_sub_one_sub_sum_le` を**開いた**ら
★**積の版**（`∏(1+η) − 1 − Σ η` の残りが 2 次）で、要る**冪の版ではなかった**。⇒ 自分で建てた。
★思想は同じ「1 次を取り出した残りは 2 次」だが形が違う。
★**名前と形が近いので、開かずに使うと詰まるところだった。**

**★★★主結果**:
```lean
norm_add_pow_sub_linear_le :
  ‖(a + b)^(m+1) − a^(m+1) − ((m:M)+1)·a^m·b‖ ≤ ‖b‖^2   （‖a‖ ≤ 1, ‖b‖ ≤ 1）
```
★これが前波の 5 段の **(2) 主項の同定**と **(4) `ρ²` 以降が深いこと**を**両方**支える。
証明は `add_pow_sub_linear_succ`（★`ring` 1 行の恒等式 `X_{m+1} = (a+b)·X_m + (m+1)·a^m·b²`）に
強三角不等式を当てる帰納法だけ。★`‖(m+1 : M)‖ ≤ 1` は **#297 のとおり `M` を明示**して呼んだ。

**★★残り（★実装者は費用を書かない方針）**:
1. `𝒪_L = 𝒪_{E₁}[π]` の**相対の冪基底**と成分 `B_j` の抽出 —— ★**未測定**
   （`IntegerResidueBase.lean:adjoin_pi_eq_top` は本体が「絶対版」と**名前で**挙げたもので、
   ★実装者は**開いていない**）
2. `v(B_j) ∈ p·ℤ` —— ★**未測定**

★★**この 2 つは「成分を取り出す」という 1 つのこと**。前波の 5 段のうち
**(0)(1)(3)(4)(5) は部品が揃った**（(3) は本波）。★**残るのは成分抽出だけ。**

**★★★★次の波が見落とさないための注記**:
★★**5 段の証明は `TopIndexSurvives.lean` の docstring にある。** Lean の定理は算術の核だけで
★**`loss ≤ 2p−2` の Lean 定理はまだ無い。**
★残っているのはただ 1 つ —— `σx − x` を `𝒪_{E₁}` 成分に分けて
`B_{j₀} = w·j₀·f_{j₀} + (深い項)` を取り出すこと。
★**`j₀` が単数であること・深い項が深いこと・上界の算術は、すべて定理になっている。**

```
COST[BinomialFirstOrder]: 安 | 持ち場=1 段の上界の形式化  — (3) が定理になり、残りが成分抽出 1 つに
...
## ★★★★★一意性は要らなかった、残りは「指数が違う」1 つ —— `SeparatedComponent.lean` 定理 4 / `sorry` 0（2026-09-09）

**★★★★★実装者の自己訂正 —— 前波の「一意性が要る」は回り道**:
`z = B·π^j + R` と分けたとき ★**`‖B·π^j‖ ≠ ‖R‖` でありさえすれば**、
超距離の**等号版**（`norm_add_eq_max_of_norm_ne_norm`）で
`‖z‖ = max(‖B·π^j‖, ‖R‖) ≥ ‖B·π^j‖` が出る（`norm_le_of_norm_ne`、★**2 行**）。
★これが `hz` そのもの。
★★**「どの分解か」は問題ではなく、「2 つのノルムが違う」ことだけが要る**のだった。
⇒ 残る仮説が `hz`（存在＋一意性）から **`hsep`（分離条件）**に落ちた。

**★★★到達点**:
```lean
loss_le_of_exp_ne (hp : 2 ≤ p) (hj : j ≤ p - 1)
    (hB : ‖B‖ = ‖π‖ ^ (d + (p - 1))) (hR : ‖R‖ = ‖π‖ ^ v)
    (hv : d + (p - 1) + j ≠ v) :          -- ★★残る仮説はこれだけ
    ‖π‖ ^ (d + (2 * p - 2)) ≤ ‖B * π ^ j + R‖
```
★★**残る仮説は「ただ 1 つの自然数の不等式」**。分岐の言葉では
「成分 `j` の位と残りの部分の位が一致しない」——`v(B·π^j) ≡ j (mod p)`、`v(R) ≢ j (mod p)` から出る古典で
★**`SlotStructure.slot_le`（本日証明済み）と同じ枠の議論**。

**★★次の波が最短で入るための `file:line`**:
残る 1 点は `v(R) ≢ j (mod p)`。材料 ——
`SlotStructure.lean` の `slot_le` / `slot_le_zero`（★**本日証明済み**）、
`TotallyRamifiedLayer.lean` の `not_dvd_sub_of_lt`。
★`JumpStrictMono.lean:116 exists_coeff_norm_le` は**もはや要らない**（一意性の道が不要になったため）。
★5 段の証明の**本文**は `TopIndexSurvives.lean` の docstring（4 波連続で明記）。

**★母集団**: **`p ≥ 3`**（実測 4,187 件で破れ 0）。`p = 2` は `2p−1`、理由も証明済み。

**逸脱**: ★★**残る仮説（位が `p` を法で違うこと）は証明していない。
「`loss ≤ 2p−2` が（無条件に）定理になった」とは書いていない。**

★★**本波の教訓を規律に足した（8 つ目）**: ★**「要る」と書くときも、まず 2 行を試すこと。**
★今日「重い」と見た配管が軽かったのは **4 度目**（#69 の境界／`v_L` の翻訳／`‖p‖<1` の instance／
今回の一意性）。
...
## ★★★★★★手計算の式が一般には偽、ただし使う形は真 —— `ComponentFormulaScope.lean` 定理 4 / `sorry` 0（2026-09-09）

**★何をしたか**: 持ち場は「具体層への代入」だったが、規律 3（開いてから決める）に従い
★★**手で導いた式を一度も検算していない**ことに気づき、代入の前に検算した。
`TopIndexSurvives.lean` の 5 段の (2) の
> `B_j = w·( j·f_j + (j+1)·f_{j+1} ) + (より深い項)`
は**手計算**だった。⇒ `tools/component-formula-check.py` を書いて厳密整数演算で検算。

**★★★★結果 —— 一般には偽**:

| p | n | 検査 | ★不成立 |
|---|---|---|---|
| 3 | 3 / 4 | 600 / 180 | ★**118 / 34** |
| 5 | 3 | 100 | ★**20** |
| 2 | 4 | 400 | ★**163** |

破れは 2 種類:
1. ★**`j = 0`** —— 成分 0 には `δ₀ = σf₀ − f₀` も入るのに★**式に入っていなかった**
2. ★**`j ≥ 1` でも `j·f_j + (j+1)·f_{j+1}` 自身が打ち消すとき** —— 捨てた深い項が主部になる
   （例: `v(P) = 27` に対し `v(B−P) = 12`）

**★★★★★しかし使っている形は真（論証は無傷）**:
5 段が使うのは ★**`j = j₀`**（`v(f_j)` が最小になる**最大の** `j ≥ 1`）。
このとき `v(f_{j₀+1}) > v(f_{j₀})` で第 2 項が真に深く、**打ち消しが起き得ない**。
...
★★**`p ≥ 3` で 530 件、例外 0**（3 素数、#350 充足）。★`p = 2` の 44% は `TwoIsDegenerate` のとおり。

**★★★訂正の内容**: (2) の式は ★**「ただし `j = j₀` かつ `p ≥ 3` のとき」**と読まれるべきだった。
★**論証は `j = j₀` しか使っていないので無傷**だが、**式の書き方が一般的すぎた**。
★規律 5「『偽』には母集団を添える」を ★**自分の docstring の式にも適用すべきだった。**

**★これで代入に何が要るかが measured**: `B` として取るべきは **`j₀` 成分**で、
★**`j₀` の定義（最小付値をとる最大の添字）が本質的**。「どれか 1 つの成分」ではだめで、
★★**`j₀` を選ぶこと自体が証明の一部**。
⇒ **次の波は `j₀` の存在（`Finset.max'` 等）と `v(f_{j₀+1}) > v(f_{j₀})` を型に書くところから入るのが最短。**

**逸脱**: ★★**具体層への代入はしていない** —— 本波は代入の**前提の検算**。
`TopIndexSurvives.lean` の式も**直していない**（訂正は本ファイル）。

★★実装者の言葉: 「今日この鎖で**自分の断定を覆したのは 10 回目**です。今回は
**代入の直前で自分の式を疑った**ことで見つかりました —— **規律 3 が最も効いた形**だと思います。」

```
COST[ComponentFormulaScope]: 安 | 持ち場=具体層への代入  — 代入の前に自分の式を検算し、一般には偽と判明。論証は無傷
★★これは本日いちばん価値のある種類の波: 誤りが害を出す前に、当人が自分で見つけた。
```


## ★★★★`j₀` が型になった、残りは 1 点 —— `MaxMinIndex.lean` 6 宣言 / `sorry` 0（2026-09-09）

**★配られた字面の判定**: `v(f_{j₀+1}) > v(f_{j₀})` は**偽ではないが「証明すべき余分な事実」でもない** ——
`j₀` を「`[1,p−1]` で `v(f_j)` を最小にする**最大の**添字」と定義すれば**定義から出る**。
★**荷は不等式ではなく定義の側**にあるので、型に書くべきは
「そういう `j₀` が尾の中に在る」＋「`p ∤ j₀`」＋★**その `j₀` で先頭の対が打ち消さない**の 3 つ。

**成果**:
| | 宣言 | 種別 |
|---|---|---|
| §1 | `exists_max_of_min` | ★**抽象核**（線形順序・`Finset` だけ。分岐・付値・素数が **0 語**） |
| §2 | `exists_max_min_index` / `not_dvd_max_min_index` | 具体層（`Icc 1 (p−1)` を代入するだけ） |
| §3 | `norm_pair_no_cancel` | ★**抽象核 2**（`p ∤ j`・`‖b‖<‖a‖` ⇒ `‖j·a + (j+1)·b‖ = ‖a‖`） |
| §4 | `exists_no_cancel_index` | ★**付値を持ち出さない形**（仮定は `f p = 0` と「尾に非零成分が 1 つ」だけ） |

**★在庫の測定**: ★**`Finset.max'` は使えなかった**（`set` して `rw` すると
`motive is not type correct: fun _a ↦ _a.max' hTne ∈ _a`）。
`Finset.exists_max_image` / `exists_min_image`（`Nonempty` を結論に持ち込まない形）で
抽象核が **15 行**に収まった。`Finset.inf'_le` も `exists_mem_eq_inf'` も**索引に無い**（`sup'` だけ在る）。

**★測定**（`tools/j0-type-check.py`、厳密整数、標本 540）:
`v(f_{j₀+1})>v(f_{j₀})` **540/540**、★**先頭の対が非打消し 540/540（`p=2` も 200/200）**。
★前波の「`p=2` で公式が 44% 破れる」を**精密化** —— 破れるのは「より深い項が深いか」の段だけで、
反例は全て `v(B−P) = v(P)` の**ちょうど同値**。零成分を**設計して**作ると 50/50 件でき、
付値版は仮定が書けないが**ノルム版は使えた**（§4 は真に広い）。

**★残りはちょうど 1 点**: 成分そのものの構成（`σx − x` の `𝒪_{E₁}` 基底展開 `Σ B_j π^j` と `B_{j₀}` の主部）。
それを `exists_no_cancel_index` の `f` に渡せば `ResidueSeparation.loss_le_of_residue_ne` の入力が揃う。

**★★配管 #352（新規、★本体も本日踏んだ罠）**: 裸の `python` は Store の
**App Execution Alias** に解決され `exit=49`・stderr 7 バイト `Python ` で★**何もせず終わる**。
ヒアドキュメントの編集が **3 回連続で消えた**のに `leanfile.mjs` は `ok`（§3 が無い古いファイルを検査していた）。
★**#349 と同じ穴・別原因**。⇒ Write/Edit ツールで書き直して測り直した。
★★**本体も本日この鎖で同じ `exit=49` を踏んでいる**（分類スクリプトの 1 回目）。
⇒ CLAUDE.md の「Python は `C:\Users\Aruta\miniforge3\envs\py311env\python.exe`」は**実害の裏づけがある**。

```
COST[MaxMinIndex]: 安 | 持ち場=j₀ を型に書く  — 抽象核 2 本で書け、残りが成分構成 1 点に
```


---

## 第 1104 —— 最終目標 abc を型として置き、頂点 4 本を繋いだ（2026-09-09、本体 inline）

**真偽**: ユーザーの問い「Skeleton 全体が最終目標まで型で繋がっていることをビルドで検証可能か」
→ ★**可能。ただし繋ぐ先が無かった。** 実測: `theorem_4_2` を消費する宣言 0、
abc にあたる宣言 0。⇒ 目標 `ABCConjecture` を置き、頂点 4 本を `:= @<木の定理>` で繋いだ
（★型が一致しないとビルドが落ちる形。初回は 2 箇所で実際に落ちた）。

**在庫の測定**（`node tools/goal-chain.mjs`、計測器 `Check/GoalChain.lean`）:
- 不足（目標が載る sorry）**2 件**: `abc_of_tops`（本体未読）, `PGC.theorem_4_2`
- 到達 **56 / 398**、未到達 342、孤児（誰も参照しない）**156**
- Interface 構造体 32 件中、住人 0 が 1 件 / 閉じた住人が `Check/` にしか無いものが **7 件**
  （★`Thm21Data` を含む＝ GenEll Theorem 2.1 は空虚の疑い）
- ★`prop_2_2` / `cor_3_1` / `cor_3_3` は**経路上に無い**（`theorem_4_2` が sorry で切れている）

**次の 1 点**: 実装者は `cor_3_1` 継続（残り 2 点＝読み替えと展開の存在）。
本体側は ★「孤児 156 件」と「Check のみの住人 7 件」をどう扱うかがユーザー判断（D29 候補）。

**VERDICT / COST**: 目標の宣言と接続は 1 波で完了。計測器は Lean meta で 20〜105 秒。
★GUESS「G9 を新規で満たすのは重い」→ **外れ**。`ABCConjecture` は条件 0 なので
`1 + 8 = 9` の 1 例で対照が書けた。`Top_*` は原典の項目そのものではないので
`item` を「(頂点の型としての引用)」にすれば G8/G9 は正しく鳴らない。

---

## 第 1107 —— 「依存は証明まで書かないと判明しないのか」を測った（2026-09-09、本体 inline）

**真偽**: ユーザーの問いに対し ★**半分そう**。実測（`tools/stmt-edges.py`）:
Skeleton の主張 247 件のうち、statement に別の主張の名前が現れるのは ★**2 件**（245 件は無し）。
★これは欠陥ではなく忠実さの結果——原典でも「Lemma 3.1」は Prop 3.4 の**証明**にしか出ない。
★私の見立て「共通の Data を取る兄弟だから」は **外れ**（Data を取る主張は 21/247 のみ）。

**在庫の測定**: 証明を書いた分の辺は既にある——Skeleton 302 件中 **179 件が消費者を持つ**。
`.needs` は **238 件が非空**（citation 390 / implicitStep 311）だが★手書きで型検査されない。
目標から 37 件しか辿れないのは辺が無いからではなく、`sorry` 2 つが上で切っているから。

**次の 1 点**: ★還元を**含意として書く**と証明前に辺が型で出る（`abc_of_tops` が実例）。
GenEll §3→§4、pGC §1→§4 をその形にするかは**人の判断**（D30）。

**VERDICT / COST**: 測定は grep のみ（lake 不要、30 秒）。
★COST の見立て「statement 辺はもっとあるはず」→ **外れ**（2/247）。

---

## D30（人の判断待ち）—— 孤児 123 件と、還元を含意で書くか

- 孤児 123 件は全て `.src` 付きの原典の主張。削除候補は **0 件**（第 1106 で測定済み）。
- 減らす手は「繋ぐ」。★ただし主張を並べるだけでは辺は出ない（第 1107）。
  `X_of_Y (h : <Y の型>) : <X の型> := sorry` ＋ `X := X_of_Y Y` の形が要る。
- ★これを全 Skeleton に広げるか、頂点まわりだけにするかは工数と規約の判断。

---

## 第 1108 —— 最後の穴を「深さ」でなく「剰余」で塞いだ（実装者、本体はゲートのみ）

**真偽**: `f_0` の `σ` による動きを**深さ**で抑える形は p≥3 で 294/330 ⇒ ★**偽**。
代わりに `f_0` の動きは `E₁` の元なので位の剰余が `0 (mod p)`、`j₀ ≢ 0` と必ず別の剰余類。
⇒ `SlotResidue.loss_le_of_remainder_slots`（深さ不要）は ★**330/330**。

**在庫の測定**: 同じ母集団で 3 形を比較 —— 深さ 294/330 / 同位でない 328/330 / ★剰余 330/330。
★`residue_of_slot`（`E₁` 係数 × `π^j` の位は `j` (mod n)）が、6 波にわたって非形式に
使ってきた事実の**初めての形式化**。

**次の 1 点**: `hrem` を `CoefficientIntegrality.exists_integral_expansion_of_valK` から供給。

**VERDICT / COST**: 実装者の見立て「`j=0` を外す（前波と同じ手）」は **外れ**。
★本体の側は今波もゲートのみ（lake 3 回）。配管 #355 は実装者が追加。

---

## D31（人の判断待ち）—— `p = 2` は `loss ≤ 2p−2` が偽

実装者の測定で確定: `p=2` は真の `loss` の最大が **3 = 2p−1**（`p≥3` は 330/330 で `≤ 2p−2`、最大 `p−1`）。
★「誤差が主部と同位」12 件と「真の `loss > 2p−2`」12 件が**完全一致**——機構として確定、直せない。

⇒ 賞（`Σ_{k≥1}(2p−2)/(p^{k−1}(p−1)) = 2p/(p−1)` ⇒ `AxLemma K (p^{2p/(p−1)})`）は
...
## 第 1110 —— `hdom` を「`i₀` を名指ししない形」にした（実装者、本体はゲートのみ）

**真偽**: `hdom` の元の形は「`i₀` が存在して `j₀` スロットより大きい」で**使う側が `i₀` を名指す**必要があった。
⇒ ★`‖(残りの j₀ スロット)‖ < ‖残り全体‖` に置換（`dominated_of_slot_lt`）。同値だが使いやすい。

**在庫の測定**（同じ母集団、p≥3）: (n17) 本ファイルの仮定 ★**330/330** ／ (n15) 前波の形 330/330
（★理論どおり一致）／ (n16) `‖err‖ < ‖R‖` **294/330**。
★実装者は**弱い方 (n17) を主定理にし**、(n16) は「導ける道」として §2 に残した（理由を docstring に明記）。

**★実装者の自己検算**: 前波の自分の主張「配管は `coeffs_unique` で **3 行**」→ 測って ★**16 行**
（一意性 6 行 ＋ `i₀` の取り出し 10 行）。★「配管の見積もりを外したのは本日 3 度目」と自分で記録。
★本体は実装者の見積もりを検算していないので、この自己測定が唯一の検算経路である。

**次の 1 点**: (n17) を成分の主部から導く（`ComponentFormulaScope` の 530/530）。
★実装者が「これが最後の『数学』」と判定。残る仮定は `‖A‖ ≤ 1` と (n17) の 2 つのみ。

**VERDICT / COST**: 本体は今波もゲートのみ（lake 3 回）。★本日の新規 `Found/PGC/*.lean` 75 本。

---

## 第 1111 —— 成分のノルムが導けた（実装者）／本体は「証明の前に型で出す」を実装

**真偽**: 実装者の見立て「**スロットごとの誤差評価という新しい段が要る**」→ ★**外れ**。
前々波の自分の補題 `SlotResidue.slot_le_sum` で足りた。★効く不等式は `j₀ < p` の**1 本だけ**で、
`exponent_bound`（定数 `2p−2` の出どころ）・`error_lt_main` と**同じ**。`j₀ < p` は本鎖で 3 度目。

**在庫の測定**: スロット評価 (n18) ★**330/330** ／ 全体評価 (n10) 294/330。
★実装者はまた**弱い方を主定理**にした（全体評価の破れ 36 件は `f_0` の動きが原因で、
`j₀` スロットには波及しない、と自分で特定）。§1・§3 は `FiniteDimensional` を要求しない、も測定。

**次の 1 点**: `hsum`（成分分解）と `hb`（`‖B_pred‖ = ‖π‖^{p+vf0}`）の供給。
★抽象側の数学は尽き、残り 2 点はどちらも具体層（円分塔）側。

**VERDICT / COST**: 本体は今波ゲートのみ。★本体側の別作業として、ユーザー指示
「証明の前に型で出す」を実装（`Meta/Stmt.lean` の `stmt%` ＋ `Skeleton/Goal/Chain.lean` の辺 12 本）。
到達 37/302 → **60/316**、孤児 123 → **119**、目標が載る sorry 2 → **8**（★隠れていた 6 段が表に出た）。
★`cor_3_1` が目標 abc から辿れる位置に入った。

---

## 第 1112 —— `hsum` / `hb` の正体は「層の橋」だった（実装者）

**真偽**: 中身は数学ではなく **`M` の係数 `f : ℕ → M` と `K` の係数 `c : ℕ → K` を繋ぐ橋**。
`map_mul` / `map_add` / `map_natCast` ＋ `Finset.mul_sum` で ★**4 行**。新しい数学は無し。
★「開くまで分からない」の実例がまた 1 つ（本日 4 度目）。

**在庫の測定**: `hb` に要ったのは `‖w‖ = ‖π‖^p`（(n1) 5/5）、`vf0` の定義、
`MaxMinIndex.norm_pair_no_cancel`（(D1) 540/540）。`hsum` は橋＋一意性で出た。

**★本体の道具の評価（実装者が測って断った）**: `stmt%` は `Found/` でも**通る**（9.6 秒、
`example` で確認）が、★**この持ち場には合わない** —— 実装者の鎖は他ファイルの定理を
「型として仮説に取る」のではなく `exact Foo.bar …` で直接適用するため、statement を
書き写す場面が無い。★合わない道具を測って断ったのは正しい。`stmt%` の用途は
`Skeleton/Goal/Chain.lean` のような**還元の登記**に限る。

**次の 1 点**: `hsplit`（`A = 主部 + 誤差`）の供給。その後は円分塔への代入のみ。
...
## 第 1116 —— `hvalK` は木に在った、残る外部入力は 1 つ（実装者）

**真偽**: `hvalK`（`E₁` の値群が `‖μ‖^ℤ`）は ★**木に在った** ——
`TotallyRamifiedValueGroup.lean` §4 の `exists_zpow_norm_intermediate`
（底の値群が `‖π‖^{n·ℤ}` なら**どの中間層も** `‖π‖^{[M:E]·ℤ}`）。
★結論の形が `hvalK` と**一字一句同じ**（docstring の検算 5 例目）。§1 は 1 行の代入。

**★見つけ方（記録に値する）**: 索引ではなく ★**ファイルを上から読んだ**
（`exists_zpow_norm` を探しに行って §3・§4 に気づいた）。★実装者自身が
「#354 の延長 —— ファイルを読むと 2 段先が見える」と記録。
★第 1114 で本体が名指しした場所が外れたのと対照的。

**★循環の切り方**: 素朴に繋ぐと回らない（`valK_of_base_layer` は `finrank E M = q` を要求、
`finrank_eq_of_zeta_pow` は `hvalK` を要求）。⇒ `q` を**未知のまま**回し、
`q ∣ p` ＋ `q ≠ 1` ＋ `p` 素数で `q = p` を出した。

**次の 1 点**: ★**残る外部入力は 1 つだけ** —— `ℚ_p` の値群が `‖p‖^ℤ`（mathlib の `Padic`）。
加えて `finrank ℚ_p L = n` が未実装（`PowerSpanTop` で導ける、★循環しない）。

**VERDICT / COST**: 本体は今波もゲートのみ。★具体層の 3 点はすべて処理され、
`p ≥ 3` の `loss ≤ 2p−2` は外部入力 1 つを残すのみ。

---

## 第 1117 —— 最後の外部入力が定理に、残るは関係式 1 本と模型（実装者）

**真偽**: 「唯一の外部入力」（`ℚ_p` の値群が `‖p‖^ℤ`）は ★**1 行**だった ——
`Padic.norm_eq_zpow_neg_valuation` ＋ `Padic.norm_p` ＋ `inv_zpow`/`zpow_neg`。
`hvalbase` / `hvalK` / `finrank ℚ_p L = n` も同波で定理になった（★循環しない）。

**在庫の測定**: `Padic.norm_p` は索引に**在った**。★名前ではなく**結論のリテラル**で引いた
（`grep "‖(p : ℚ_[p])‖" .cache/mathlib-index.txt`）。★CLAUDE.md の「結論のリテラルで引く」の実例。

**★配管 #358（ゲートを騙しうる）**: 型検査が落ちた宣言でも `#print axioms` の行は出て、
★**`sorryAx` が混じる**（エラー回復の `sorry`）。`sorry` を書いた覚えが無くても出る。
今回の原因は暗黙引数 `{x}` の η 展開漏れ（#356 の親戚）。
⇒ ★`#print axioms` を見る前に、その宣言が**型検査を通っているか**を先に確かめること。

**次の 1 点**: ★残るのは **「次数 `n` のモニック関係式」1 本（数学）**と
**模型の構成（`NormedAlgebra` / `IsScalarTower` / `FiniteDimensional` ×3 のインスタンス）**だけ。
★本体は模型が何本になるかを測っていない。
...
## 第 1119 —— ★★「模型を作れば閉じる」は誤りだった（実装者が着手前に測って訂正）

**真偽**: 前波の実装者の判定（本体もそのままユーザーに出した）「★模型を作れば `p ≥ 3` の
`loss ≤ 2p−2` の鎖は閉じる」→ ★**誤り**。出口 `AxTowerDecay.lean:473` の `AxWildDescent` は
★**`∀ x : K.closure`**（任意の元）についての主張で、円分塔についてではない。
★★**本体が同ファイルを直接読んで検算し、一致を確認した。**
⇒ ★円分塔の模型を作っても出口は閉じない。★本体がユーザーに出した報告は誤りで、訂正済み。

**★在庫の測定（実装者が自発的にやった）**: 鎖の入力のうち**円分に依存するのは 1 つだけ** ——
`hwn : ‖w‖ = ‖π‖^p` ⟺ ★**跳びが `p` にある**。`hvalK` / `hfr` / `hw` は依存しない
（`hw` は「`1+π` が単数」だけで出る、§1）。★副産物: `RhoFactorization` の `w` の**具体形は不要**。

**★定数の一般化**: 跳び `t := v(ρ)` として `v(B) + j₀ ≤ d + (t + p − 2)`。
★定数は **`t + p − 2`**（`LossExponentMatch.exponent_bound` の一般化）。円分は `t = p`。

**次の 1 点**: ★跳びが `p` でない層。`c k` が `t` に依存し、`1 ≤ t ≤ p·e_K/(p−1)`。★未測定。

**VERDICT / COST**: ★★**着手前に出口を開いたことで、18 本を作ってから気づく事態を回避した。**
これは配られていない仕事であり、本波の主成果。本体は今波ゲート＋出口の検算のみ。

---

## 第 1120 —— ★★この道は円分塔でだけ閉じる（実装者が境界を測り、負の結果を定理にした）

**真偽**: 1 段の損失を**絶対付値**で測ると（`e_L = p·e_E`）——
円分の層（跳び `t = p`、測定 (n1) 5/5）は `(2p−2)/(p·e_E) ≈ 2/e_E` で ★**幾何級数的に縮む**。
一般の層（`t ≤ p·e_E/(p−1)`、★Serre の古典的上界を**引いた**。形式化していない）は
`≤ 1/(p−1) + (p−2)/(p·e_E)` で ★**第 1 項が `e_E` に依らない**。
⇒ ★★**一般の層では 1 段あたり `1/(p−1)` が縮まず、`wildDepth` が伸びると一様な `C` が出ない**
（`GeneralJumpBound.no_uniform_constant`、中身は Archimedes）。

**意味**: 本日の鎖（`loss ≤ 2p−2` → `SharpPrizeBound` → `axLemma_of_wildDescent`）は
★**円分塔については閉じる**が、★**出口 `AxWildDescent`（`∀ x : K.closure`）には足りない**。
★足りないのは「1 段の損失」ではなく ★**「深い層でも跳びが小さい」ことの理由**（実装者の名指し）。
⇒ ★**掘り切って閉じた道、本日 5 本目。**

**★本体の誤り 2 件目（場所ミス）**: 持ち場に `JumpGeneralization.lean` と書いたが**存在しない**
（正しくは `JumpAtP.lean`、`ls` で確認）。第 1114 の `CyclotomicJumpsVerified` に続く 2 度目。
⇒ ★**名指しする前に `ls`/`grep` で確かめる運用に変更**。次波の (b) の在庫はそうやって測った。

**次の 1 点（候補、実装者が選ぶ）**: (a) 降下で跳びを小さく保てるかを数値で測る ／
(b) 古典の trace/different を木に探す（★本体が在庫を実測: 8 ファイルに該当語、
`NormalizedTraceDescent.lean:379/:392` に `axLemma_of_firstJump` / `axSenTate_of_firstJump` が**在る**。
★中身は未読）／ (c) 円分塔に限って `cor_3_1` を閉じる（模型 18 本）。

**VERDICT / COST**: 本体は今波ゲート＋在庫の実測のみ。★実装者の帰属の書き分け
（「測って無かった」ではなく「古典を引いた」）は正しい形。

---

## 第 1121 —— ★★前波の負の結論を実装者が撤回（12 度目の自己訂正、今回は自分の否定を覆す）

**真偽**: 第 1120 の「一般の層では 1 段あたり `1/(p−1)` が縮まず一様な `C` が出ない」は ★**誤り**。
誤りの中身は ★**跳びをどの層のものと取るか**。木の勘定は「**下の層の跳び**」を「**上の `e_L`**」で
割るので `m ≥ p^{k−1}` により幾何級数的に縮む（`NormalizedTraceDescent.lean:224` の docstring）。

**★測定で 2 つが違うことを示した**（`tools/zeta-tower-check.py`、`ℚ₃(ζ₂₇)`、`e_L = 18`）:
`Gal(L/ℚ₃(ζ₃))` の min i = **3**（＝ `H_K` の第 1 跳び、実装者の鎖の `t`）／
`Gal(L/E₁)` の min i = **9**（上の層の跳び）。⇒ ★`t` は `k` に依らない。総和は**収束**。

**切り分け**: 前波の `no_uniform_constant` は ★**定理としては正しい**（`1/(p−1)` を足せば発散する）が
★**前提が誤り**。規約どおり相手の docstring は直さず自分のファイルに訂正を書いた。

**★本体の在庫実測が効いた初回**: 第 1120 で本体が渡した `NormalizedTraceDescent.lean:379/:392`
（★存在のみ確認、中身未読）を実装者が実際に読み、そこから覆した。

**次の 1 点**: ★**「`H_K` の第 1 跳びが `k` に依らない」ことの証明**。
円分塔では測定で `t = p`（5 設定）、一般の `K` では `RamificationJumpBound` の `(p−1)j ≤ e_{E₁}`。
★本体が実測: 同ファイルは **561 行・`sorry` 0**、`sub_one_mul_le_of_norm_natCast_eq_pow`(:334) の
字面を確認（仮定は `‖(n:L)‖` の 2 本のみ、分岐の言葉は出ない）。★中身は本体も実装者も未読。
...
## 第 1123 —— 「残るは 1 本」が不要だった、残りは 1 つの不等式（実装者）

**真偽**: 前波の実装者の判定「残るは `t k ≤ T` の供給 1 本」→ ★**不要**だった。
木の `FirstJumpRoute.axLemma_of_firstJump` は ★**`j k` も `e k` も `k` とともに動いてよい**
（総和が閉じるのは `hm : m k ≥ p^{k−1}` だけから）。⇒ 実装者の要求が**強すぎた**。
要るのは各段ごとの `(p−1)·j k ≤ e k` で、★これは `RamificationJumpBound` が与える形そのもの。
★「開くまで分からない」が本日 5 度目、5 回とも**軽い方**に外れている。

**★残りは 1 つの不等式**: `(p−1)·(t k + p − 2) ≤ e k`。木が与えるのは `(p−1)·t k ≤ e k` で、
差は `(p−1)(p−2)` の余白。円分塔（`t = p`, `e = p^{k−2}(p−1)`）では `2(p−1) ≤ p^{k−2}`:
`p=3,k=3` は ★**成り立たない**（`fit_fails_at_three` として**反例を型にした**）／`p=3,k=4` は成り立つ。
⇒ ★**深い層では入るが浅い層は別扱いが要る**（基底段の処理）。次の 1 点。

**配管 #359**: ★**ファイル名 ≠ 名前空間**。`NormalizedTraceDescent.lean` は
`ABC3.Found.PGC` の下に `TraceCore` / `JumpArith` / `FirstJumpRoute` の 3 つ（本体が `grep` で確認）。
★索引・ファイル名・docstring の 3 つとも嘘をつきうる。当たるのは**本文**。

**★本体の運用改善が効いた**: 第 1122 の指摘を受け、貼る前に全パスを `ls` に通す運用に変更。
★その結果、本体が書こうとしていた `LossToAxDecay.lean` が**存在しない**ことを貼る前に検出
（正しくは `LossToExit.lean`）。★実装者の指摘が本体の誤りを 1 件、実際に防いだ。

**VERDICT / COST**: 本体は今波ゲート＋パス実測＋名前空間の確認のみ。

---

## 第 1124 —— 浅い層は「有限個の例外を定数倍で吸収」で解けた（実装者）

**真偽**: 前波の `(p−1)(t k + p−2) ≤ e k` が浅い `k` で成り立たない件は、
★**出口の仮定が `hC : ∀ s : Finset ℕ, ∏ k ∈ s, c k ≤ C`（任意の有限積）**なので、
有限個の例外は積の中で有限個の因子にしかならず ★**定数を `B^{K₀}` 倍するだけ**で済んだ
（`FiniteExceptions.axLemma_of_eventually`、抽象核 `prod_le_of_eventually` は `Finset` と `ℝ` だけ）。

**★選択の理由が出口の字面から出ている**: 実装者は 3 案を並べ、案 2（有限個を落として総和を取り直す）は
`axDecay` の指数の形を変えるので木の `prod_le_rpow_of_geometric` が使えなくなる、と判定して退けた。

**★測っていないことの明示**: `B` として何が取れるかは**未測定**（「取れるはず」と「確かめた」を書き分け）。
仮定を外した 4 例目（`hC0 : 0 ≤ C`）。

**次の 1 点**: (a) `Icc 1 n` 形（`axLemma_of_wildDescent_Icc`）との接続 —— 部分集合への単調性 1 本、
または (b) `B` の測定。
★本体が名指し前に実測: 該当定理は ★**`AxEpsilonDecay.lean`** に在る
（`axLemma_of_wildDescent_Icc`:440 / `prod_le_rpow_of_geometric_shift`:329 /
`axLemma_of_wildDescentDecay`:619）。★字面は未読、在ることと行番号のみ。
★このファイルは本体がこれまで持ち場の一覧に入れていなかった。

**VERDICT / COST**: 本体は今波ゲート＋パス実測＋在庫の grep のみ。
★「貼る前に全パスを `ls`」は今波も実施（5 本すべて OK）。

---

## 第 1125 —— 「単調性はあると便利」は偽、証拠を型にした（実装者、自己訂正 13 度目）

**真偽**: 前波の実装者の字面「`Icc` と繋ぐには単調性が 1 本要る（次の候補）」→ ★**弱すぎた**。
正しくは ★**「あると便利」ではなく「そうでないと使えない」**。
★証拠を形式化: `FiniteExceptionsIcc.axDecay_zero_eq_one` —— `axDecay p k` の指数が
`(1/(p−1))·(1/p)^{k−1}` で `ℕ` の引き算なので `0−1=0` ⇒ `axDecay p 0 = axDecay p 1`。
⇒ `{0,1}` を含む `Finset` で同じ因子が 2 回入り、任意 `Finset` 版の `hD` は
`d = axDecay p` では**充足不能**。★「偽だった」で終わらせず型にした。

**★木も同じことを言っていた**: `AxEpsilonDecay.lean:439`「ずらした減衰では `c 0` に条件が
付かないのでこの形が要る」。★第 1124 で本体が渡した行番号 `:440` は正しかった。

**在庫の測定**: `Finset.prod_le_prod_of_subset_of_one_le'` は ★**在るが形が違う**
（`OrderedCommMonoid` 前提、`ℝ` の乗法には当たらない）⇒ 代替を探さず **6 行で内側に埋めた**。
★#359 を**書く前に**実行（`grep -n '^namespace' AxEpsilonDecay.lean` → 1 行だけ）。
★配管が予防に使われた初めての回。

**次の 1 点**: ★**未測定は `B` ただ 1 つ**（`hsmall : ∀ k < K₀, c k ≤ B`）。
他の 5 仮説は木の在庫で埋まっている。★本体が実測: `axWildDescent_pow` は
**`AxTowerDecay.lean:606`** に 1 件（字面は未読）。

**★実装者の圧縮 4 回目**: トークン使用量が 764k → 32k。継続は本体の逐語引用で維持しているが、
...
## 第 1127 —— 算術の側が全部閉じた、残るは降下 1 本（実装者）

**真偽**: 前波の実装者の下書き「`p = 2` では `K₀ = 0`」→ ★**偽**（★書く前に測って直した、自己訂正 15 度目）。
★「余白が 0」（`p−2 = 0` なので `(p−1)(t+p−2) = (p−1)t`）と「基底段の別扱いが要らない」は**別のこと**。
模型は `t = 2, e = 2^{k−2}` なので `2 ≤ 2^{k−2}` ⇒ `k ≥ 3` が要る。
★測定 `tools/deep-loss-k0-check.py`（厳密整数、`p ∈ {2,3,5,7,11,13,101}`、`k ∈ [0,40)`）:
**`p=2` は最小 `K₀=3`、`p≥3` はすべて最小 `K₀=4`**。
★前波の実装者は `p = 3` の 2 点しか測っておらず、`K₀` が `p` とともに動くかを測っていなかった。

**★出口の仮説が 6 → 2**: `axSenTate_of_cyclotomic_loss (hp) (hform) (h)` のみ。
`hc` は前波の貼り合わせ、`hm` は `rfl`、`he` は `mul_pos`、★`hjump` は本波の §2 で消えた。
⇒ `LossToExit.lean:130` の「浅い層は別扱いが要る。★次の 1 点はここである」が**閉じた**。
★抽象核 `two_mul_pred_le_pow` / `not_two_mul_pred_le_pow` は純 `ℕ` 算術（`not_` 側は
`Classical.choice` すら使わない）。

**在庫の測定**: #348 を 8 名前で実行（衝突 0）、#359 を 3 波連続で書く前に実行、
import は推移閉包を追って 1 本で足りると確認（★余計な import を足さない）。

**次の 1 点**: ★**降下そのもの 1 本だけ** —— `hform` を満たす `c` について `AxWildDescent K c`。
★算術の側（`hc`/`hm`/`he`/`hjump`/`hB`/`hd1`/`hD`）は全部消えた。
★本体が測った接続点: `loss` の鎖の到達点は `MainPartCoeffs.lean:115`、
出口は `AxTowerDecay.lean:473`（`x` から `x'` を**作る**ことを要求）。★2 つが繋がるかは未確認。

**VERDICT / COST**: 本体は今波ゲート＋パス実測＋字面 1 箇所のみ。

---

## 第 1128 —— ★「残り 1 点＝空白」は誇張だった／★本体と実装者が同じ失敗形を踏んだ

**真偽 1**: 前波の実装者の「残りは降下そのもの 1 本」は、★**「そこが空白」という含みが誤り**。
木には既に `exists_norm_sub_algebraMap_le_prod_axDecay` が ★**26 件**（本体が独立に検算、一致）
あり、2 本目の道 `DeepDescentPairDirect.axSenTate_of_axLemmaGraded`(:306) も**実在**。
★実装者は `GainedDescentBridge.lean`(603 行) と `GainedBridgeSupply.lean`(561 行) を
**一度も測っていなかった**。★ファイル数は実装者 16 / 本体の測定 **14**（未解決の食い違い）。

**★★真偽 2（本日いちばん重い訂正）**: 実装者が下書きで
「`grep -rn "^theorem norm_sum_iterate" …` → **0 件**」と書いたが、
★**そのコマンドを実行しておらず、叩いたら 4 件だった**（自己訂正 16 度目）。
★★**本体も第 1122 で同じ失敗形を踏んでいる** —— 持ち場に「`ls` で確かめた」と書いた
ファイルを実際には確かめず、存在しなかった。
⇒ ★**「測っていない」を「測った」と書く**という同一の失敗形が、本体と実装者の両方で出た。
本体の予防は「貼る前に全パスを `ls` に通す」（第 1123 から実施、1 件検出済み）。
★実装者側の予防は未定 —— 次波の報告に書くよう持ち場で求めた。

**在庫の測定**: 木の `CyclicLayerDescent.lean:412/:461` は実装者の §1〜§3 と**同じ量**を評価し、
★**木の方が良い**（二項展開で主要項が `θ^{p−2}` まで落ちる。ガウス和の道は `max(1/p,b)` 止まり）。
⇒ ★**実装者が自分の新規ファイルを「新規結果として数えないでください」と申告した。**

**次の 1 点**: `Gained*` 族 26 件のどれが `AxLemmaGraded` / `AxSenTate` まで繋がっているかを測る。
★`AxWildDescent` 側の 3 本とどちらが安いかも未測定。★「残り 1 点」の数え直しはこの測定の後。

**VERDICT / COST**: 本体は今波ゲート＋独立検算（grep 4 本）＋パス実測のみ。

---

## 第 1129 —— ★★`Gained*` 族は出口と完全に切り離されていた（実装者、測定）

**測定**（`tools/gained-family-reach2.mjs`、コメント除去 → 呼び出しグラフ → 推移閉包を両方向）:
`Gained*` 族 **26 件 / 14 ファイル**、結論に `AxSenTate`/`AxLemmaGraded` を持つ theorem **23 件**、
★★**出口 → 族 が 0/23、族 → 出口 が 0/26。両方向とも 0。**
★健全性検査を同スクリプトに入れた（既知の道 4 本がすべて `YES`）——
「0 が出た」ではなく「0 が script のバグでない」まで示した形。
★本体が独立に検算: `grep` で `Gained*` の名前は出口側 3 ファイルに **0 件**（整合）。

**★問いの立て直し**: `Gained*` の結論は「**1 つの層 `M/F`（`[M:F]=p`）の中の `x`**」、
出口は「`x : K.closure`（任意の深さ）から `x'` を作る」。⇒ ★**「どちらが安いか」は立たない。
欠けているのは繋ぎ金具。** 実装者が `GainedToWildStep.axWildDescent_of_depth_approx` を作った
（「深さ `k` の元が `K` の元で `c k · ε` 以内に近似できる」だけで `AxWildDescent` が出る）。

**★自己訂正 17 と予防策**: 「26 件 / 16 ファイル」→ ★**14 が正しい**（本体の測定と一致）。
原因は `grep` の出力を**目視で数えた**こと。★実装者が決めた予防:
**数は必ず `| wc -l` の出力そのものを使う。目視で数えない。**
⇒ ★「測っていないを測ったと書く」は本体 1 + 実装者 2 の**計 3 例**。
本体はメモリに保存した（`memory/claimed-verification-not-run.md`）。

**次の 1 点**: `Gained*` 族 1 件を `F := K.carrier`, `M := K(x)`（深さ 1）に代入し、
26 個の仮説のうち何個が `PAdicLocalField` の設定で自動的に満たされるかを**数える**。
...
## 第 1135 —— ★★★塞いでいるのは `hchar` ちょうど 1 本（実装者）

**真偽**: `Gained*` が層ごと（`[M:F] = p`）である理由を測った。
★**桁展開そのものは底まで届く** —— `MinpolyOrbitSplit.lean:528 exists_digitSum_of_adjoin_eq_top` の
`p` は**素数である必要すらなく**、単に `minpoly K π` の次数（★本体も `^theorem` で実在を検算）。
⇒ `[M:K] = p^{k+1}` でも係数は**底の `K`**。

★★★**届かないのはノルムの等式**: 値段を与える `CyclicJumpNorm.lean:404` の仮説
`hchar : ∀ j, 0 < j → j < n → ‖(j : L)‖ = 1`（出どころは同 `:189` の抽象核 `norm_pow_sub_pow_eq`）が
★**`n > p` で偽**（`j := p` を取れば `‖(p:L)‖ < 1`）。
⇒ ★★**層の次数の上限はちょうど `p`**（`LayerDegreeIsP.max_layer_degree`、`IsGreatest` として型に）。
★測定: `hchar` は具体層 **20 箇所すべてが `j < p`**、抽象核の 5 箇所だけ `j < n`（本体も一致）。

**★★予防が実際に重複を止めた初回**: 予防 4 本目（抽象核と自称する節を先に見る）を
**書く前に実行**し、`CyclicJumpNorm.lean:179` が心臓だと分かって `hchar` の出どころを特定
⇒ ★**重複 4 件目を防いだ。**

**★本体の #360 を改訂（2 度目の同じ罠）**: `grep -rn '^theorem …'` が実装者の新ファイルの
★**module docstring 内の ``` ブロック**（`LayerDegreeIsP.lean:18`、namespace は `:85`）を拾った。
⇒ ★**行頭固定だけでは足りない。最初の `^namespace` より後かを見る**（#360 に改訂して記録）。
★どちらの回も報告前に確かめたので誤報は出ていない。

**次の 1 点**: `p ∣ j` の桁を持つ展開でも使えるノルムの**下界**。等号を諦めて
「どの桁が主役か」を選ぶ形 ⇒ ★**第 1120 波台の `j₀` の議論（`MaxMinIndex` / `SlotResidue` /
`MainPartCoeffs`）と同じ形**だと実装者が名指し。★第 1131 で「この道では使わない」と
言われた 5 本のうち、`j₀` 系がここで復活する可能性がある。

**VERDICT / COST**: 本体は今波ゲート＋検算 3 件＋#360 の改訂のみ。

---

## 第 1136 —— 「破れる量」を型にした／★残りは `p ∣ j₀` の扱い 1 つ（実装者）

**在庫の測定**: 木が持つのは 2 つだけで**どちらも `j` に依らない** ——
`CyclicJumpNorm.lean:189 norm_pow_sub_pow_eq`（★等号、`hchar` が要る）と
`:162 norm_pow_succ_sub_pow_succ_le`（粗い上界）。★**中間の形は 0 件**（本体も独立に確認）。

**★埋めたもの**: `DigitLossFactor.norm_pow_sub_pow_le_max`（★抽象核。超距離ノルム体だけで、
分岐・付値・Galois・桁展開の語彙が 1 語も出ず ★**`p` すら出ない**）——
`‖u^{j+2} − π^{j+2}‖ ≤ ‖u−π‖ · max (‖(j+2:L)‖·‖π‖^{j+1}) (‖u−π‖·‖π‖^j)`。
★第 1 項は形式微分、第 2 項は 2 次以上。`‖(n:L)‖ = 1` なら木の粗い上界と一致。
併せて `norm_pow_sub_pow_lt_of_natCast_lt_one`（★`‖(n:L)‖ < 1` なら**必ず**等号が破れる）。
⇒ ★**損失因子は `max (‖(n:L)‖) (‖u−π‖/‖π‖)`** —— 第 1135 の「`hchar` は `n > p` で偽」の**量的な姿**。

**★`j₀` 系との繋がり（実装者が名指し、繋いではいない）**:
`p ∤ j₀` なら §1 の右辺が潰れて主役が残る／★`p ∣ j₀` は §2 により下界が出ない。
⇒ ★**残りは「`p ∣ j₀` の場合の扱い」ちょうど 1 つ。**

**★本体が先に測った点**: `MaxMinIndex.lean:143 not_dvd_max_min_index` は
`1 ≤ j₀ ≤ p−1 ⇒ p ∤ j₀`。⇒ ★**旧設定では `j₀ ∈ [1,p−1]` に制限され `p ∤ j₀` が自動**だったが、
新設定（次数 `p^{k+1}`）では `j₀ ∈ [1, p^{k+1}−1]` なので ★**`p ∣ j₀` が起こりえる**。
★実装者が「記憶ではなく grep で確かめてから書く」と宣言した点を、本体が先に測って渡した。

**次の 1 点**: `p ∣ j₀` のとき `x` が「`π^p` で生成される部分体の元」にどれだけ近いか。
★そこで `wildDepth` が下がるなら `AxWildDescentMulti` の予算がそのまま使える。

**VERDICT / COST**: 本体は今波ゲート＋検算 3 件（#360 改訂版を初めて実運用）＋在庫の測定 1 件。

---

## 第 1137 —— ★★「下界は別の項から出る」—— 第 1135 の自分の否定を覆した（実装者）

**真偽**: 前波の実装者の予定「`x` は `π^p` で生成される部分体の元にどれだけ近いかを測る」→
★**測らなかった。問いを変えた**（自己訂正 20 度目）。代わりに測ったのは
「`p ∣ j₀` でも `‖u^{j₀} − π^{j₀}‖` の**下界が別の項から出るか**」。
★前波 `DigitLossFactor` は「上界がどれだけ落ちるか」、本波はその裏「**下界がどこから出るか**」。

**★★核心**: `(π+δ)^j − π^j = Σ_{k ∈ range j} π^k δ^{j−k} C(j,k)` の
★**`k = 0` の項 `δ^j` は係数 `C(j,0) = 1` で必ず単数**。
★前波の実装者は `k = j−1`（形式微分）の項だけを主役と見ていた。
⇒ ★★★**第 1135 で「下界が出ない」と書いた正準例 `x = π^{p^a}` で下界が出た**
（`Nat.Prime.dvd_choose_pow` で `hdom` が機械的に確認できる。`‖(π+δ)^{p^a} − π^{p^a}‖ = ‖δ‖^{p^a}`）。

**在庫の測定**: 下界の抽象核（`norm_sum_eq_of_dominant`：超距離の加法群だけ）は
★**木にも mathlib にも無かった** —— 木は**付値版** `map_sum_eq_of_lt`（本体の測定では宣言は
`UniformizerExpansion.lean:363`。実装者の挙げた `:74` は docstring 側）、mathlib は
「相異なる」版で ★実装者の「1 つが勝つ」の方が**仮説が弱い**。

**★配管の再発（記録）**: `Nat.pos_pow_of_pos` の不在は `lean-idioms.md` に
★**4 箇所**（本体の測定。実装者は 2 箇所と書いた）あってなお再発。
原因は「書く前に `grep -n <名前> tools/lean-idioms.md` を叩かなかった」。
★`idiom-recur.mjs --similar` が 0 件で外し **grep が当たった**のは 2 度目。
⇒ ★予防: **mathlib の名前を書く前に lean-idioms を grep する。**

**次の 1 点**: `j = p^a·m`（`p ∤ m`, `m ≥ 2`）。Kummer で `p ∤ C(j, p^a)` のはずだが
★**mathlib の Kummer は `C(p^n, k)` の形しか無い**（`emultiplicity_choose_prime_pow`）。
★一般の `C(j, p^a)` の `p` 進付値は未測定。

**VERDICT / COST**: 本体は今波ゲート＋検算 3 件（#360 改訂版で 6 宣言）＋訂正 2 件のみ。

---

## 第 1138 —— ★★前波 §4 は意図した応用で空虚だった（実装者、自己訂正 21 度目）

**真偽**: 第 1137 の §4 `norm_pow_sub_pow_prime_pow` は `hπδ : ‖π‖ ≤ ‖δ‖` を仮定していたが、
★**向きが逆**。木の該当補題はすべて `‖u − π‖ < ‖π‖`（本体が `CyclicJumpNorm.lean:155/:163/:189/:405`
で検算、一致）で、具体層の `hbreak` から `‖δ‖ < ‖π‖` が**常に**従う（`delta_lt_pi_of_break`）。
⇒ ★★**第 1137 の「正準例 `x = π^{p^a}` で下界が出た」は意図した応用では使えない。
本体がユーザーに出した報告も誤り。訂正済み。**
★`norm_sum_eq_of_dominant`（抽象核）と `pow_sub_pow_eq_sum_choose` は**正しいまま**。

**★正しい向きでの主役**: `‖δ‖ < ‖π‖` なら `‖π‖^k‖δ‖^{j−k}` は `k` について狭義単調増加
（`term_lt_of_index_lt`）⇒ 主役は `k = j−1` ＝ ★**形式微分の項**。
⇒ ★これが第 1135 の「`hchar`（`‖(j:L)‖ = 1`）が要る」ことの理由。`p ∣ j` なら次は `k = j−p^a`。

**★★Kummer を 1 段減らした**: `Nat.add_one_mul_choose_eq` から**厳密な等式**
`C(p^a·m, p^a) = m · C(p^a·m−1, p^a−1)`（割り算も付値も使わず、公理は `[propext]` だけ）。
⇒ ★Kummer が要るのは `p ∤ C(p^a·m−1, p^a−1)` だけになった。

**★★索引の「3 通り目の嘘」（#361 に記録）**: `.cache/mathlib-index.txt` は
★**deprecated な名前も載せている**（`Nat.succ_mul_choose_eq`）。索引では deprecated か分からず、
`leanfile.mjs` の warning で気づく。既存は「無いが嘘」(#354)「形が嘘」(#297)。
★`idiom-recur.mjs --similar` が 0 件で外し grep が当たったのは **3 度目**。

**次の 1 点**: `p ∤ C(p^a·m−1, p^a−1)`。★mathlib に Lucas は無い（実装者が測定）。
自分で書くか `Nat.factorization_choose` 系で回るかは未測定。
