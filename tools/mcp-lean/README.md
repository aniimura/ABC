...
| ★**遊んでいる REPL を落とす** | **900 秒**触られなければ | `ABC3_LEAN_IDLE_KILL_SEC` |
...
### ★★★★遊んでいる REPL の回収（2026-08-20 に追加）

★★上の建て直しは **`lean_check` の中でしか走らない**。
したがって、途中から `lake build` に切り替えたセッションでは
**使われない REPL が数 GB を抱えたまま残り続ける**。

★実測（2026-08-20）: `lake build` だけを使っていたセッションの `repl.exe` が
**1.5 GB** を抱えたまま 3 時間放置されていた（並行セッションの方は **18.7 GB**）。

★対処: `IDLE_KILL_MS`（既定 900 秒）触られていなければ、60 秒ごとの見回りで落とす。
`state.envLog` は残るので、次の `lean_check` が import を読み直して再生する。
