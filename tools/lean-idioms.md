...
#### #350 ★小さい場合の偶然の一致 —— 本日 4 例。法則を立てる前に 3 つ目の場合で測る

2026-09-09 の 1 日で、**小さい `p` や小さい `n` でたまたま 2 つの式が一致し、
誤った法則が立った**例が **4 件**出た。いずれも 3 つ目の場合を測って初めて壊れた。

| 誤った法則 | 一致していた理由 | 壊した測定 |
|---|---|---|
| `zeta16_sharp` の `(S,T) = (4,7)` | `min i` と break が `ℚ₃` の 2 行では一致 | `ℚ₂(ζ₁₆)` の break が `[1,3,7]` |
| `loss ≤ 2·i₁` | `p=3` で `2·i₁ = 4` ＝ 実測の最大 | `p = 2` で損失 3 が 14.1% |
| `loss ≤ p+1` | `p=2` で `2p−1`、`p=3` で `2p−2` と**別々に**一致 | `p = 5` で `loss = 8` |
| `t = e_{E₁}` | `e_{E₁} = p^{n−2}(p−1)` が **`n = 3` でだけ** `p(p−1)` と一致 | `p=3,n=4`（18 vs 6） |

**手**: 法則を立てたら**必ず 3 つ目の場合**（`p` を変える／`n` を変える／両方）で測る。
