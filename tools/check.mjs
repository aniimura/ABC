...
const AXIOM_EXEMPT = ['ABC3/Meta/Calibration.lean'];
...
function checkLean() {
  ...
  let axiomCount = 0;
  for (const f of leanFiles) {
    const relf = relative(ROOT, f).replace(/\\/g, '/').replace(/^lean\//, '');
    const exempt = AXIOM_EXEMPT.includes(relf);
    readFileSync(f, 'utf8').split('\n').forEach((line, i) => {
      if (/^\s*axiom\s+/.test(line)) {
        axiomCount++;
        if (!exempt) {
  ...
  console.log(`  -- axiom 宣言: ${axiomCount} 件(較正デモ ${AXIOM_EXEMPT.length} ファイルのみ免除)`);

  const ifaceDir = join(LEAN_SRC, 'Interface');
  const nIface = existsSync(ifaceDir) ? walk(ifaceDir).filter((p) => p.endsWith('.lean')).length : 0;
  console.log(`  -- Interface: ${nIface} ファイル(= Track B の作業キュー)`);
