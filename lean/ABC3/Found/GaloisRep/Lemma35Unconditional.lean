...
| 項 | 原典 | 形式化 | 理由・消費側への影響 |
|---|---|---|---|
| `l` | 任意の素数 | ★**`l ≠ 2` を追加** | Vélu の `±` の対（`veluWFull` の `/2`）が不動点を持たないことを使う。☆**消費側では条件 (a) で自動的**（下記） |
...
### ★★★★★★★★残る `l ≠ 2` がどこまで吸収されるか（実測）

| 枝 | `l` の下界 | 逸脱は吸収されるか |
|---|---|---|
| 条件 (a)（`100·d·(ht^Falt + C·d^ε) ≤ l`） | `l ≥ 100·d ≥ 100` | ★**される**。第 1088 で実際に吸収した（`htFalt_le_of_condA_lcyclic`、外部入力なし） |
| 条件 (b)（`[E_L] ∈ K_V` かつ `l` が局所高さと素） | **`2 ≤ l` だけ** | ☆**されない**。`htFalt_le_of_condB` は `2 ≤ l` で `(†)` を消費する |

★★★★**したがって本定理の `.src` は条つきのままである**（残るのは `l ≠ 2` だけ）。

### ☆`l ≠ 2` を外すには

`l = 2` の Vélu の分岐。`veluU = 0` なので `/2` は無害だが、
`preΨ_2 = 1` なので捩れ点との橋が別物になる（`Ψ₂Sq` の主係数は `4`）。
...
def lemma_3_5_height_ineq.src : Source :=
  { paper := "GenEll", pdfPage := 17,
    item := "Lemma 3.5(外部入力なし。★原典に無い仮説は l ≠ 2 の 1 つだけ——d+1 < l は第 1141 で外れた)",
