...
/-!
# 第 1131 ブロック —— **`p ∣ l` でも Vélu の商は体の上で `E_{q^l}` になる**（`Found`）
...
`exists_vw_tate_mu`（`Skeleton/GenEll/TateIsogeny.lean:1794`）は
...
☆しかし**それは障害ではない**。第 1129-1130 で

    `l⁶·c₄(E_q) + 240·∑ veluV2DF = l¹⁰·c₄(E_{q^l})`
    `l⁸·c₆(E_q) + 504·l²·∑ veluV2DF + 3024·∑(…) = l¹⁴·c₆(E_{q^l})`

が**分母なしで**出ているので、商体 `K` の中で `l⁶`・`l⁸` で割れば

    `c₄(E_q/H) = l⁴·c₄(E_{q^l})`,   `c₆(E_q/H) = l⁶·c₆(E_{q^l})`

になる。★これは「`E_q/H` は `E_{q^l}` を `u = 1/l` で変数変換したもの」という意味であり、
判別式は `Δ(E_q/H) = l¹²·Δ(E_{q^l})` になる。

## ★★★★★★★★なぜ `Δ_min` は変わらないか

`v_p(Δ(E_q/H)) = 12·v_p(l) + v_p(Δ(E_{q^l}))` だが、**`Δ_min` は極小モデルの判別式**なので
`12·v_p(l)` は極小化で消える（`u = 1/l` の変数変換をほどけばよい）。
★したがって `minDeltaExp p E' = minDeltaExp p E_{q^l} = l·minDeltaExp p E` は
`p ∣ l` でも成り立つ。

☆**`p ∤ l` を要求していたのは Vélu の係数を `R` に取る段だけであり、
結論そのものには要らない**——これが節点 5 の設計である。
-/
