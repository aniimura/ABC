...
★★☆★★☆**訂正（2026-09-02、第 1275）**——本ファイルの 2 定理は
**`σA` が恒等射のときしか適用できない**。
☆`tateSetup_algHom_eq_id`（第 1275）が示すとおり、`TateSetup R I K` があると
`K` は `R` の像の分数体になるので `K →ₐ[R] K` は恒等射しかない。
★したがって `tate_point_unipotent` は真だが内容が無く、
`tate_point_exists_ne` は仮説（`σU π = ζπ`）が満たされないので空虚である。
☆正しい形は `tatePhi_map`（在庫、`σR : R →+* R` と `σK : K →+* K` の**対**）から
書き直したもので、曲線を**固定される部分環 `R₀` の上**に置く必要がある。
★第 1272（抽象な `Φ`・`τ`・`hτ`）はこの影響を受けない——そちらが本体である。
