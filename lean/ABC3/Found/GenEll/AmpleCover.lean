...
★★★**アフィン性も持たせてある**（2026-08-28、`§9-915`）——
`IsAmple` の定義はチャートのアフィン性を含んでいるのに、
以前はここで捨てていた。段 E3 の `haff` はこれをそのまま運べば出る。 -/
structure AmpleChart (M : X.PresheafOfModules) where
  ...
  /-- ★非消失軌跡はアフィンである（`IsAmple` の定義が与えるもの）。 -/
  haff : IsAffineOpen (nonVanishing (presheafTensorPow M n) s)
...
/-- ★★**チャートの非消失軌跡はアフィンである**。 -/
theorem AmpleChart.isAffineOpen_open' {M : X.PresheafOfModules} (c : AmpleChart M) :
    IsAffineOpen c.open' := c.haff
...
theorem exists_finite_cover_of_isAmple (M : X.PresheafOfModules) (h : IsAmple M) :
  ...
  have hcover : (Set.univ : Set X) ⊆ ⋃ c : AmpleChart M, ((c.open' : Set X)) := by
    intro x _
    obtain ⟨n, hn, s, hx, hs⟩ := hpt x
    exact Set.mem_iUnion.2 ⟨⟨n, hn, s, hs⟩, hx⟩
