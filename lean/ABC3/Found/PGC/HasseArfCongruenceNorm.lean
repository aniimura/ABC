...
/-! ## §4 ★★検算 —— 木の具体例 2 つと突き合わせる

持ち場の「証拠 4」で指定された 2 例で、`φ` の値を手で計算し、
本ファイルの抽象核 `dvd_sub_of_phi_eq` に**実際に代入**して確かめる。

### (a) `ConcreteNormedModelK1.lean:561 harith_k1`　`p = 2`、`k = 1`、`u = (4, 8)`

`|G| = 4`、`|G_i| = 4 (1 ≤ i ≤ 4)`、`= 2 (5 ≤ i ≤ 8)`、`= 1 (i ≥ 9)` なので

    φ(u₀) = φ(4) = (4·4)/4 = 4,　φ(u₁) = φ(8) = (4·4 + 4·2)/4 = 6.

差は `6 − 4 = 2`、一方 `(u₁−u₀)/p^{0+1} = 4/2 = 2`。★**一致**。

### (b) `JumpFromValueGroup.lean:332 harith_zeta81`　`p = 3`、`k = 2`、`u = (2, 8, 26)`

`|G| = 27`、`|G_i| = 27 (1–2)`、`= 9 (3–8)`、`= 3 (9–26)`、`= 1 (i ≥ 27)` なので

    φ(2) = 54/27 = 2,　φ(8) = (54 + 6·9)/27 = 4,　φ(26) = (54 + 54 + 18·3)/27 = 6.

差は `4−2 = 2 = 6/3^1`、`6−4 = 2 = 18/3^2`。★**両方一致**。

★この表は本ファイルの `card_eq_pow_of_minimal` の予言とも合う:
`|G_{u m}| = p^{k+1−m}` は `27, 9, 3`、`|G_i| = p^{k−m}`（`u m < i ≤ u(m+1)`）は `9, 3`、
そして ★`|G_{u k + 1}| = |G_27| = 1 = p^0`（`m = k+1` の場合）。 -/

section Check

/-- 検算 (a): `p = 2`、`m = 0`、`u = (4, 8)`、`φ = (4, 6)`。 -/
example : ((2 : ℤ) ^ (0 + 1)) ∣ ((8 : ℕ) : ℤ) - ((4 : ℕ) : ℤ) :=
  dvd_sub_of_phi_eq (p := 2) (m := 0) (a := 4) (b := 8) (ja := 4) (jb := 6)
    (x := (4 : ℝ)) (y := (6 : ℝ)) (by norm_num) (by norm_num) (by norm_num) (by norm_num)

/-- 検算 (b1): `p = 3`、`m = 0`、`u = (2, 8, 26)`、`φ = (2, 4, 6)`。 -/
example : ((3 : ℤ) ^ (0 + 1)) ∣ ((8 : ℕ) : ℤ) - ((2 : ℕ) : ℤ) :=
  dvd_sub_of_phi_eq (p := 3) (m := 0) (a := 2) (b := 8) (ja := 2) (jb := 4)
    (x := (2 : ℝ)) (y := (4 : ℝ)) (by norm_num) (by norm_num) (by norm_num) (by norm_num)

/-- 検算 (b2): `p = 3`、`m = 1`。★分母が `3^2 = 9` であることがここで効く。 -/
example : ((3 : ℤ) ^ (1 + 1)) ∣ ((26 : ℕ) : ℤ) - ((8 : ℕ) : ℤ) :=
  dvd_sub_of_phi_eq (p := 3) (m := 1) (a := 8) (b := 26) (ja := 4) (jb := 6)
    (x := (4 : ℝ)) (y := (6 : ℝ)) (by norm_num) (by norm_num) (by norm_num) (by norm_num)

/-- ★**反例の向きの検算** —— 分母を `p^{m+1}` でなく `p` にすると、
(b2) で `φ` が整数にならない。`4 + 18/3 = 10 ≠ 6`。
★すなわち分母の `p^{m+1}` は飾りではない。 -/
example : (4 : ℝ) + ((26 : ℕ) - (8 : ℕ) : ℝ) / (3 : ℝ) ≠ (6 : ℝ) := by norm_num
