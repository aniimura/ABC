...
★★★**残りは `1` 点ではなく `3` 点である**(★数え直した。前の波の「残り 1 点」は
`AxLemmaGraded` を 1 個の命題として数えたもので、この道で埋めるべき部品は 3 個ある):

1. ★`P = (1/p)Tr_{M/E}` の**構成**(`hPsub` / `hPE` / `hPid` / `hPeq`) —— 配管。
2. ★`‖P‖ ≤ p^{max(0, v_M(p) + p − 1 − d_{M/E})/e_M}` —— Serre V §3 Lemme 4 の翻訳。
3. ★★★`d_{M/E}` と上の層の跳び `t` の**トレードオフ**(`t + max(0, v(p) + p − 1 − d) ≤ v(p)` 型)
   —— ★これだけが数学で、`t` が小さいときは `Q` が大きいことを言う。

★1 と 2 は分岐の配管、3 が本体である。★3 が出れば
`towerBudget_le` に代入するだけで `AxLemmaGraded` が閉じる。
