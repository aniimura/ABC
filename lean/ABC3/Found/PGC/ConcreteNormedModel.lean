...
★★この 1 本は本ファイル §QuadCore の `exists_zpow_norm_of_quadratic`
（抽象核）＋ `quadratic_decomp` で埋まり、
★`Found/PGC/ConcreteNormedModelP3.lean` で **`p = 3`, `e = 2` の模型**
（`ℚ₃(ζ₃)((√−3)^{1/3}) / ℚ₃(ζ₃)`、`model3_exists`）が閉じた。
そちらは `(p−1)·u₀ = 6 = p^{k+1}·e` と **`harith` の上界に等号で乗る**ので、
定数勘定を境界で 1 度検算したことになる。
