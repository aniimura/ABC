...
★★ただし**逃げ道は 3 つのうち 2 つまで塞げる**(手計算。★本ファイルでは形式化していない)。
`N := K(x')`、`v(x−x') ≥ 8` とし、`N ∩ M` は `K` か `F = ℚ₃(ζ₉)` のどちらかである
(`M` の `K` 上の部分体で `v₃([N∩M:K]) ≤ 1` を満たすのはこの 2 つだけ)。

* **A. `N ⊇ F` の場合** ⇒ `[N:F] = m` は `3` と互いに素。
  `y₀ := (1/m)·Tr_{N/F}(x')` を取ると `1/m` はノルム 1、`g` は `Gal(M/F)` に落ちるので
  `v(gx − x) ≥ 15 > 8`、したがって `v(x − y₀) ≥ 8` かつ `y₀ ∈ F`。
  ★`d(x,F)` は `v = 7` なので**矛盾**。⇒ A は起きない。
* **B1. `N ∩ M = K` かつ `3 ∤ [N:K]`** ⇒ 同じ平均化を `K` の上で行う
  (`v(gx−x) ≥ 11 > 8`)。`y₀ ∈ K` で `v(x−y₀) ≥ 8` だが `d(x,K)` は `v = 3`。**矛盾**。
* **B2. `N ∩ M = K` かつ `[N:K] = 3m`(`3∤m`)** ⇒ ★**塞げていない**。
  `M` に入らない 3 次拡大の中の元である。★手計算で追うと
  「`(1/3)Tr` の sharp な評価」と「`d(x,N₂) = d(x,K)`」の 2 本からは
  `−1 ≤ v(x−y₀) ≤ 3` しか出ず、★**ちょうど矛盾に届かない**。
  ⇒ 確定させるには `K = ℚ₃(ζ₃)` の 40 個の巡回 3 次拡大(Kummer)の中で
  `d(x,N)` を実際に測る必要がある。★測っていない。
