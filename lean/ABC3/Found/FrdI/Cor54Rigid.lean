...
## ★★前半(関手そのものの一意性)が残っている理由(2026-08-24 の測定)

`Corollary 4.11, (ii)` では `cor_4_11_ii_uniq` が
**`projPrecompIsoGen`**(`Theorem 3.4, (v)` の 1-一意性で作った
「`P.proj` との前合成は関手の同型を反射する」)で片付いた。
★これが効くのは `P.proj : 𝒞 ⥤ 𝒟` が**本質的全射**だからである。

★★**`Corollary 5.4` では同じ道が通らない** —— 縦の矢印
`cToSc : 𝒞 ⥤ 𝒞^rlf` は本質的全射ではない。
`𝒞^rlf` の対象は `(d, α)`(`α ∈ (Φ^rlf)^gp(d)`、**実係数**)で、
`𝒞` から来るのは `α ∈ Φ(d)` の像だけ。
model Frobenioid の同型は `α` を `ℝ·Φ^birat(d)` の分だけしか動かせないので、
実係数の対象は像に入らない。

★★★したがって原文どおり **`Corollary 4.11, (iii)(iv)` の側**
(底の関手 `Ψ_𝒟` と単系の同型 `η` の 1-一意性)から降ろすしかない。
`Ψ^rlf`(`psiSc`)は `(Ψ_𝒟, η)` だけで決まる(`scModelHomOver` の定義)ので、
`(Ψ_𝒟, η)` の 1-一意性がそのまま `Ψ^rlf` の 1-一意性になる。
★**この段は未着手**である。 -/
