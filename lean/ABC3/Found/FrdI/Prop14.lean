...
* **(ii)–(v)** は原文では `𝒞` が **Frobenioid** であることを要求する

★したがって (i) だけは条件を引数に取らない。

## ★★測定結果 —— **我々の証明は原文より強い**(2026-08-15 訂正の記録)

★**(ii)–(v) の証明は `FrobenioidCore` しか使っていない。**
すなわち **`Definition 1.3, (iii), (d)` の圏同値2本を一度も使わない。**

原文は「Suppose that 𝒞 is a **Frobenioid**」と書くが、実際に必要なのは
`FrobenioidCore`(= (iii)(d) 以外の18条)だけである。★したがって本ファイルの
`prop_1_4_ii` 〜 `prop_1_4_v_mpr` は、**原文の主張より仮定が弱い**。

★**一度この docstring は「(ii)–(v) は Frobenioid を要求する」とだけ書いており、
この測定結果を書き落としていた。** 規律「証明が示していないことを docstring に
書かない」は、**逆向き(示したのに書いていない)にも効く**——これで4回目の
食い違いである。
