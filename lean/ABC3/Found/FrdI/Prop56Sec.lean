...
/-! ## ★4. claim の `φ` 側 —— 素数から全次数へ -/

/-- ★素数のところで一致する 2 つの単系準同型 `ℕ≥1 →* M` は一致する。

★★原文の「Since `φ`, `φ'` are homomorphisms, and `N≥1` is generated by `Primes`,
this completes the proof of the claim.」そのもの。 -/
theorem eq_on_pnat_of_primes {M : Type*} [Monoid M] (f g : ℕ+ →* M)
    (h : ∀ p : ℕ+, Nat.Prime (p : ℕ) → f p = g p) (n : ℕ+) : f n = g n := by
  induction n using PNat.strongInductionOn with
  | _ n ih =>
    rcases eq_or_lt_of_le n.one_le with h1 | h1
    · rw [← h1, map_one, map_one]
    · obtain ⟨p, hp, hd⟩ := Nat.exists_prime_and_dvd (n := (n : ℕ))
        (by exact_mod_cast h1.ne')
      obtain ⟨m, hm⟩ := hd
      have hnpos : 0 < (n : ℕ) := n.pos
      rw [hm] at hnpos
      have hmpos : 0 < m := Nat.pos_of_ne_zero (fun h0 => by simp [h0] at hnpos)
      set Mm : ℕ+ := ⟨m, hmpos⟩ with hMm
      set Pp : ℕ+ := ⟨p, hp.pos⟩ with hPp
      have hsplit : n = Pp * Mm := PNat.coe_injective (by exact hm)
      have hlt : Mm < n := by
        refine (PNat.coe_lt_coe Mm n).mp ?_
        show m < (n : ℕ)
        rw [hm]
        calc m = 1 * m := (one_mul m).symm
          _ < p * m := Nat.mul_lt_mul_of_lt_of_le hp.one_lt (le_refl m) hmpos
      rw [hsplit, map_mul, map_mul, h Pp hp, ih Mm hlt]

/-- ★単元による共役は `End A` の単系準同型。 -/
def endConjHom {A : C} {u ui : End A} (hui : u * ui = 1) (hiu : ui * u = 1) :
    End A →* End A where
  toFun x := u * x * ui
  map_one' := by rw [mul_one, hui]
  map_mul' x y := by
    calc u * (x * y) * ui = u * x * (ui * u) * y * ui := by
          rw [hiu]; simp only [mul_assoc, one_mul]
      _ = (u * x * ui) * (u * y * ui) := by simp only [mul_assoc]

/-- ★`ε ↦ ε.app ⟨A, hA⟩` は単系準同型。 -/
def SectionEnd.appHom {S : BaseSection P} {A : C} (hA : S.objP A) : SectionEnd S →* End A where
  toFun ε := ε.app ⟨A, hA⟩
  map_one' := rfl
  map_mul' _ _ := rfl

/-- ★★★**claim の `φ` 側** —— 素数のところで共役が合えば、すべての次数で合う。 -/
theorem prop_5_6_phi_conj {A : C}
    {S S' : BaseSection P} {Fs : ℕ+ →* SectionEnd S} {Fs' : ℕ+ →* SectionEnd S'}
    (hA : S.objP A) (hA' : S'.objP A)
    {u ui : End A} (hui : u * ui = 1) (hiu : ui * u = 1)
    (hconj : ∀ p : ℕ+, Nat.Prime (p : ℕ) →
      u * ((Fs p).app ⟨A, hA⟩) * ui = (Fs' p).app ⟨A, hA'⟩)
    (n : ℕ+) : u * ((Fs n).app ⟨A, hA⟩) * ui = (Fs' n).app ⟨A, hA'⟩ :=
  eq_on_pnat_of_primes
    ((endConjHom hui hiu).comp ((SectionEnd.appHom hA).comp Fs))
    ((SectionEnd.appHom hA').comp Fs') hconj n

/-- ★★★★★**[FrdI] Proposition 5.6 の claim 全体** ——
素数のところで `F` を共役で移す `u ∈ 𝒪^×(A)` が 1 つあれば、
`(σ, φ)` の**両方**がその `u` の共役で移る。

★★これが原文の「it suffices to show the existence of a `u ∈ 𝒪^×(A)` such that
`u · φ_p · u⁻¹ = φ'_p` for all `p ∈ Primes`」の中身である。
★残るのは**その `u` の存在**(unit-profinite 型を使う pro-`p` の議論)。 -/
theorem prop_5_6_claim (htot : IsTotallyEpimorphic C)
    {A : C} (hfn : IsFrobeniusNormalized P A)
    {S S' : BaseSection P} {Fs : ℕ+ →* SectionEnd S} {Fs' : ℕ+ →* SectionEnd S'}
    (hFs : IsFrobeniusSection S Fs)
    (hA : S.objP A) (hA' : S'.objP A)
    ...
    (∀ n : ℕ+, u * ((Fs n).app ⟨A, hA⟩) * ui = (Fs' n).app ⟨A, hA'⟩) ∧
      (∀ (α : (P.toElem.obj A).base ⟶ (P.toElem.obj A).base), IsIso α →
        u * S.sigma hA α * ui = S'.sigma hA' α) :=
  ⟨prop_5_6_phi_conj hA hA' hui hiu hconj,
   fun α hα => by
     haveI := hα
     exact prop_5_6_sigma_conj htot hfn hFs hA hA' huT huiT hui hiu
       (hconj 2 Nat.prime_two) α⟩
