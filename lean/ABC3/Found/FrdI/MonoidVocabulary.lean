...
/-! ### ★★`Φ^pf` の障壁 —— `MChar (Pf M)` の記述（2026-08-16 の分析）

★`Φ^gp` はこの日完成したが、★★**`Φ^pf` は同じ形にならない**。

| | `Φ^gp` | `Φ^pf` |
|---|---|---|
| 条件 (a) 第1成分 | `gpMap_injective` ✓ | `Pf.map_injective` ✓ |
| 条件 (a) 第2成分 | ★**無料**（値が群） | ★★**未実装** |

★**理由**: `Pf M` は群ではないので `MChar (Pf M)` は自明でない。
`charMap (Pf.map f)` の単射性は、★**`Pf N` の可逆元が
`Pf M` の可逆元から来る**ことを要求するが、それは自明ではない。

★★**先に必要なのは `Pf M` の可逆元の特徴づけである**。
`mk m a` が可逆 ⇔ `∃ (m', a'), mk (a'•m + a•m') (a a') = 0`
⇔（`mk_eq_zero_iff` より）`∃ k, k•(a'•m + a•m') = 0`。

★**作業仮説**（未検証）: `mk m a` が可逆 ⇔ `∃ k : ℕ+, IsAddUnit (k • m)`。
★`M = ℕ` では `Pf ℕ = ℚ≥0` で可逆元は `{0}` 、
`∃k, k•n = 0 ⇔ n = 0` と一致するので、★**少なくとも 1 例では合う**。

★★**これは「まだ試していない」ではなく「新しい数学が要る」側である**。
`Φ^gp` では「値が群」という一行で済んだが、ここはそうではない。
★**`Definition 1.1` に残る唯一の実質的な作業である。** -/
