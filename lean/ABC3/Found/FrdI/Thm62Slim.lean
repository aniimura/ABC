...
/-!
# [FrdI] Theorem 6.2, (iv) —— slim 性の 3 条(`Found`、`sorry` 無し)
...
## ★★原文の論証(p.112)を 3 つに割る

原文はこう進む。

1. `L ↔ H`(開部分群)のもとで `Z_G(H) ≃ Aut(𝒟_{Spec L} → 𝒟)`
   …… ★**[Mzk7] `Corollary 1.1.6`**(他論文)。**仮定として受ける**。
2. 「`G` は副有限、したがって特に residually finite なので、
   **formally** に `Z`、`Z_G(H)` も residually finite。ゆえに `Remark 3.1.2` により
   `𝒟` は Frobenius-slim」
3. 「`Z` は `Z_G(H)` の形の部分群の合併なので、**formally** に
   `𝒟` が slim ⟺ `Z = {1}`、`𝒟` が Div-slim ⟺ …」

★★本ファイルは **2 と 3 を `sorry` 無しで閉じる**。1 は他論文の入力なので仮定に置く。

## ★★★「formally」の中身(2026-08-25 に測った)

| 語 | 中身 | どこ |
|---|---|---|
| 「hence, in particular, residually finite」 | 副有限 ⟹ `Group.ResiduallyFinite` | `Profinite.lean`(★mathlib に無かった) |
| 「it follows formally that `Z`, `Z_G(H)` are also」 | 単射準同型に沿って下りる | `Profinite.lean`(★mathlib に無かった) |
| 「by `Remark 3.1.2`」 | `Aut` が residually finite ⟹ Frobenius-slim | `Remark312.lean`(在庫) |
| 「it follows formally that `𝒟` is slim iff `Z = {1}`」 | `Aut ≃* Z_G(H)` の移送 | ★本ファイル |

★★原文の `Z` は `⋃_H Z_G(H)` なので、「`Z = {1}`」は
「すべての開部分群 `H` について `Z_G(H) = {1}`」と同値である。
我々は後者(添字ごと)の形で書く —— 添字 `A : 𝒟` と `H` の対応は
[Mzk7] 側の入力であり、そこを仮定 `Hsub` に押し込んである。
-/
