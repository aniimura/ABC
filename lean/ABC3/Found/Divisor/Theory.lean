...
★この木は**論文ではなく理論**である。`lean/ABC3/<bucket>/Divisor/` に置かれた宣言は、
...
★消費者が 2 論文ある。**理論を論文の下に入れてはならない**ことの実例である。 -/
def theory : ABC3.Meta.Theory :=
  { what := "因子の理論——Weil 因子・Cartier 因子・正規化・算術因子"
    consumers := ["FrdI", "GenEll"]
    mathlibStatus := "スキームの Weil / Cartier 因子は mathlib に部分的にある。算術因子は無い。" }
