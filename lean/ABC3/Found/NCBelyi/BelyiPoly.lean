...
/-!
# [NCBelyi] `ℙ¹` の場合の Belyi 多項式(`Found`)
...
`Theorem 2.5` の証明は
**(1) `Lemma 2.4` で `X = ℙ¹` の場合に帰着し、(2) `Lemma 2.2` で閉じる**。
★本ファイルは **(2) の側、すなわち `ℙ¹` の場合**を取る。

`Lemma 2.2` は `S ⊆ ℙ¹(ℚ)` が条件 (i)(ii)(iii) を満たすことを要求するが、
★**その条件はアフィン変換で常に達成できる**——原文が
『applying an automorphism … and then multiplying by some positive rational number』
と言うところの、`ℙ¹(ℚ)` に閉じた場合である。

| 段 | 中身 |
|---|---|
| `exists_shift_nonneg` | `x ↦ x − min S` で `0 ∈ S` と非負性を作る |
| `exists_beta_large` | `β ≝ 2·max S + 1` で条件 (iii) を満たす `β` を作る |

★★1 次式は臨界点を持たないので、これらの前処理は不分岐性を壊さない。
-/
