...
★この木は**論文ではなく理論**である。`lean/ABC3/<bucket>/Arakelov/` に置かれた宣言は、
...
☆[GenEll] §1 の高さ理論がここに乗る。 -/
def theory : ABC3.Meta.Theory :=
  { what := "Arakelov 幾何——算術次数・Hermite 計量・射影モデル・Faltings 高さ"
    consumers := ["GenEll"]
    mathlibStatus := "mathlib に `Arakelov` は 0 件(2026-08-16 実測)。`LineBundle` も 0 件。" }
