...
/-! ## イデアルへの所属は有限段階へ降りる(`2026-09-05夜、続き16`)

`ψ`・`ψ'`の構成でも、2つの`R`レベル表示を突き合わせる場面でも、
「ℝレベルで成り立つ関係式は、`R`を少し大きくすれば`R`レベルでも
成り立つ」という形の降下が繰り返し要る。`A⊗[ℚ]ℝ = colim_R (A⊗[ℚ]R.1)`
がフィルター余極限であることの、実用形での言い換えである。

鍵は2つの既存部品:(a)`exists_fg_subalgebra_tensor_mvPolynomial_finset`
(有限個の多項式の係数を単一の`R`へ降ろす)、(b)`mvPolynomial_map_
algebraTensorMap_val_injective`(`ℚ`が体なので`A⊗R.1 → A⊗ℝ`は単射)。
(a)で「ℝレベルの線形結合の係数」を降ろし、(b)で「降ろした等式が
実際に`R'`レベルで成り立つ」ことを言う。 -/

open scoped TensorProduct in
/-- `MvPolynomial.map (val R'.1) ∘ MvPolynomial.map (inclusion hR) =
MvPolynomial.map (val R.1)`——`Subalgebra.val R'.1 ∘ Subalgebra.inclusion hR
= Subalgebra.val R.1`が`rfl`であることを`Algebra.TensorProduct.map_comp`と
`MvPolynomial.map_map`で持ち上げるだけ。降下の議論で何度も使う。

★**sorry 無し**。標準3公理のみ。 -/
theorem mvPolynomial_map_val_inclusion_comp (A : Type) [CommRing A] [Algebra ℚ A]
    (R R' : FgSubalgebra ℚ ℝ) (hR : R ≤ R') (ι : Type)
    (y : MvPolynomial ι (A ⊗[ℚ] R.1)) :
    MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.val R'.1)).toRingHom
      (MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.inclusion hR)).toRingHom y) =
    MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.val R.1)).toRingHom y := by
  rw [MvPolynomial.map_map]
  congr 2
  have h : (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.val R'.1)).comp
      (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.inclusion hR)) =
      Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.val R.1) := by
    rw [← Algebra.TensorProduct.map_comp, AlgHom.id_comp]
    rfl
  exact congrArg AlgHom.toRingHom h

open scoped TensorProduct Classical in
/-- **ℝレベルでイデアルに属していれば、`R`を大きくすれば`R'`レベルでも属する**
——`x`と生成元の族`q₀`が`R`レベルにあり、ℝへ底変換した像が
`span(range(q₀のℝレベル像))`に属するなら、ある`R' ≥ R`があって
`R'`レベルでも`x`の像が`span(range(q₀のR'レベル像))`に属する。

証明: (1)`Ideal.mem_span_range_iff_exists_fun`でℝレベルの線形結合
`Σ c k * (q₀ k のℝ像) = x のℝ像`を取り出す。(2)係数`c k`(有限個の多項式)を
`exists_fg_subalgebra_tensor_mvPolynomial_finset`で単一の`R₁`へ降ろす。
(3)`R' := R ⊔ R₁`(`exists_fgSubalgebra_upperBound`)へ両方を送る。
(4)降ろした等式は、ℝへ送ると(1)の等式になるので、
`mvPolynomial_map_algebraTensorMap_val_injective`(単射性)により
`R'`レベルで既に成り立つ。

★**sorry 無し**。標準3公理のみ。 -/
theorem exists_fgSubalgebra_mvPolynomial_ideal_mem_descend (A : Type) [CommRing A] [Algebra ℚ A]
    (R : FgSubalgebra ℚ ℝ) {ι κ : Type} [Fintype κ]
    (q₀ : κ → MvPolynomial ι (A ⊗[ℚ] R.1)) (x : MvPolynomial ι (A ⊗[ℚ] R.1))
    (h : MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.val R.1)).toRingHom x ∈
      Ideal.span (Set.range (fun k => MvPolynomial.map
        (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.val R.1)).toRingHom (q₀ k)))) :
    ∃ (R' : FgSubalgebra ℚ ℝ) (hR : R ≤ R'),
      MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.inclusion hR)).toRingHom x ∈
        Ideal.span (Set.range (fun k => MvPolynomial.map
          (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.inclusion hR)).toRingHom (q₀ k))) := by
  obtain ⟨c, hc⟩ := Ideal.mem_span_range_iff_exists_fun.mp h
  obtain ⟨R₁, hR₁⟩ := exists_fg_subalgebra_tensor_mvPolynomial_finset A
    (Finset.image c (Finset.univ : Finset κ))
  choose c₀ hc₀ using fun k : κ => hR₁ (c k) (Finset.mem_image_of_mem c (Finset.mem_univ k))
  obtain ⟨R', hRle⟩ := exists_fgSubalgebra_upperBound (k := ℚ) (K := ℝ)
    (Finset.univ : Finset (Fin 2)) ![R, R₁]
  have hR : R ≤ R' := hRle 0 (Finset.mem_univ 0)
  have hR1 : R₁ ≤ R' := hRle 1 (Finset.mem_univ 1)
  refine ⟨R', hR, ?_⟩
  rw [Ideal.mem_span_range_iff_exists_fun]
  refine ⟨fun k => MvPolynomial.map
    (Algebra.TensorProduct.map (AlgHom.id ℚ A) (Subalgebra.inclusion hR1)).toRingHom (c₀ k), ?_⟩
  apply mvPolynomial_map_algebraTensorMap_val_injective A R' ι
  rw [map_sum, mvPolynomial_map_val_inclusion_comp A R R' hR ι x]
  rw [← hc]
