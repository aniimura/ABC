...
open scoped Classical in
/-- **`pieceAlgebra_relation_descend_q₀`の定義方程式**——降ろした関係式を
`R.1 ↪ ℝ`で戻すと元の`P.relation k`に一致する。`choose_spec`から`k`番目を
取り出すだけだが、`pieceAlgebra_R_model_baseChange`の中で2度インラインに
書かれていたもの(`在庫`原則)。

**`algebraMap`ではなく`(Algebra.TensorProduct.map (AlgHom.id ℚ A)
(Subalgebra.val R.1)).toRingHom`という明示形で述べてある**のが要点:
`algebraMap`形にすると、これを使う側で`whnf`のヒートビートが尽きる
(`lean-idioms` #65)。

★**sorry 無し**。標準3公理のみ。 -/
theorem pieceAlgebra_relation_descend_q₀_map (X : Over BaseK) (U : X.left.Opens)
    ...
    ∀ k, MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, U))
          (Subalgebra.val (pieceAlgebra_relation_descend_R X U hU C α).1)).toRingHom
        (pieceAlgebra_relation_descend_q₀ X U hU C α k)
      = (Algebra.Presentation.ofFinitePresentation (Γ(X.left, U) ⊗[ℚ] ℝ)
          Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U))).relation k := by
  letI := pieceAlgebra X U hU
  letI : Algebra (Γ(X.left, U) ⊗[ℚ] ℝ) Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U)) :=
    ((Scheme.Hom.appLE α (pullback.fst X.hom toBaseK ⁻¹ᵁ U)
      (α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U)) le_rfl).hom.comp
  ...
  exact ((exists_fg_subalgebra_tensor_mvPolynomial_finset (Γ(X.left, U))
    ...
    ((Algebra.Presentation.ofFinitePresentation (Γ(X.left, U) ⊗[ℚ] ℝ)
      Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U))).relation k)
    (Finset.mem_image_of_mem _ (Finset.mem_univ k))).choose_spec
...
/-! ## `Lemma 4.1`——`descendPieceR`の`R'`レベル局所化のℝへの底変換が
`Γ(C,piece(D(f*g)))`に一致することの完成(`2026-09-05夜、続き6`)

`exists_piece_basicOpen_R_lift`(具体的な`R'`・`p₀`と対応の等式`hp₀`)・
`piece_isLocalization_basicOpen_mul`(`Γ(C,piece(D(f*g)))`が局所化の
実現であること)・`exists_ringEquiv_of_piece_lift`(`FieldLimit.lean`、
純粋代数の合成補題)・`piece_basicOpen_mul_eq`(開集合の等式による
移送)という4つの既存部品を1つの証明に組み立てた——`CorrHyp`固有の
`pieceAlgebra`等の足場を伴うため、`maxHeartbeats`を大幅に上げる必要が
あった(`4000000`では足りず`40000000`で227秒かけて通った)。 -/

set_option maxHeartbeats 40000000 in
open scoped TensorProduct Classical in
/-- **`descendPieceR`の`R'`レベル局所化を`ℝ`へ底変換すると、実際に
`Γ(C,piece(D(f*g)))`そのものになる**——`Lemma 4.1`のGlueData構築に
おける「`R`レベルの局所化と正しいℝレベルの対象を結びつける」核心部分
の完成形。`descendPieceR_localization_isOpenImmersion`(開埋め込み性)
と合わせれば、`GlueData`の`f i j`に要る全データ(開埋め込みであり、
かつℝレベルで正しい対象を指す)が揃う。

★**sorry 無し**。標準3公理のみ。 -/
theorem exists_descendPieceR_localization_baseChange (X : Over BaseK) (U : X.left.Opens) (hU : IsAffineOpen U)
    (f g : Γ(X.left, U)) (C : Scheme) (α : C ⟶ (ExtF.obj X).left) [IsFinite α] [Etale α] :
    letI hUf := hU.basicOpen f
    letI := pieceAlgebra X (X.left.basicOpen f) hUf
    letI : Algebra (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ)
    ...
    letI algB'T : Algebra (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ) :=
      (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, X.left.basicOpen f)) (Subalgebra.val R'.1)).toRingHom.toAlgebra
    letI hCRp : CommRing (MvPolynomial (Fin n) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) ⧸
        Ideal.map (MvPolynomial.map (algebraMap
          (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] (pieceAlgebra_relation_descend_R X (X.left.basicOpen f) hUf C α).1)
          (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1))) I) := inferInstance
    ∀ (M : Type) [CommRing M] [Algebra (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) M]
      [Algebra (MvPolynomial (Fin n) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) ⧸
          Ideal.map (MvPolynomial.map (algebraMap
            (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] (pieceAlgebra_relation_descend_R X (X.left.basicOpen f) hUf C α).1)
            (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1))) I) M]
      [IsScalarTower (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1)
        (MvPolynomial (Fin n) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) ⧸
          Ideal.map (MvPolynomial.map (algebraMap
            (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] (pieceAlgebra_relation_descend_R X (X.left.basicOpen f) hUf C α).1)
            (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1))) I) M]
      [IsLocalization.Away (Ideal.Quotient.mk (Ideal.map (MvPolynomial.map (algebraMap
          (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] (pieceAlgebra_relation_descend_R X (X.left.basicOpen f) hUf C α).1)
          (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1))) I) p₀) M],
    Nonempty (M ⊗[Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1] (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ)
  ...
  set R := pieceAlgebra_relation_descend_R X (X.left.basicOpen f) hUf C α with hRdef
  letI hCR : CommRing (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R.1) := inferInstance
  letI algBT : Algebra (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R.1) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ) :=
    (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, X.left.basicOpen f)) (Subalgebra.val R.1)).toRingHom.toAlgebra
  set n := Fin (Algebra.Presentation.ofFinitePresentationVars (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ)
    Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ (X.left.basicOpen f)))) with hndef
  obtain ⟨e⟩ := pieceAlgebra_R_model_baseChange X (X.left.basicOpen f) hUf C α
  set h₂ := piece_basicOpen_localizationElem X U f g C α with hh₂def
  set I := Ideal.span (Set.range (pieceAlgebra_relation_descend_q₀ X (X.left.basicOpen f) hUf C α)) with hIdef
  obtain ⟨R', hR, p₀, hp₀⟩ := exists_piece_basicOpen_R_lift X U hU f g C α e
  haveI hWloc : IsLocalization.Away h₂ Γ(C, C.basicOpen h₂) :=
    piece_isLocalization_basicOpen_mul X U hU f g C α
  refine ⟨R', hR, p₀, ?_⟩
  letI hCR' : CommRing (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) := inferInstance
  letI algBB' : Algebra (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R.1) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) :=
    (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, X.left.basicOpen f)) (Subalgebra.inclusion hR)).toRingHom.toAlgebra
  letI algB'T : Algebra (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ) :=
    (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, X.left.basicOpen f)) (Subalgebra.val R'.1)).toRingHom.toAlgebra
  haveI hTower : IsScalarTower (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R.1) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1)
      (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ) := by
    apply IsScalarTower.of_algebraMap_eq
    intro x
    have h1 : (Subalgebra.val R'.1).comp (Subalgebra.inclusion hR) = Subalgebra.val R.1 := rfl
    have h2 := Algebra.TensorProduct.map_comp (S := ℚ) (R := ℚ) (AlgHom.id ℚ Γ(X.left, X.left.basicOpen f))
      (AlgHom.id ℚ Γ(X.left, X.left.basicOpen f)) (Subalgebra.val R'.1) (Subalgebra.inclusion hR)
    rw [AlgHom.id_comp, h1] at h2
    exact RingHom.congr_fun (congrArg AlgHom.toRingHom h2.symm).symm x
  intro M _ _ _ _ _
  obtain ⟨e8⟩ := exists_ringEquiv_of_piece_lift
    (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R.1) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] R'.1) (Γ(X.left, X.left.basicOpen f) ⊗[ℚ] ℝ)
    n I p₀ Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ (X.left.basicOpen f))) Γ(C, C.basicOpen h₂) e h₂ hp₀ M
...
open scoped Classical in
set_option maxHeartbeats 1000000 in
/-- ★**`Lemma 4.1`の`f i j`を降ろすための仮説`hψ`——具体データで検証済み**★

`exists_mvPolynomial_quotient_ringHom_descend2_of_map`(`FieldLimit.lean`)を
`A := Γ(X.left,U)`・`A' := Γ(X.left,V)`・`φ := pieceRestrictAlgHom`・
`R := pieceAlgebra_relation_descend_R X U hU C α`・`R₂ := 同 V`・
`q := pieceAlgebra_relation_descend_q₀ X U hU C α`・`q₂ := 同 V`・
`ψ i := P_V.σ (制限 (P_U.val i))` で呼ぶときの仮説そのもの。

**組み立て**(3つの部品の合成):
1. `tensorProduct_map_split`で`φ ⊗ val R`を`(φ ⊗ id) ∘ (id ⊗ val R)`に分け、
   後半に`pieceAlgebra_relation_descend_q₀_map`を当てて
   `map (φ ⊗ id) (P_U.relation k)`にする(`h1`)。
2. `V`側も同じ定義方程式で`span (range q₂の像) = span (range P_V.relation)`
   に直す(`h2`)。
3. あとは`aeval_map_relation_mem_span`に
   `hcomm := pieceAlgebraMap_comp_naturality`を食わせるだけ。

**配管の注意**: `h1`は`rw`で書くと`whnf`のヒートビートが尽きる
(4000000でも足りない)。`congrArg` + `Eq.trans`の**項で組む**と2秒で通る
(`lean-idioms` #65)。

★**sorry 無し**。標準3公理のみ。 -/
theorem descendPieceR_hψ (X : Over BaseK) (U V : X.left.Opens)
    (hU : IsAffineOpen U) (hV : IsAffineOpen V) (hVU : V ≤ U)
    (C : Scheme) (α : C ⟶ (ExtF.obj X).left) [IsFinite α] [Etale α] :
    letI := pieceAlgebra X U hU
    letI := pieceAlgebra X V hV
    ...
    ∀ k, MvPolynomial.aeval
        ...
        (MvPolynomial.map (Algebra.TensorProduct.map (pieceRestrictAlgHom X U V hU hV hVU)
            (Subalgebra.val (pieceAlgebra_relation_descend_R X U hU C α).1)).toRingHom
          (pieceAlgebra_relation_descend_q₀ X U hU C α k))
      ∈ Ideal.span (Set.range (fun k' => MvPolynomial.map
          (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, V))
            (Subalgebra.val (pieceAlgebra_relation_descend_R X V hV C α).1)).toRingHom
  ...
  haveI : Algebra.FinitePresentation (Γ(X.left, V) ⊗[ℚ] ℝ)
      Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ V)) := inferInstance
  have hqU := pieceAlgebra_relation_descend_q₀_map X U hU C α
  have hqV := pieceAlgebra_relation_descend_q₀_map X V hV C α
  have hsplit := tensorProduct_map_split (R := ℚ) (pieceRestrictAlgHom X U V hU hV hVU)
      (Subalgebra.val (pieceAlgebra_relation_descend_R X U hU C α).1)
  have h1 : ∀ k, MvPolynomial.map (Algebra.TensorProduct.map (pieceRestrictAlgHom X U V hU hV hVU)
        (Subalgebra.val (pieceAlgebra_relation_descend_R X U hU C α).1)).toRingHom
        (pieceAlgebra_relation_descend_q₀ X U hU C α k)
      = MvPolynomial.map (Algebra.TensorProduct.map (pieceRestrictAlgHom X U V hU hV hVU)
          (AlgHom.id ℚ ℝ)).toRingHom
          ((Algebra.Presentation.ofFinitePresentation (Γ(X.left, U) ⊗[ℚ] ℝ)
            Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U))).relation k) := fun k =>
    Eq.trans (congrArg (fun t : (Γ(X.left, U) ⊗[ℚ] (pieceAlgebra_relation_descend_R X U hU C α).1)
        →ₐ[ℚ] (Γ(X.left, V) ⊗[ℚ] ℝ) => MvPolynomial.map t.toRingHom
        (pieceAlgebra_relation_descend_q₀ X U hU C α k)) hsplit)
      (Eq.trans (MvPolynomial.map_map _ _ _).symm
        (congrArg (MvPolynomial.map (Algebra.TensorProduct.map
          (pieceRestrictAlgHom X U V hU hV hVU) (AlgHom.id ℚ ℝ)).toRingHom) (hqU k)))
  have h2 : Ideal.span (Set.range (fun k' => MvPolynomial.map
        (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, V))
          (Subalgebra.val (pieceAlgebra_relation_descend_R X V hV C α).1)).toRingHom
        (pieceAlgebra_relation_descend_q₀ X V hV C α k')))
      = Ideal.span (Set.range (Algebra.Presentation.ofFinitePresentation (Γ(X.left, V) ⊗[ℚ] ℝ)
          Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ V))).relation) :=
    congrArg (fun s => Ideal.span (Set.range s)) (funext hqV)
  intro k
  have hmain := aeval_map_relation_mem_span
    (Algebra.Presentation.ofFinitePresentation (Γ(X.left, U) ⊗[ℚ] ℝ)
      Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U)))
    (Algebra.Presentation.ofFinitePresentation (Γ(X.left, V) ⊗[ℚ] ℝ)
      Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ V)))
    (Algebra.TensorProduct.map (pieceRestrictAlgHom X U V hU hV hVU) (AlgHom.id ℚ ℝ)).toRingHom
    (C.presheaf.map (homOfLE (piece_le_of_le X U V hVU C α)).op).hom
    (pieceAlgebraMap_comp_naturality X U V hU hV hVU C α) k
  exact h2 ▸ (congrArg (MvPolynomial.aeval
    (fun i => (Algebra.Presentation.ofFinitePresentation (Γ(X.left, V) ⊗[ℚ] ℝ)
        Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ V))).σ
      ((C.presheaf.map (homOfLE (piece_le_of_le X U V hVU C α)).op).hom
        ((Algebra.Presentation.ofFinitePresentation (Γ(X.left, U) ⊗[ℚ] ℝ)
...
open scoped Classical in
set_option maxHeartbeats 1000000 in
/-- ★**`GlueData`が要求する形**——「ある閾値`R₀`があって、**それ以上の
すべての`R'`について**`f i j`が存在する」。

`exists_descendPieceR_ringHom`は対`(U,V)`ごとに自分の`R'`を返すので、
`GlueData`(全添字対に**共通の1つ**の`R'`を要求する)には直接使えない。
この`_ge`版なら、有限個の対の閾値`R₀`たちを
`exists_fgSubalgebra_upperBound`(既存)で1つにまとめ、その共通段階で
**すべての対の`f i j`を同時に**取れる。

証明は`exists_descendPieceR_ringHom`と同じで、`descend2_of_map`の
生データ`(ev, hrel)`を`descend2_relation_promote`(`FieldLimit.lean`)で
`R'`へ持ち上げてから`mvPolynomial_quotient_ringHom_of_data`に渡すだけ。

★**sorry 無し**。標準3公理のみ。 -/
theorem exists_descendPieceR_ringHom_ge (X : Over BaseK) (U V : X.left.Opens)
    (hU : IsAffineOpen U) (hV : IsAffineOpen V) (hVU : V ≤ U)
    (C : Scheme) (α : C ⟶ (ExtF.obj X).left) [IsFinite α] [Etale α] :
    letI := pieceAlgebra X U hU
    letI := pieceAlgebra X V hV
    ...
    ∃ (R₀ : FgSubalgebra ℚ ℝ)
      ...
      Nonempty (
        (MvPolynomial (Fin (Algebra.Presentation.ofFinitePresentationVars (Γ(X.left, U) ⊗[ℚ] ℝ)
          ...
          Ideal.span (Set.range (fun k => MvPolynomial.map
            (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, U))
              (Subalgebra.inclusion (h₀U.trans h))).toRingHom
            (pieceAlgebra_relation_descend_q₀ X U hU C α k))))
        →+*
        (MvPolynomial (Fin (Algebra.Presentation.ofFinitePresentationVars (Γ(X.left, V) ⊗[ℚ] ℝ)
            Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ V)))) (Γ(X.left, V) ⊗[ℚ] R'.1) ⧸
          Ideal.span (Set.range (fun k' => MvPolynomial.map
            (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, V))
              (Subalgebra.inclusion (h₀V.trans h))).toRingHom
            (pieceAlgebra_relation_descend_q₀ X V hV C α k'))))) := by
  letI := pieceAlgebra X U hU
  letI := pieceAlgebra X V hV
  letI algU : Algebra (Γ(X.left, U) ⊗[ℚ] ℝ) Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U)) :=
  ...
  haveI : Algebra.FinitePresentation (Γ(X.left, V) ⊗[ℚ] ℝ)
      Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ V)) := inferInstance
  obtain ⟨R₀, h₀U, h₀V, ev, hev, hrel⟩ := exists_mvPolynomial_quotient_ringHom_descend2_of_map
    (Γ(X.left, U)) (Γ(X.left, V)) (pieceRestrictAlgHom X U V hU hV hVU)
    (pieceAlgebra_relation_descend_R X U hU C α) (pieceAlgebra_relation_descend_R X V hV C α)
    (pieceAlgebra_relation_descend_q₀ X U hU C α) (pieceAlgebra_relation_descend_q₀ X V hV C α)
    (fun i => (Algebra.Presentation.ofFinitePresentation (Γ(X.left, V) ⊗[ℚ] ℝ)
      ...
      ((C.presheaf.map (homOfLE (piece_le_of_le X U V hVU C α)).op).hom
        ((Algebra.Presentation.ofFinitePresentation (Γ(X.left, U) ⊗[ℚ] ℝ)
          Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U))).val i)))
    (descendPieceR_hψ X U V hU hV hVU C α)
  refine ⟨R₀, h₀U, h₀V, fun R' h => ⟨mvPolynomial_quotient_ringHom_of_data
    (Algebra.TensorProduct.map (pieceRestrictAlgHom X U V hU hV hVU)
      (AlgHom.id ℚ R'.1)).toRingHom
    (fun k => MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, U))
      (Subalgebra.inclusion (h₀U.trans h))).toRingHom
    ...
    (fun k' => MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, V))
      (Subalgebra.inclusion (h₀V.trans h))).toRingHom
      (pieceAlgebra_relation_descend_q₀ X V hV C α k'))
    (fun i => MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, V))
      (Subalgebra.inclusion h)).toRingHom (ev i))
    (fun k => Eq.mpr (congrArg (fun x => x ∈ Ideal.span (Set.range (fun k' =>
        MvPolynomial.map (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, V))
          (Subalgebra.inclusion (h₀V.trans h))).toRingHom
        (pieceAlgebra_relation_descend_q₀ X V hV C α k'))))
      (congrArg (MvPolynomial.aeval (fun i => MvPolynomial.map
          (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, V))
            (Subalgebra.inclusion h)).toRingHom (ev i)))
        (mvPolynomial_map_tensorMap_split (pieceRestrictAlgHom X U V hU hV hVU)
          (Subalgebra.inclusion (h₀U.trans h))
          (pieceAlgebra_relation_descend_q₀ X U hU C α k))))
      (descend2_relation_promote (Γ(X.left, U)) (Γ(X.left, V))
        (pieceRestrictAlgHom X U V hU hV hVU) h₀U h₀V h
        (pieceAlgebra_relation_descend_q₀ X U hU C α)
        (pieceAlgebra_relation_descend_q₀ X V hV C α) ev hrel k))⟩⟩
...
set_option maxHeartbeats 1000000 in
set_option synthInstance.maxHeartbeats 800000 in
/-- ★**`descendPieceRModel`(新記法、`span`形)と、`f_open`側で使われている
`Ideal.map … I`形が同じ対象であること**——`ideal_map_mvPolynomial_span_range`
(`FieldLimit.lean`)を`congrArg`で商へ送るだけ。

これで`descendPieceR_localization_isOpenImmersion`・
`exists_descendPieceR_localization_baseChange`(いずれも`Ideal.map … I`の形で
述べられている既存の結果)を、続き45以降の軽い記法`descendPieceRModel`の
上に載せられる。

★**sorry 無し**。標準3公理のみ。 -/
theorem descendPieceRModel_eq_quotient_map (X : Over BaseK) (U : X.left.Opens)
    (hU : IsAffineOpen U) (C : Scheme) (α : C ⟶ (ExtF.obj X).left) [IsFinite α] [Etale α]
    (R' : FgSubalgebra ℚ ℝ) (h : pieceAlgebra_relation_descend_R X U hU C α ≤ R') :
    letI := pieceAlgebra X U hU
    letI algU : Algebra (Γ(X.left, U) ⊗[ℚ] ℝ) Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U)) :=
  ...
  letI algRR' : Algebra (Γ(X.left, U) ⊗[ℚ] (pieceAlgebra_relation_descend_R X U hU C α).1)
    ...
    (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, U))
      (Subalgebra.inclusion h)).toRingHom.toAlgebra
  exact congrArg (fun I => MvPolynomial (Fin (Algebra.Presentation.ofFinitePresentationVars
      (Γ(X.left, U) ⊗[ℚ] ℝ) Γ(C, α ⁻¹ᵁ (pullback.fst X.hom toBaseK ⁻¹ᵁ U))))
      (Γ(X.left, U) ⊗[ℚ] R'.1) ⧸ I)
    (ideal_map_mvPolynomial_span_range (Algebra.TensorProduct.map (AlgHom.id ℚ Γ(X.left, U))
      (Subalgebra.inclusion h)).toRingHom
