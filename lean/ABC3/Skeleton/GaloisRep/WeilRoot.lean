...
後続の証明に影響は出ない。 -/
theorem exists_nthRoot_comp_mulByN [IsAlgClosed F] (W : WeierstrassCurve.Affine F)
    [W.IsElliptic] {x y : F}
  ...
  obtain ⟨J, hJ⟩ := exists_fractionalIdeal_pow W h n hn hP μ hns hμP hμx hμy fP hfP
  obtain ⟨g, hg⟩ := fractionalIdeal_isPrincipal W h n hn hP μ hns hμP hμx hμy fP hfP J hJ
