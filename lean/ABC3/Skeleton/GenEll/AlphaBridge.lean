...
| II | **`α` が mod `l` 像に入ること** | ☆**本ファイル**（進捗枠 **2.6 / 3**、残り重み 5） |
...
☆総重み 22 → **5**（第 1161-1165 で節点 1、第 1167-1169 で節点 3 が閉じた）。
★★**残るのは節点 2 だけ**——Tate 一意化で `(ζ, π)` を `E[l]` の基底として実現する段である。
...
## ★★★★★★何が `Theorem 3.8` に残るか（第 1160 の総括）
    ...
    I（`l`-巡回の読み、残り 4） ＋ II（本ファイル、残り 5） ＝ **9**
