...
| II | **`α` が mod `l` 像に入ること** | ☆**本ファイル**（進捗枠 **2.8 / 3**、残り重み 2） |
...
| 2 | `alpha_mem_localImage` | ★**座標の側（第 1164）と群論の側（第 1170 `exists_zeta_pi_of_torsion`）は揃った**。☆残るのは `Φ`（Tate 一意化）を介して `E[l]` の言葉に直す配管だけ | 8 → **2** |
...
☆総重み 22 → **2**（第 1161-1165 で節点 1、第 1167-1169 で節点 3、
...
## ★★★★★★何が `Theorem 3.8` に残るか（第 1160 の総括）
    ...
    I（`l`-巡回の読み、残り 4） ＋ II（本ファイル、残り 2） ＝ **6**
