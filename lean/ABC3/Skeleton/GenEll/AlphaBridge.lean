...
| II | **`α` が mod `l` 像に入ること** | ☆**本ファイル**（進捗枠 **0.6 / 3**、残り重み 17） |
...
| # | 節点 | 内容 | 重み |
|---|---|---|---|
| 1 | `tateTorsion_basis_zeta_pi` | ★**独立性の側は第 1161 で取れた**（`zeta_pi_indep`）。☆残るのは全射の側（`E[l]` が実際にこの 2 元で生成されること）と `Q` の無限位数の紐付け | 8 → **3** |
...
## ★★★★★★何が `Theorem 3.8` に残るか（第 1160 の総括）
    ...
    I（`l`-巡回の読み、残り 5） ＋ II（本ファイル、残り 17） ＝ **22**
