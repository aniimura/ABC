...
### ★★★★★★★★★★★★★★★★★★★★★★★★★★★★第 1318-1321——II 側は入口から出口まで繋がった

| 定理 | 内容 | 第 |
|---|---|---|
| `local_inputs_of_bad_prime` | ★★悪い素点の言葉で局所の 2 入力 | 1318 |
| `exists_h2_h1_of_localData′` | ★★`ζ` が局所体にある版の橋 | 1319 |
| `exists_h2_h1_of_bad_prime` | ★★★★**悪い素点 → 大域の `h2`・`h1`** | 1320 |
| `imageContainsSL2J_of_h2_h1` | ★★★`h2`・`h1` → `ImageContainsSL2J` | 1321 |

☆**入口から出口まで**:

    悪い素点（`jExp p E < 0`・`¬ l ∣ jExp p E`・分裂乗法還元・`ζ ∈ L_v`）
      → 第 1320（大域の `h2`・`h1` を満たす `σ`）
      → 第 1321（`ImageContainsSL2J`）

### ★★★★★★★★残るのは局所体の帳簿 2 つ

| # | 残り | 見立て |
|---|---|---|
| i | 悪い素点で**分裂**乗法還元を用意する | 非分裂なら不分岐 2 次捻り（第 1007-1029 の在庫） |
| iii | `ζ ∈ L_v` にする（素点を `L(ζ_l)` の上で取る）と付値の橋 `hp` | `l` は `p` の剰余標数でない（`ζ` が原始 `l` 乗根であることに含まれる）。分岐指数 `e` は `l−1` を割るので `l ∤ e`——`hp` の一般化（`valuation_algebraMap_ext`、第 1029）で吸収できる |

★★**未知の数学は無い**——どちらも在庫の呼び出しと帳簿である。
