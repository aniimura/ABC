...
| II | **`α` が mod `l` 像に入ること** | ☆**本ファイル**（進捗枠 **1.3 / 3**、残り重み 12） |
...
| # | 節点 | 内容 | 重み |
|---|---|---|---|
| 1 | `tateTorsion_basis_zeta_pi` | ★**核はちょうど `lℤ × lℤ`（第 1161-1162、`zeta_pi_mem_zpowers_iff`）**。☆残るのは全射の側（`E[l]` が実際にこの 2 元で生成されること）と `Q` の無限位数の紐付け | 8 → **1** |
...
☆総重み 22 → **12**（第 1161-1164 で**座標の側が全部揃った**）。
...
☆**これで座標の側は全部揃った**。残るのは Tate 一意化で
`(ζ, π)` を `E[l]` の基底として実現する段と、分解群経由で大域へ移す段だけである。
...
## ★★★★★★何が `Theorem 3.8` に残るか（第 1160 の総括）
    ...
    I（`l`-巡回の読み、残り 5） ＋ II（本ファイル、残り 12） ＝ **17**
