...
☆`imageContainsSL2J_torsionExt` は `imageContainsSL2J_of_h2_h1`（第 1321）と
`exists_h2_h1_of_bad_prime`（第 1320）で**一本道になっている**。
★残るのは悪い素点 `p` で**局所のデータを作る 3 枚**だけである:

| 枚 | 必要なもの | 状態 |
|---|---|---|
| (i) | `IsMinimal R (E ⊗ L_v)`（完備体の上の極小モデル） | ☆`SemistableAt p E` から作る配管 |
| (ii) | `HasSplitMultiplicativeReduction`（**分裂**） | ☆非分岐二次捧りの段 |
| (iii) | `ζ : L_vˣ` が原始 `l` 乗根 | ☆`L_v(ζ_l)/L_v` は `p ∤ l` なら非分岐 |

★★(iii) の道は 2 つある——`L` を `L(ζ_l)` に取り直す（`Gal(L̄/L(ζ_l)) ⊆ Gal(L̄/L)` なので
像の `SL₂` 包含はそのまま伝わる）か、`hp` を分岐指数分だけ緩めるかである。
☆どちらも局所の配管であり、**未証明の外部引用は 1 本もない**。
