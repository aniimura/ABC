...
| # | 内容 | 状態 |
|---|---|---|
| 1 | **組み立て**（悪い素点の局所パッケージ、良い素点の極小モデルへの正規化） | ☆残 |
| 2 | **形式群の鋭い捩れ限界** `(l−1)·k ≤ v_p(l)`（`p ∣ l` の場合） | ☆残 |

★在庫は粗い版 `k ≤ v_p(l)`（第 1077）だけ。
鋭い版があれば原文の条件 `[L:ℚ]+1 < l` から `k = 0`（核に深い点は無い）が出て、
`p ∣ l` は `p ∤ l` と**同じ形**（第 1385 ＋ 第 1386）に落ちる。
-/
