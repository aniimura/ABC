...
| 段 | 状態 |
|---|---|
| `c₄`・`c₆`・`Δ` の関係（商体） | ★第 1131 で**証明済み**（`hlu` なし） |
| `minDeltaExp_eq_mul_at_bad_prime`（第 1000）の `hvw` の口を付け替える | ☆残り（配管、10-20 ブロック） |

☆`hvw` の現在の口は「`R` の `v`・`w` で `v = ∑veluV2` かつ `2w = ∑(…)`」である。
★これを「`K` の `v`・`w` ＋ `c₄`・`c₆` の関係」に付け替えれば `hlu` が落ちる。
