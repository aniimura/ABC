...
##### ★★★★★★★★★★★★★★★★第 1206-1208——降ろす段と最後の仮説

| 定理 | 内容 | 第 |
|---|---|---|
| `exists_rhPoint_eq` | ★座標が `f` の像に入る点は `rhPoint f` の像 | 1206 |
| `castPoint` 他 | ★曲線の等式に沿った点の輸送 | 1206 |
| `exists_finite_point_descent` | ★★★`L̄` の位数 `l` の点は**有限次拡大で有理**になる | 1207 |
| `not_dvd_jExp_baseChange` | ★★分岐指数が `l` と素なら `l ∤ v_P(j)` は保たれる | 1208 |

★第 1207 の座標集合の等式は、第 1197
（`veluQuotientFull_descends_to_intermediate`）が受け取る `hT` の形そのものである。
☆`NumberField M` は mathlib の `NumberField.of_module_finite` がそのまま与える
（2026-09-02 実測、自前に書かなくてよい）。

##### ★★★★見えている義務は 1 つ——**`l ∤ e(P|p)`**
...
☆第 1208 で `l ∤ e(P|p)` さえあれば出ることになった。
...
| 定理 | 内容 | 第 |
|---|---|---|
| `nsmul_nsmul_mem_image_of_not_dvd` | ★★`⟨Q⟩∖{O}` は `Q ↦ c·Q`（`l ∤ c`）で保たれる（群論の核） | 1213 |
| `finrank_le_of_algHom_mem_finset` | ★★共役の個数で次数を上から抑える | 1214 |

☆第 1214 は標数 0（分離的）なら `#(M →ₐ[L] L̄) = [M:L]`（mathlib の `AlgHom.card`）
を使う——**Galois 群を直接使わない形**で数え上げを用意した。

##### ★★★★★★★★★★★★★★★★★★★★第 1218——**`[M:L] ≤ l−1` が出た**

`finrank_adjoin_ptCoordSet_le`（`Found/GenEll/DegreeLine.lean`）:

    安定直線なら [L(x_Q, y_Q) : L] ≤ l − 1
