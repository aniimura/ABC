...
```
と定める(`Base`・`Div`・`degFr` は `G` 成分を無視する)。すると:

- **`Frobenius-normalized` が自動で成り立つ**: `α = (𝟙, a)`(次数 1)、
  `ζ = (z, 1)`(次数 `d`)とすると
  `ζ ≫ α^d = (z, 1^1 · a^d) = (z, a^d)`、
  `α ≫ ζ = (z, a^d · 1) = (z, a^d)` ★**一致する**
- ★★**`ζ` は mono でない**: `(f₀, u) ≫ (z, 1) = (f₀ ≫ z, u^d)` なので、
  `u^d = v^d` から `u = v` は出ない。★**`G = ℤ/p`、`d = p` なら
  すべての `u` で `u^p = 1` となり、`ζ` は全く mono でない**
- `𝒪^×(A) ≅ G` なので `p`-捻れがある ✓
- `faithfulUpToUnits` は成り立つ(不変量が同じ 2 射は `G` 成分だけ違う)✓
- `preStepMono` は成り立つ(pre-step は次数 1 なので `u^1 = v^1` ⟹ `u = v`)✓

★★**したがって残る仕事は「この捻れ積が `Definition 1.3` の全条件を満たす」ことの
検証である。** ★**それが済めば分類は ③(`sourceGap`)に上がる。**

★**`𝔽_Φ` が Frobenioid であることは `Proposition 1.5` で既に証明してある**ので、
捻れ積の各条件は `𝔽_Φ` の対応する条件に `G` 成分の処理を足す形になると見ている
(★未検証)。
-/
