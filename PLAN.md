...
### ★★未着手の塊の下流を測る —— `node tools/frdi-blocked.mjs`(2026-08-18 導入)

★★**まだ閉じていない 3 つの塊の下流に、何件あるかを機械で測る。**
そこから依存を上流へ伝播させると、節ごとの「いまチェーン待ちの数」が出る。

```
★[FrdI] 残り 24 件のうち   3 つのチェーンの下流 13 件 / 単独で閉じる 11 件

  §1  現在 13/15   チェーンに依らず届く 15/15
  §2  現在  7/8    チェーンに依らず届く  7/8   (チェーン待ち 1)
  §3  現在  7/9    チェーンに依らず届く  9/9
  §4  現在  3/10   チェーンに依らず届く  5/10  (チェーン待ち 5)
  §5  現在  0/7    チェーンに依らず届く  2/7   (チェーン待ち 5)
  §6  現在  0/5    チェーンに依らず届く  3/5   (チェーン待ち 2)
```

★★**この表は到達不能の証明ではない。** 3 つの塊はいずれも既知数学であり、
下の「塊を割って葉を出す」に分解した DAG がある。

★★★**この節の「壁」という読み方は 2026-08-18 夕に撤回した。次節を見ること。**

| # | 塊 | 下流の数 | 割った先 |
|---|---|---|---|
| 1 | 副有限アーベル群の **pro-`l` 分解**(`Definition 2.8, (ii)`) | §2 の最後の 1 件 | チェーン `prol`(7 節点・4 層・葉 4) |
| 2 | **`Proposition 4.4, (ii)`**(`otriBase` ⟺ `𝒪^▷(A^birat)` の可換性) | §4 で 5 件・§5 で 5 件 | チェーン `otricomm`(7 節点・5 層、★葉 2 は 2026-08-18 に閉じた) |
| 3 | **six exponentials theorem**(`Lemma 6.5, (ii)`) | §6 で 2 件 | チェーン `sixexp`(8 節点・6 層・葉 3) |

★**塊 2 が最大の律速**(11 件)。ここが閉じると §4 は 10/10、§5 は 7/7 まで開く。

### ★★塊を割って葉を出す —— `node tools/frdi-newleaves.mjs`(2026-08-18 導入)

★★**CLAUDE.md の姿勢**——「工数の山を『壁』と呼ばない。既知数学の person-years は
壁でなく道」——に従い、上の 3 つを**内部の小目標の DAG に割った**。
台帳は `ResearchPaper/frdi-decomposition.json`(22 節点)、印字は

```
node tools/frdi-newleaves.mjs
```

> ★**実測(2026-08-18)**: 3 チェーンで **9 個の新しい葉**が出た。
> うち **2 個(`ore-square`・`divgp-hom`)は同日に閉じた**。

★★★**割ってみると、在庫の測定が 2 件覆った**——これが「割る」ことの実利である:

1. **six exponentials の在庫**。2026-08-17 に「mathlib の `Analysis/Transcendental/` には
   Liouville 数と `e` の超越性しか無い」と書いていたが、
   ★**mathlib に `Analysis/Transcendental/` というディレクトリは無い**
   (実体は `NumberTheory/Transcendental/`)。**存在しないパスを探して 0 件を得ていた。**
   ★改めて測ると **`NumberTheory/SiegelsLemma.lean`** と
   **`NumberTheory/NumberField/House.lean`**(数体上の Siegel の補題、house 版)があり、
   これは超越性証明の**算術側の心臓部**である。「解析的整数論の一分野を入れる作業」ではない。
   ★`Meta/Claim.lean` の `LeanStatus.absent` が `searched` を要求するのは
   まさにこの型の誤りを捕まえるためだった —— **`searched` のパスが実在するかを
   確かめる段が抜けていた。**
2. **`Proposition 4.4, (ii)` の Ore の四角形**。「(i)(b) `preStepSpan` からは出ない」と
   書いていたが、★**見る条を間違えていた**。効くのは **(iii)(d) `coaPreOverEquiv`**
   (`(𝒞^coa-pre)_A ≃ Order(Φ(A))^opp`)である。`Order(Φ(A))` は**前順序圏**で
   `Φ(A)` は `δ ≼ δ+ε` により**有向**だから、スライスは**余フィルター**になる。
   ★★**Ore の四角形は一般の Frobenioid で作れた**
   (`Found/FrdI/Prop44Ore.lean`、`sorry` 無し)。model 限定ではなかった。

★★**穴の形が変わった**——`Proposition 4.4, (ii)` は
「圏論の条件 1 つ」から「**中心拡大の交換子が定める交代双線形形式が消えるか**」になった。

★statement は `Skeleton/FrdI/Def28ProL.lean`・`Skeleton/FrdI/Lemma65SixExp.lean` に
**型で固定した**(`Skeleton/` の `sorry` は正しい状態である)。

★**チェーン待ちがゼロの節は §1 と §3 だけ**である。★★したがって
**いちばん早く満点にできるのは §3(`Proposition 3.2` → `Theorem 3.4`、`Proposition 1.14` 経由)**である。
