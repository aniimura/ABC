...
### ★★木の docstring の断定も疑え（2026-09-08、実害 2 件が確定）

★**持ち場だけでなく、木が過去に書いた断定も検査の対象である。** 波をまたぐ知識は
docstring でしか運ばれないので、★**誤りが伝言ゲームで増幅する**（当日 39 箇所で参照された）。

確定した実害:
- 「1 段では閉じない」→ ★**実は 2 段を測っていた**（次の波が数値で発見）
- 「木にも mathlib にも無い」→ ★**`CyclicJumpNorm.lean` の 542 行に等式で在った**（転記が 1 本ずれ）

⇒ ★**docstring が「無い」「閉じない」「越えられない」と言っていたら、その場で 1 回測る**
（`grep -n <名前> lean/ABC3/Found/PGC/*.lean` か `#check`。0.3 秒〜10 秒）。
★**「前の波がそう書いたから」は根拠にならない。**
★誤りを見つけたら、★**その docstring を直すのではなく（他が読んでいる）、
自分のファイルに「訂正」として書き、どこが誤っていたかを名指しせよ。**
